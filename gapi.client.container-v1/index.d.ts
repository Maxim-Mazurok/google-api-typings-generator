...
declare namespace gapi.client {
    ...
    function load(urlOrObject: "https://container.googleapis.com/$discovery/rest?version=v1"): Promise<void>;
    ...
    namespace container {
        ...
        interface ResourceManagerTags {
            /**
             * TagKeyValue must be in one of the following formats ([KEY]=[VALUE]) 1. `tagKeys/{tag_key_id}=tagValues/{tag_value_id}` 2. `{org_id}/{tag_key_name}={tag_value_name}` 3.
        ...
        interface UpdateNodePoolRequest {
            ...
            confidentialNodes?:
                ConfidentialNodes;
            /**
             * Optional. The desired disk size for nodes in the node pool specified in GB. The smallest allowed disk size is 10GB. Initiates an upgrade operation that migrates the nodes in the
             * node pool to the specified disk size.
             */
            diskSizeGb?:
                string;
            /**
             * Optional. The desired disk type (e.g. 'pd-standard', 'pd-ssd' or 'pd-balanced') for nodes in the node pool. Initiates an upgrade operation that migrates the nodes in the node pool
            ...
            loggingConfig?:
                NodePoolLoggingConfig;
            /**
             * Optional. The desired [Google Compute Engine machine type](https://cloud.google.com/compute/docs/machine-types) for nodes in the node pool. Initiates an upgrade operation that
             * migrates the nodes in the node pool to the specified machine type.
             */
            machineType?:
                string;
