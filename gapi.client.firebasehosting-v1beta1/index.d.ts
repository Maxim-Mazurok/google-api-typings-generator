    ...
    namespace firebasehosting {
        ...
        interface CustomDomain {
            ...
            annotations?:
                { [P in string]: string };
            /**
             * Output only. The SSL certificate Hosting has for this `CustomDomain`'s domain name. For new `CustomDomain`s, this often represents Hosting's intent to create a certificate, rather
             * than an actual cert. Check the `state` field for more.
             */
            cert?:
                Certificate;
            /**
             * A field that lets you specify which SSL certificate type Hosting creates for your domain name. Spark plan `CustomDomain`s only have access to the `GROUPED` cert type, while Blaze
             * plan can select any option.
             */
            certPreference?:
                string;
            ...
            createTime?:
                string;
            /**
             * Output only. The time the `CustomDomain` was deleted; null for `CustomDomains` that haven't been deleted. Deleted `CustomDomains` persist for approximately 30 days, after which time
             * Hosting removes them completely. To restore a deleted `CustomDomain`, make an `UndeleteCustomDomain` request.
             */
            deleteTime?:
                string;
            /**
            ...
            etag?:
                string;
            /** Output only. The minimum time before a soft-deleted `CustomDomain` is completely removed from Hosting; null for `CustomDomains` that haven't been deleted. */
            expireTime?:
                string;
            ...
            ownershipState?:
                string;
            /**
             * Output only. A field that, if true, indicates that Hosting's systems are attmepting to make the `CustomDomain`'s state match your preferred state. This is most frequently `true`
             * when initially provisioning a `CustomDomain` after a `CreateCustomDomain` request or when creating a new SSL certificate to match an updated `cert_preference` after an
             * `UpdateCustomDomain` request.
             */
            reconciling?:
                boolean;
            /**
             * A domain name that this CustomDomain should direct traffic towards. If specified, Hosting will respond to requests against this CustomDomain with an HTTP 301 code, and route traffic
        ...
        interface CustomDomainMetadata {
            ...
            issues?:
                Status[];
            /**
             * A set of DNS record updates and ACME challenges that allow you to transition domain names to Firebase Hosting with zero downtime. These updates allow Hosting's to create an SSL
