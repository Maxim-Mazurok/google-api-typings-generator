...
declare namespace gapi.client {
    ...
    namespace retail {
        ...
        interface GoogleCloudRetailV2AddFulfillmentPlacesRequest {
             ...
             * [a-zA-Z0-9_-]+, such as "store1" or "REGION-2". Otherwise, an INVALID_ARGUMENT error is returned. If the total number of place IDs exceeds 2000 for this type after adding, then the
        ...
        interface GoogleCloudRetailV2ColorInfo {
             ...
             * color. May consider using single "Mixed" instead of multiple values. A maximum of 5 colors are allowed. Each value must be a UTF-8 encoded string with a length limit of 128
        ...
        interface GoogleCloudRetailV2CompleteQueryResponse {
            ...
            /**
             * Matched recent searches of this user. This field is a restricted feature. Contact Retail Search support team if you are interested in enabling it. This feature is only available
             * when CompleteQueryRequest.visitor_id field is set and UserEvent is imported. The recent searches satisfy the follow rules: * They are ordered from latest to oldest. * They are
             * matched with CompleteQueryRequest.query case insensitively. * They are transformed to lower cases. * They are UTF-8 safe. Recent searches are deduplicated. More recent searches will
        ...
        interface GoogleCloudRetailV2FulfillmentInfo {
            /**
             * The IDs for this type, such as the store IDs for FulfillmentInfo.type.pickup-in-store or the region IDs for FulfillmentInfo.type.same-day-delivery. A maximum of 2000 values are
             * allowed. Each value must be a string with a length limit of 10 characters, matching the pattern [a-zA-Z0-9_-]+, such as "store1" or "REGION-2". Otherwise, an INVALID_ARGUMENT error
             * is returned.
             */
            placeIds?: string[];
            /**
        ...
        interface GoogleCloudRetailV2Product {
             ...
             * needs to pass all below criteria, otherwise an INVALID_ARGUMENT error is returned: * Max entries count: 200 by default; 100 for Type.VARIANT. * The key must be a UTF-8 encoded
             * string with a length limit of 128 characters. * Max indexable entries count: 200 by default; 40 for Type.VARIANT. * Max searchable entries count: 30. * For indexable attribute, the
             * key must match the pattern: a-zA-Z0-9*. For example, key0LikeThis or KEY_1_LIKE_THIS.
             */
            attributes?: { [P in string]: GoogleCloudRetailV2CustomAttribute };
             ...
             * error is returned. Google Merchant Center property [brand](https://support.google.com/merchants/answer/6324351). Schema.org property [Product.brand](https://schema.org/brand).
             */
            brands?: string[];
            /**
            ...
            /**
             * The color of the product. Google Merchant Center property [color](https://support.google.com/merchants/answer/6324487). Schema.org property
             * [Product.color](https://schema.org/color).
             */
            colorInfo?: GoogleCloudRetailV2ColorInfo;
            /**
             * The condition of the product. Strongly encouraged to use the standard values: "new", "refurbished", "used". A maximum of 5 values are allowed per Product. Each value must be a UTF-8
             * encoded string with a length limit of 128 characters. Otherwise, an INVALID_ARGUMENT error is returned. Google Merchant Center property
             ...
             * product can still be retrieved by ProductService.GetProduct and ProductService.ListProducts. Google Merchant Center property
             ...
             * returned. Google Merchant Center property [gtin](https://support.google.com/merchants/answer/6324461). Schema.org property [Product.isbn](https://schema.org/isbn) or
             * [Product.gtin8](https://schema.org/gtin8) or [Product.gtin12](https://schema.org/gtin12) or [Product.gtin13](https://schema.org/gtin13) or
             * [Product.gtin14](https://schema.org/gtin14). If the value is not a valid GTIN, an INVALID_ARGUMENT error is returned.
             */
            gtin?: string;
            /**
            ...
            /**
             * Language of the title/description and other string attributes. Use language tags defined by BCP 47. For product prediction, this field is ignored and the model automatically detects
             * the text language. The Product can include text in different languages, but duplicating Products to provide text in multiple languages can result in degraded model performance. For
             * product search this field is in use. It defaults to "en-US" if unset.
             */
            languageCode?: string;
            /**
             * The material of the product. For example, "leather", "wooden". A maximum of 5 values are allowed. Each value must be a UTF-8 encoded string with a length limit of 128 characters.
             * Otherwise, an INVALID_ARGUMENT error is returned. Google Merchant Center property [material](https://support.google.com/merchants/answer/6324410). Schema.org property
            ...
            name?: string;
            /**
             * The pattern or graphic print of the product. For example, "striped", "polka dot", "paisley". A maximum of 5 values are allowed per Product. Each value must be a UTF-8 encoded string
             * with a length limit of 128 characters. Otherwise, an INVALID_ARGUMENT error is returned. Google Merchant Center property
             ...
             * length limit of 128 characters. Otherwise, an INVALID_ARGUMENT error is returned. Google Merchant Center property [size](https://support.google.com/merchants/answer/6324492),
             * [size_type](https://support.google.com/merchants/answer/6324497) and [size_system](https://support.google.com/merchants/answer/6324502). Schema.org property
            ...
            title?: string;
            /**
             * Input only. The TTL (time to live) of the product. If it is set, expire_time is set as current timestamp plus ttl. The derived expire_time is returned in the output and ttl is left
             * blank when retrieving the Product. If it is set, the product is not available for SearchService.Search after current timestamp plus ttl. However, the product can still be retrieved
             * by ProductService.GetProduct and ProductService.ListProducts.
             */
            ttl?: string;
        ...
        interface GoogleCloudRetailV2ProductLevelConfig {
             ...
             * merchant_center_product_id_field is `itemGroupId`, an INVALID_ARGUMENT error is returned. See [Using product
        ...
        interface GoogleCloudRetailV2SearchRequest {
             ...
             * more details at this [user guide](/retail/private/docs/filter-and-order#filter). If this field is unrecognizable, an INVALID_ARGUMENT is returned.
             */
            filter?: string;
            /**
             ...
             * more details at this [user guide](/retail/private/docs/filter-and-order#order). If this field is unrecognizable, an INVALID_ARGUMENT is returned.
             */
            orderBy?: string;
            /**
            ...
            /**
             * The keys to fetch and rollup the matching variant Products attributes. The attributes from all the matching variant Products are merged and de-duplicated. Notice that rollup variant
             * Products attributes will lead to extra query latency. Maximum number of keys is 10. For FulfillmentInfo, a fulfillment type and a fulfillment ID must be provided in the format of
             * "fulfillmentType.fulfillmentId". E.g., in "pickupInStore.store123", "pickupInStore" is fulfillment type and "store123" is the store ID. Supported keys are: * colorFamilies * price *
             * originalPrice * discount * attributes.key, where key is any key in the Product.attributes map. * pickupInStore.id, where id is any FulfillmentInfo.place_ids for FulfillmentInfo.type
             * "pickup-in-store". * shipToStore.id, where id is any FulfillmentInfo.place_ids for FulfillmentInfo.type "ship-to-store". * sameDayDelivery.id, where id is any
             * FulfillmentInfo.place_ids for FulfillmentInfo.type "same-day-delivery". * nextDayDelivery.id, where id is any FulfillmentInfo.place_ids for FulfillmentInfo.type "next-day-delivery".
             * * customFulfillment1.id, where id is any FulfillmentInfo.place_ids for FulfillmentInfo.type "custom-type-1". * customFulfillment2.id, where id is any FulfillmentInfo.place_ids for
             * FulfillmentInfo.type "custom-type-2". * customFulfillment3.id, where id is any FulfillmentInfo.place_ids for FulfillmentInfo.type "custom-type-3". * customFulfillment4.id, where id
             * is any FulfillmentInfo.place_ids for FulfillmentInfo.type "custom-type-4". * customFulfillment5.id, where id is any FulfillmentInfo.place_ids for FulfillmentInfo.type
             ...
             * device. This unique identifier should not change if the visitor logs in or out of the website. The field must be a UTF-8 encoded string with a length limit of 128 characters.
             * Otherwise, an INVALID_ARGUMENT error is returned.
             */
            visitorId?: string;
        }
        ...
        interface GoogleCloudRetailV2SearchRequestBoostSpecConditionBoostSpec {
             ...
             * Examples: * To boost products with product ID "product_1" or "product_2", and color "Red" or "Blue": *(id: ANY("product_1", "product_2")) * *AND * *(colorFamilies: ANY("Red",
             * "Blue")) *
             */
            condition?: string;
        }
        ...
        interface GoogleCloudRetailV2SearchRequestFacetSpecFacetKey {
             ...
             * is not specified: * textual_field = *# The Product.brands. * | "brands"; *# The Product.categories. * | "categories"; *# The Audience.genders. * | "genders"; *# The
             * Audience.age_groups. * | "ageGroups"; *# The Product.availability. Value is one of * *# "IN_STOCK", "OUT_OF_STOCK", PREORDER", "BACKORDER". * | "availability"; *# The
             * ColorInfo.color_families. * | "colorFamilies"; *# The ColorInfo.colors. * | "colors"; *# The Product.sizes. * | "sizes"; *# The Product.materials. * | "materials"; *# The
             * Product.patterns. * | "patterns"; *# The Product.conditions. * | "conditions"; *# The textual custom attribute in Product object. Key can * *# be any key in the Product.attributes
             * map * *# if the attribute values are textual. * | "attributes.key"; *# The FulfillmentInfo.place_ids for FulfillmentInfo.type * *# "pickup-in-store". * | "pickupInStore"; *# The
             * FulfillmentInfo.place_ids for FulfillmentInfo.type * *# "ship-to-store". * | "shipToStore"; *# The FulfillmentInfo.place_ids for FulfillmentInfo.type * *# "same-day-delivery". * |
             * "sameDayDelivery"; *# The FulfillmentInfo.place_ids for FulfillmentInfo.type * *# "next-day-delivery". * | "nextDayDelivery"; *# The FulfillmentInfo.place_ids for
             * FulfillmentInfo.type * *# "custom-type-1". * | "customFulfillment1"; *# The FulfillmentInfo.place_ids for FulfillmentInfo.type * *# "custom-type-2". * | "customFulfillment2"; *# The
             * FulfillmentInfo.place_ids for FulfillmentInfo.type * *# "custom-type-3". * | "customFulfillment3"; *# The FulfillmentInfo.place_ids for FulfillmentInfo.type * *# "custom-type-4". *
             * | "customFulfillment4"; *# The FulfillmentInfo.place_ids for FulfillmentInfo.type * *# "custom-type-5". * | "customFulfillment5"; * numerical_field = *# The PriceInfo.price. * |
             * "price"; *# The discount. Computed by (original_price-price)/price * | "discount"; *# The Rating.average_rating. * | "rating"; *# The Rating.rating_count. * | "ratingCount"; *# The
             * numerical custom attribute in Product object. Key can * *# be any key in the Product.attributes map * *# if the attribute values are numerical. * | "attributes.key";
             */
            key?: string;
            /**
        ...
        interface GoogleCloudRetailV2SearchResponseSearchResult {
             ...
             * value with type google.protobuf.Value. For example, {key: "pickupInStore.store1" value { number_value: 10 }} means a there are 10 variants in this product are available in the store
             * "store1".
             */
        ...
        interface GoogleCloudRetailV2SetInventoryRequest {
             ...
             * to false, an INVALID_ARGUMENT error is returned if the Product is not found.
             */
            allowMissing?: boolean;
            /**
            ...
            /**
             * Indicates which inventory fields in the provided Product to update. If not set or set with empty paths, all inventory fields will be updated. If an unsupported or unknown field is
        ...
        interface GoogleCloudRetailV2UserEvent {
            ...
            cartId?: string;
            /**
             * The main completion details related to the event. In a `completion` event, this field represents the completions returned to the end user and the clicked completion by the end user.
             ...
             * `completion`: Completion query result showed/clicked. * `detail-page-view`: Products detail page viewed. * `home-page-view`: Homepage viewed. * `promotion-offered`: Promotion is
             * offered to a user. * `promotion-not-offered`: Promotion is not offered to a user. * `purchase-complete`: User finishing a purchase. * `search`: Product search. *
        ...
        interface PlacementsResource {
            ...
            search(request: {
                 ...
                 * identify the set of models that will be used to make the search. We currently support one placement with the following ID: * `default_search`.
                 */
                placement: string;
                /** Returns response with indentations and line breaks. */
                prettyPrint?: boolean;
        ...
        interface CatalogsResource {
            ...
            completeQuery(request?: {
                ...
                dataset?: string;
                /**
                 * The device type context for completion suggestions. It is useful to apply different suggestions on different device types, e.g. DESKTOP, MOBILE. If it is empty, the suggestions
                 * are across all device types. Supported formats: * UNKNOWN_DEVICE_TYPE * DESKTOP * MOBILE * A customized string starts with OTHER_, e.g. OTHER_IPHONE.
                 */
                deviceType?: string;
                /** Selector specifying which fields to include in a partial response. */
                fields?: string;
