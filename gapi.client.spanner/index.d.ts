    ...
    namespace spanner {
        ...
        interface BackupOperationsResource {
            ...
            list(request?: {
                 ...
                 * AND` \ `metadata.database:prod` - Returns operations where: * The operation's metadata type is CreateBackupMetadata. * The database the backup was taken from has a name
                 * containing the string "prod". * `(metadata.@‍type=type.googleapis.com/google.spanner.admin.database.v1.CreateBackupMetadata) AND` \ `(metadata.name:howl) AND` \
                 * `(metadata.progress.start_time < \"2018-03-28T14:50:00Z\") AND` \ `(error:*)` - Returns operations where: * The operation's metadata type is CreateBackupMetadata. * The backup
                 * name contains the string "howl". * The operation started before 2018-03-28T14:50:00Z. * The operation resulted in an error. *
                 ...
                 * \"2022-01-18T14:50:00Z\") AND` \ `(error:*)` - Returns operations where: * The operation's metadata type is CopyBackupMetadata. * The source backup of the copied backup name
                 * contains the string "test". * The operation started before 2022-01-18T14:50:00Z. * The operation resulted in an error. *
                 ...
                 * where: * The operation's metadata matches either of criteria: * The operation's metadata type is CreateBackupMetadata AND the database the backup was taken from has name
                 * containing string "test_db" * The operation's metadata type is CopyBackupMetadata AND the backup the backup was copied from has name containing string "test_bkp" * The operation
