...
declare namespace gapi.client {
    ...
    function load(name: "servicedirectory", version: "v1"): PromiseLike<void>;
    ...
    namespace servicedirectory {
        ...
        interface Endpoint {
             ...
             * accoss all key-value pairs. Annotations that goes beyond any these limits will be rejected. - Valid annotation keys have two segments: an optional prefix and name, separated by a
             * slash (/). The name segment is required and must be 63 characters or less, beginning and ending with an alphanumeric character ([a-z0-9A-Z]) with dashes (-), underscores (_), dots
             * (.), and alphanumerics between. The prefix is optional. If specified, the prefix must be a DNS subdomain: a series of DNS labels separated by dots (.), not longer than 253
             * characters in total, followed by a slash (/). Annotations that fails to meet these requirements will be rejected. - The '(*.)google.com/' and '(*.)googleapis.com/' prefixes are
             * reserved for system annotations managed by Service Directory. If the user tries to write to these keyspaces, those entries will be silently ignored by the system. Note: This field
             * is equivalent to the 'metadata' field in the v1beta1 API. They have the same syntax and read/write to the same location in Service Directory.
             */
        ...
        interface ResolveServiceRequest {
            /**
             * Optional. The filter applied to the endpoints of the resolved service. General filter string syntax: *`field operator value`* (*`logical connector`*) *`field`* can be `name` or
             * `metadata.`*`key`* for map field. *`operator`* can be `\<`, `>`, `\<=`, `>=`, `!=`, `=`, `:`. Of which `:` means `HAS` and is roughly the same as `=`. *`value`* must be the same
             * data type as the field. *`logical connector*` can be `AND`, `OR`, `NOT`. Examples of valid filters: * `metadata.owner` returns endpoints that have a label with the key `owner`, this
             * is the same as `metadata:owner` * `metadata.protocol=gRPC` returns endpoints that have key/value `protocol=gRPC` * `metadata.owner!=sd AND metadata.foo=bar` returns endpoints that
             * have `owner` field in metadata with a value that is not `sd` and have the key/value `foo=bar`.
             */
