    ...
    namespace cloudidentity {
        ...
        interface DevicesResource {
            ...
            get(request?: {
                ...
                callback?: string;
                /**
                 * Optional. [Resource name](https://cloud.google.com/apis/design/resource_names) of the Customer in format: `customers/{customer_id}`, where customer_id is the customer to whom
                 * the device belongs.
                 */
                customer?: string;
                /** Selector specifying which fields to include in a partial response. */
        ...
        interface GroupsResource {
            ...
            search(request?: {
                ...
                oauth_token?: string;
                /**
                 * The ordering of groups for the display name or email in the search groups response. The syntax for this field can be found at
                 * https://cloud.google.com/apis/design/design_patterns#sorting_order. Example: Sort by the ascending name: order_by="display_name" Sort by the descending group key email:
                 ...
                 * in labels`). * Can contain an optional equality operator on `domain_name` or `startsWith/contains/equality` operator on `group_key`, e.g. `domain_name == 'abc.com'`,
                 * `group_key.startsWith('dev')`, `group_key.contains('dev'), group_key == 'dev@abc.com'` * Can contain an optional `startsWith/contains/equality` operator on `display_name`, such
                 * as `display_name.startsWith('dev')` , `display_name.contains('dev')`, `display_name == 'dev'`
                 */
                query?: string;
                /** Available to use for quota purposes for server-side applications. Can be any arbitrary string assigned to a user, but should not exceed 40 characters. */
                quotaUser?: string;
        ...
        interface InboundSsoAssignmentsResource {
             ...
             * `/v1beta1/inboundSsoAssignments/0abcdefg1234567&update_mask=rank` with a body of `{ "rank": 1 }` moves that (presumably group-targeted) SSO assignment to the highest priority and
             * shifts any other group-targeted assignments down in priority.
             */
            patch(request: {
                /** V1 error format. */
                "$.xgafv"?: string;
