...
declare namespace gapi.client {
    ...
    namespace chat {
        ...
        interface AttachmentDataRef {
            /**
             * Opaque token containing a reference to an uploaded attachment. Treated by clients as an opaque string and used to create or update Chat messages with attachments. [Developer
        ...
        interface Message {
            ...
            createTime?:
                string;
            /**
             * Output only. The time at which the message was deleted in Google Chat server. If the message is never deleted, this field is empty. [Developer
             * Preview](https://developers.google.com/workspace/preview).
             */
            deleteTime?:
                string;
            /** Output only. Information about a deleted message. A message is deleted when `delete_time` is set. [Developer Preview](https://developers.google.com/workspace/preview). */
            deletionMetadata?:
                DeletionMetadata;
            /** Output only. The list of emoji reaction summaries on the message. [Developer Preview](https://developers.google.com/workspace/preview). */
        ...
        interface Space {
            ...
            spaceDetails?:
                SpaceDetails;
            /** The message history state for messages and threads in this space. [Developer Preview](https://developers.google.com/workspace/preview). */
            spaceHistoryState?:
                string;
            /** Output only. The threading state in the Chat space. */
            spaceThreadingState?:
                string;
            /** The type of space. Required when creating or updating a space. Output only for other usage. [Developer Preview](https://developers.google.com/workspace/preview). */
            spaceType?:
                string;
        ...
        interface MembersResource {
            /**
             * [Developer Preview](https://developers.google.com/workspace/preview): Creates a human membership or app membership for the calling app. Creating memberships for other apps isn't
             * supported. Requires [user authentication](https://developers.google.com/chat/api/guides/auth/users) and the `chat.memberships` (for human membership) or `chat.memberships.app` (for
             * app membership) scope. To specify the member to add, set the `membership.member.name` in the `CreateMembershipRequest`: - To add the calling app to the space, use `users/app`. - To
             * add a human user, use `users/{user}`, where `{user}` is either the `{person_id}` for the [person](https://developers.google.com/people/api/rest/v1/people) from the People API, or
             * the `id` for the [user](https://developers.google.com/admin-sdk/directory/reference/rest/v1/users) in the Directory API. For example, if the People API `Person` `resourceName` is
             * `people/123456789`, you can add the user to the space by setting the `membership.member.name` to `users/123456789`.
             */
            create(request: {
                /** V1 error format. */
                "$.xgafv"?:
            ...
            /**
             * [Developer Preview](https://developers.google.com/workspace/preview): Deletes a membership. Requires [user authentication](https://developers.google.com/chat/api/guides/auth/users)
        ...
        interface ReactionsResource {
            /**
             * [Developer Preview](https://developers.google.com/workspace/preview): Creates a reaction and adds it to a message. Requires [user
            ...
            /**
             * [Developer Preview](https://developers.google.com/workspace/preview): Deletes a reaction to a message. Requires [user
            ...
            /**
             * [Developer Preview](https://developers.google.com/workspace/preview): Lists reactions to a message. Requires [user
        ...
        interface MessagesResource {
            ...
            /**
             * [Developer Preview](https://developers.google.com/workspace/preview): Lists messages in a space that the caller is a member of, including messages from blocked members and spaces.
        ...
        interface SpacesResource {
            /**
             * [Developer Preview](https://developers.google.com/workspace/preview): Creates a named space. Spaces grouped by topics or that have guest access aren't supported. Requires [user
            ...
            /**
             * [Developer Preview](https://developers.google.com/workspace/preview): Deletes a named space. Always performs a cascading delete, which means that the space's child resources—like
             * messages posted in the space and memberships in the space—are also deleted. Requires [user authentication](https://developers.google.com/chat/api/guides/auth/users) from a user who
             * has permission to delete the space, and the `chat.delete` scope.
             */
            delete(request?: {
                /** V1 error format. */
                "$.xgafv"?:
            ...
            /**
             * [Developer Preview](https://developers.google.com/workspace/preview): Returns the existing direct message with the specified user. With [user
             * authentication](https://developers.google.com/chat/api/guides/auth/users), returns the direct message space between the specified user and the authenticated user. With [service
             * account authentication](https://developers.google.com/chat/api/guides/auth/service-accounts), returns the direct message space between the specified user and the calling Chat app.
             * If no direct message space is found, returns a `404 NOT_FOUND` error. Requires [user authentication](https://developers.google.com/chat/api/guides/auth/users) or [service account
            ...
            list(request?: {
                 ...
                 * still validates the query syntax with service accounts, so invalid queries are still rejected. [Developer Preview](https://developers.google.com/workspace/preview).
                 */
                filter?:
                    string;
                /** API key. Your API key identifies your project and provides you with API access, quota, and reports. Required unless you provide an OAuth 2.0 token. */
            ...
            /**
             * [Developer Preview](https://developers.google.com/workspace/preview): Updates a space. Requires [user authentication](https://developers.google.com/chat/api/guides/auth/users) and
            ...
            body: Space): Request<Space>;
            /**
             * [Developer Preview](https://developers.google.com/workspace/preview): Creates a space and adds specified users to it. The calling user is automatically added to the space, and
             * shouldn't be specified as a membership in the request. To specify the human members to add, add memberships with the appropriate `member.name` in the `SetUpSpaceRequest`. To add a
             * human user, use `users/{user}`, where `{user}` is either the `{person_id}` for the [person](https://developers.google.com/people/api/rest/v1/people) from the People API, or the `id`
             * for the [user](https://developers.google.com/admin-sdk/directory/reference/rest/v1/users) in the Admin SDK Directory API. For example, if the People API `Person` `resourceName` is
             * `people/123456789`, you can add the user to the space by including a membership with `users/123456789` as the `member.name`. For a space or group chat, if the caller blocks or is
             ...
             * `Space.singleUserBotDm` to true and don't specify any memberships. You can only use this method to add app memberships to DMs. To add the calling app as a member of other space
             * types, use [create membership](https://developers.google.com/chat/api/reference/rest/v1/spaces.members/create) If a DM already exists between two users, even when one user blocks
             * the other at the time a request is made, then the existing DM is returned. Spaces with threaded replies or guest access aren't supported. Requires [user
             * authentication](https://developers.google.com/chat/api/guides/auth/users) and the `chat.spaces.create` or `chat.spaces` scope.
             */
            setup(request: {
                /** V1 error format. */
