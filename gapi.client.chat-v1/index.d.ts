...
declare namespace gapi.client {
    ...
    namespace chat {
        ...
        interface MembersResource {
            /**
             * [Developer Preview](https://developers.google.com/workspace/preview): Creates a human membership or app membership for the calling app. Creating memberships for other apps is not
             ...
             * the `id` for the [user](https://developers.google.com/admin-sdk/directory/reference/rest/v1/users) in the Admin SDK Directory API. For example, if the People API `Person`
             * `resourceName` is `people/123456789`, the user can be added to the space by setting the `membership.member.name` to `users/123456789`.
             */
            create(request: {
                /** V1 error format. */
                "$.xgafv"?:
        ...
        interface ReactionsResource {
            ...
            list(request?: {
                ...
                oauth_token?:
                    string;
                /**
                 * Optional. The maximum number of reactions returned. The service may return fewer reactions than this value. If unspecified, the default value is 25. The maximum value is 200;
                 ...
                 * filter value should match the call that provided the page token. Passing a different value may lead to unexpected results.
                 */
                pageToken?:
                    string;
        ...
        interface MessagesResource {
            ...
            list(request?: {
                ...
                orderBy?:
                    string;
                /**
                 * The maximum number of messages returned. The service may return fewer messages than this value. If unspecified, at most 25 are returned. The maximum value is 1000; values above
                 * 1000 are coerced to 1000. Negative values return an `INVALID_ARGUMENT` error.
                 */
                pageSize?:
                    number;
                /**
                 * Optional, if resuming from a previous query. A page token received from a previous list messages call. Provide this to retrieve the subsequent page. When paginating, all other
                 * parameters provided should match the call that provided the page token. Passing different values to the other parameters may lead to unexpected results.
                 */
                pageToken?:
                    string;
        ...
        interface SpacesResource {
            /**
             * [Developer Preview](https://developers.google.com/workspace/preview): Creates a named space. Spaces grouped by topics or that have guest access are not supported. Requires [user
             ...
             * messages posted in the space and memberships in the space - are also deleted. Requires [user authentication](https://developers.google.com/chat/api/guides/auth/users) from a user
             * who has permission to delete the space, and the `chat.delete` scope.
             */
            delete(request?: {
                /** V1 error format. */
                "$.xgafv"?:
            ...
            findDirectMessage(request?: {
                 ...
                 * [user](https://developers.google.com/admin-sdk/directory/reference/rest/v1/users) in the Admin SDK Directory API. For example, if the People API `Person.resourceName` is
            ...
            list(request?: {
                 ...
                 * `GROUP_CHAT` (the `space_type` cannot be `SPACE_TYPE_UNSPECIFIED`). To query for multiple space types, use the `OR` operator. For example, the following queries are valid: ```
                 ...
                 * account authentication](https://developers.google.com/chat/api/guides/auth/service-accounts), this field is ignored and the query always returns all spaces. But Chat API still
                 * validates the query syntax with service accounts, so invalid queries are still rejected. [Developer Preview](https://developers.google.com/workspace/preview).
                 */
                filter?:
                    string;
                /** API key. Your API key identifies your project and provides you with API access, quota, and reports. Required unless you provide an OAuth 2.0 token. */
            ...
            patch(request: {
                ...
                quotaUser?:
                    string;
                /**
                 * Required. The field paths to be updated, comma separated if there are multiple. Currently supported field paths: - display_name (Only supports changing the display name of a
                 * space with the SPACE type, or when also including the `space_type` mask to change a GROUP_CHAT space type to SPACE. Trying to update the display name of a GROUP_CHAT or a
                 ...
                 * `space_type` in the update mask and ensure that the specified space has a non-empty display name and the SPACE space type. Including the `space_type` mask and the SPACE type in
                 * the specified space when updating the display name is optional if the existing space already has the SPACE type. Trying to update the space type in other ways results in an
             ...
             * the other at the time a request is made, then the existing DM is returned. Spaces with threaded replies or guest access are not supported. Requires [user
             * authentication](https://developers.google.com/chat/api/guides/auth/users) and the `chat.spaces.create` or `chat.spaces` scope.
             */
            setup(request: {
                /** V1 error format. */
