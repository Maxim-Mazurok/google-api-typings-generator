...
declare namespace gapi.client {
    ...
    namespace chat {
        ...
        interface GoogleAppsCardV1Action {
             ...
             * [LoadIndicator.NONE](https://developers.google.com/workspace/add-ons/reference/rpc/google.apps.card.v1#loadindicator) for actions, `persist_values` = `true`is recommended, as it
             * ensures that any changes made by the user after form or on change actions are sent to the server are not overwritten by the response. If `false`, the form values are cleared when
             * the action is triggered. When `persist_values` is set to `false`, it is strongly recommended that the card use
             * [LoadIndicator.SPINNER](https://developers.google.com/workspace/add-ons/reference/rpc/google.apps.card.v1#loadindicator) for all actions, as this locks the UI to ensure no changes
             * are made by the user while the action is being processed. Not supported by Chat apps.
             */
        ...
        interface GoogleAppsCardV1Button {
             ...
             * new browser tab and navigates to the Google Chat developer documentation at https://developers.google.com/chat". Has no effect when an icon is set; use `icon.alt_text` instead.
             */
            altText?: string;
            /**
        ...
        interface GoogleAppsCardV1DecoratedText {
            ...
            /**
             * An icon displayed after the text. Supports [standard](https://developers.google.com/chat/api/guides/message-formats/cards#builtinicons) and
            ...
            /**
             * A switch widget can be clicked to change its state and trigger an action. Currently supported in [dialogs](https://developers.google.com/chat/how-tos/dialogs). Support for [card
        ...
        interface GoogleAppsCardV1Icon {
            /**
             * Optional. A description of the icon used for accessibility. If unspecified, a default value is provided. As a best practice, you should set a helpful description. For example, if an
             * icon displays a user's account portrait, you could describe it as "A user's account portrait." If the icon displays in a Button, this alt text takes precedence and overwrites the
             * button's alt text, so you should write alt text for the button: Set descriptive text that lets users know what the button does. For example, if a button opens a hyperlink, you might
             * write: "Opens a new browser tab and navigates to the Google Chat developer documentation at https://developers.google.com/chat".
             */
            altText?: string;
            /**
            ...
            imageType?: string;
            /**
             * Display one of the standard icons provided by Google Workspace. For example, to display an airplane icon, specify `AIRPLANE`. For a bus, specify `BUS`. For a full list of supported
             * icons, see [standard icons](https://developers.google.com/chat/api/guides/message-formats/cards).
             */
            knownIcon?: string;
        }
        ...
        interface GoogleAppsCardV1TextInput {
            ...
            /**
             * Text that appears inside the text input field meant to assist users by prompting them to enter a certain value. This text is not visible after users begin typing. Required if
             * `label` is unspecified. Otherwise, optional.
             */
            hintText?: string;
            /**
        ...
        interface GoogleAppsCardV1Widget {
             ...
             * "buttons": [ "button": { "text": "Edit", "color": { "red": 0, "green": 0, "blue": 1, "alpha": 1 } "disabled": true }, "button": { "icon": { "knownIcon": "INVITE" "altText": "check
             ...
             * datetime picker to schedule an appointment: ``` "date_time_picker": { "name": "appointment_time", "label": "Book your appointment at:", "type": "DateTimePickerType.DATE_AND_TIME",
             ...
             * "topLabel": "Email Address", "text": "sasha@example.com", "bottomLabel": "This is a new Email address!", "switchWidget": { "name": "has_send_welcome_email_to_sasha", "selected":
             ...
             * number of columns. A grid with 10 items and 2 columns has 5 rows. A grid with 11 items and 2 columns has 6 rows. Currently supported in
             * [dialogs](https://developers.google.com/chat/how-tos/dialogs). Support for [card messages](https://developers.google.com/chat/api/guides/message-formats/cards) is coming soon. For
             * example, the following JSON creates a 2 column grid with a single item: ``` "grid": { "title": "A fine collection of items", "numColumns": 2, "borderStyle": { "type": "STROKE",
             * "cornerRadius": 4.0 }, "items": [ "image": { "imageUri": "https://www.example.com/image.png", "cropStyle": { "type": "SQUARE" }, "borderStyle": { "type": "STROKE" } }, "title": "An
             * item", "textAlignment": "CENTER" ], "onClick": { "openLink": { "url":"https://www.example.com" } } } ```
             */
            ...
            /**
             * Displays a selection control that lets users select items. Selection controls can be check boxes, radio buttons, switches, or dropdown menus. Currently supported in
             * [dialogs](https://developers.google.com/chat/how-tos/dialogs). Support for [card messages](https://developers.google.com/chat/api/guides/message-formats/cards) is coming soon. For
             * example, the following JSON creates a dropdown menu that lets users choose a size: ``` "selectionInput": { "name": "size", "label": "Size" "type": "SelectionType.DROPDOWN", "items":
             * [ { "text": "S", "value": "small", "selected": false }, { "text": "M", "value": "medium", "selected": true }, { "text": "L", "value": "large", "selected": false }, { "text": "XL",
             * "value": "extra_large", "selected": false } ] } ```
             */
            selectionInput?: GoogleAppsCardV1SelectionInput;
            /**
             * Displays a text box that users can type into. Currently supported in [dialogs](https://developers.google.com/chat/how-tos/dialogs). Support for [card
             * messages](https://developers.google.com/chat/api/guides/message-formats/cards) is coming soon. For example, the following JSON creates a text input for an email address: ```
             * "textInput": { "name": "mailing_address", "label": "Mailing Address" } ``` As another example, the following JSON creates a text input for a programming language with static
             * suggestions: ``` "textInput": { "name": "preferred_programing_language", "label": "Preferred Language", "initialSuggestions": { "items": [ { "text": "C++" }, { "text": "Java" }, {
        ...
        interface Message {
             ...
             * supported widgets include: - `TextParagraph` - `DecoratedText` - `Image` - `ButtonList` - `Divider`
             */
            cardsV2?: CardWithId[];
            /**
        ...
        interface Thread {
             ...
             * thread](/chat/api/guides/crudl/messages#start_or_reply_to_a_message_thread). For other requests, this is an output only field.
             */
            threadKey?: string;
        }
        interface TimeInput {
        ...
        interface ConversationsResource {
            ...
            messages(request: {
                 ...
                 * example usage, see [Start or reply to a message thread](/chat/api/guides/crudl/messages#start_or_reply_to_a_message_thread).
                 */
                threadKey?: string;
        ...
        interface MembersResource {
            /**
             * [Developer Preview](https://developers.google.com/workspace/preview): Returns a membership. Requires [authentication](https://developers.google.com/chat/api/guides/auth/). Fully
             * supports [service account authentication](https://developers.google.com/chat/api/guides/auth/service-accounts). Supports [user
             * authentication](https://developers.google.com/chat/api/guides/auth/users) as part of the [Google Workspace Developer Preview
             * Program](https://developers.google.com/workspace/preview), which grants early access to certain features. [User
            ...
            /**
             * [Developer Preview](https://developers.google.com/workspace/preview): Lists memberships in a space. Requires [authentication](https://developers.google.com/chat/api/guides/auth/).
             * Fully supports [service account authentication](https://developers.google.com/chat/api/guides/auth/service-accounts). Supports [user
             * authentication](https://developers.google.com/chat/api/guides/auth/users) as part of the [Google Workspace Developer Preview
             * Program](https://developers.google.com/workspace/preview), which grants early access to certain features. [User
             * authentication](https://developers.google.com/chat/api/guides/auth/users) requires the `chat.memberships` or `chat.memberships.readonly` authorization scope.
             */
            list(request?: {
                ...
                pageSize?: number;
                /**
                 * A page token, received from a previous list memberships call. Provide this to retrieve the subsequent page. When paginating, all other parameters provided should match the call
                 * that provided the page token. Passing different values to the other parameters may lead to unexpected results.
                 */
                pageToken?: string;
        ...
        interface MessagesResource {
             ...
             * authentication](https://developers.google.com/chat/api/guides/auth/users) requires the `chat.messages` or `chat.messages.create` authorization scope.
             */
            create(request: {
                /** V1 error format. */
                "$.xgafv"?: string;
