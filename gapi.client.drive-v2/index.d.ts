...
declare namespace gapi.client {
    ...
    namespace drive {
        ...
        interface File {
            ...
            /** Output only. A short-lived link to the file's thumbnail. Typically lasts on the order of hours. Only populated when the requesting app can access the file's content. */
            thumbnailLink?:
                string;
        ...
        interface Permission {
            ...
            etag?:
                string;
            /**
             * The time at which this permission will expire (RFC 3339 date-time). Expiration dates have the following restrictions: - They cannot be set on shared drive items - They can only be
             * set on user and group permissions - The date must be in the future - The date cannot be more than a year in the future - The date can only be set on drive.permissions.update or
        ...
        interface FilesResource {
             ...
             * `alt=media` only works if the file is stored in Drive. To download Google Docs, Sheets, and Slides use [`files.export`](/drive/api/reference/rest/v2/files/export) instead. For
             * further information on downloading files, refer to [Download files](/drive/api/guides/manage-downloads).
             */
            get(request?: {
                /** V1 error format. */
                "$.xgafv"?:
            ...
            /**
             * Inserts a new file. This method supports an *‍/upload* URI and accepts uploaded media with the following characteristics: - *Maximum file size:* 5120GB - *Accepted Media MIME
             * types:*`*‍/*` Note: Specify a valid MIME type, rather than the literal `*‍/*` value. The literal `*‍/*` is only used to indicate that any valid MIME type can be uploaded. For
             * further information on uploading files, refer to [Upload files](/drive/api/guides/manage-uploads). Apps creating shortcuts with `files.insert` must specify the MIME type
             ...
             * filename (with extension) based on the title. In cases where the extension is missing, Google Drive attempts to determine the extension based on the file's MIME type.
             */
            insert(request: {
                /** V1 error format. */
                "$.xgafv"?:
            ...
            body: File): Request<File>;
            /**
             * Lists the user's files. This method accepts the `q` parameter, which is a search query combining one or more search terms. For more information, see the [Search for
             * files](/drive/api/guides/search-files) guide. *Note:* This method returns *all* files by default, including trashed files. If you don't want trashed files to appear in the list, use
             * the `trashed=false` query parameter to filter trashed files from the results.
             */
            list(request?: {
                /** V1 error format. */
                "$.xgafv"?:
             ...
             * metadata in an API response. All users with access can copy, download, export, and share the file. *Note:* Files moved to the trash still appear by default in results from
             ...
             * *Maximum file size:* 5120GB - *Accepted Media MIME types:*`*‍/*` Note: Specify a valid MIME type, rather than the literal `*‍/*` value. The literal `*‍/*` is only used to indicate
             * that any valid MIME type can be uploaded. For further information on uploading files, refer to [Upload files](/drive/api/guides/manage-uploads).
             */
            update(request: {
                /** V1 error format. */
                "$.xgafv"?:
