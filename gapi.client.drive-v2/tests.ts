...
gapi.load('client', async () => {
    ...
    async function run() {
         ...
         * `alt=media` only works if the file is stored in Drive. To download Google Docs, Sheets, and Slides use [`files.export`](/drive/api/reference/rest/v2/files/export) instead. For further
         * information on downloading files, refer to [Download files](/drive/api/guides/manage-downloads).
         */
         ...
         * types:*`*‍/*` Note: Specify a valid MIME type, rather than the literal `*‍/*` value. The literal `*‍/*` is only used to indicate that any valid MIME type can be uploaded. For further
         * information on uploading files, refer to [Upload files](/drive/api/guides/manage-uploads). Apps creating shortcuts with `files.insert` must specify the MIME type
         * `application/vnd.google-apps.shortcut`. Apps should specify a file extension in the `title` property when inserting files with the API. For example, an operation to insert a JPEG file
         * should specify something like `"title": "cat.jpg"` in the metadata. Subsequent `GET` requests include the read-only `fileExtension` property populated with the extension originally
         ...
         * extension) based on the title. In cases where the extension is missing, Google Drive attempts to determine the extension based on the file's MIME type.
         */
         ...
         * files](/drive/api/guides/search-files) guide. *Note:* This method returns *all* files by default, including trashed files. If you don't want trashed files to appear in the list, use the
         * `trashed=false` query parameter to filter trashed files from the results.
         */
         ...
         * *Maximum file size:* 5120GB - *Accepted Media MIME types:*`*‍/*` Note: Specify a valid MIME type, rather than the literal `*‍/*` value. The literal `*‍/*` is only used to indicate that
         * any valid MIME type can be uploaded. For further information on uploading files, refer to [Upload files](/drive/api/guides/manage-uploads).
         */
