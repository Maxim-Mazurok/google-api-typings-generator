    ...
    namespace chromeuxreport {
        ...
        interface HistoryKey {
            ...
            origin?:
                string;
            /**
             * Url specifies a specific url that this record is for. This url should be normalized, following the normalization actions taken in the request to increase the chances of successful
             * lookup. Note: When specifying a "url" only data for that specific url will be aggregated.
             */
            url?:
                string;
        }
        interface HistoryRecord {
            /**
             * The collection periods indicate when each of the data points reflected in the time series data in metrics was collected. Note that all the time series share the same collection
             * periods, and it is enforced in the CrUX pipeline that every time series has the same number of data points.
             */
            collectionPeriods?:
                CollectionPeriod[];
            ...
            key?:
                HistoryKey;
            /**
             * Metrics is the map of user experience time series data available for the record defined in the key field. Metrics are keyed on the metric name. Allowed key values:
             * ["first_contentful_paint", "first_input_delay", "largest_contentful_paint", "cumulative_layout_shift", "experimental_time_to_first_byte", "experimental_interaction_to_next_paint"]
             */
            metrics?:
                { [P in string]: MetricTimeseries };
        }
        interface Key {
            /**
        ...
        interface Record {
             ...
             * "first_input_delay", "largest_contentful_paint", "cumulative_layout_shift", "experimental_time_to_first_byte", "experimental_interaction_to_next_paint"]
             */
            metrics?:
                { [P in string]: Metric };
        }
        interface TimeseriesBin {
            /**
             * The proportion of users that experienced this bin's value for the given metric in a given collection period; the index for each of these entries corresponds to an entry in the
             * CollectionPeriods field in the HistoryRecord message, which describes when the density was observed in the field. Thus, the length of this list of densities is equal to the length
             * of the CollectionPeriods field in the HistoryRecord message.
             */
            densities?:
                number[];
            /** End is the end of the data bin. If end is not populated, then the bin has no end and is valid from start to +inf. */
        ...
        interface TimeseriesPercentiles {
            /**
             * 75% of users experienced the given metric at or below this value. The length of this list of densities is equal to the length of the CollectionPeriods field in the HistoryRecord
             * message, which describes when the density was observed in the field.
             */
            p75s?:
                any[];
        }
        ...
        interface RecordsResource {
            /**
             * Queries the Chrome User Experience Report for a timeseries `history record` for a given site. Returns a `history record` that contains one or more `metric timeseries` corresponding
            ...
            body: QueryHistoryRequest): Request<QueryHistoryResponse>;
            /**
