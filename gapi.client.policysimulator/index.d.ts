...
declare namespace gapi.client {
    ...
    namespace policysimulator {
        ...
        interface GoogleCloudPolicysimulatorV1beta1BindingExplanation {
             ...
             * `group:product-eng@example.com` The member in the replayed access tuple is `user:bob@example.com`. This user is a member of the group `group:product-eng@example.com`. For the first
             * member in the binding, the key is `user:alice@example.com`, and the `membership` field in the value is set to `MEMBERSHIP_NOT_INCLUDED`. For the second member in the binding, the
             * key is `group:product-eng@example.com`, and the `membership` field in the value is set to `MEMBERSHIP_INCLUDED`.
             */
        ...
        interface GoogleCloudPolicysimulatorV1beta1ExplainedPolicy {
            ...
            access?: string;
            /**
             * Details about how each binding in the policy affects the member's ability, or inability, to use the permission for the resource. If the user who created the Replay does not have
