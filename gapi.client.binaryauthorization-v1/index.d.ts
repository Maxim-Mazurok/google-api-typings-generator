...
declare namespace gapi.client {
    ...
    namespace binaryauthorization {
        ...
        interface AttestationAuthenticator {
            /**
             * Optional. A user-provided name for this AttestationAuthenticator. This field has no effect on the policy evaluation behavior except to improve readability of messages in evaluation
             * results.
             */
            displayName?:
                string;
        ...
        interface Check {
            /**
             * Optional. A special-case check that always denies. Note that this still only applies when the scope of the CheckSet applies and the image isn't exempted by an image allowlist. This
             * check is primarily useful for testing, or to set the default behavior for all unmatched scopes to "deny".
             */
            alwaysDeny?:
                boolean;
            ...
            simpleSigningAttestationCheck?:
                SimpleSigningAttestationCheck;
            /**
             * Optional. Require that an image was built by a trusted builder (such as Google Cloud Build or GitHub), meets requirements for Supply chain Levels for Software Artifacts (SLSA), and
             * was built from a trusted source code repostitory.
             */
            slsaCheck?:
                SlsaCheck;
        ...
        interface CheckSet {
            ...
            imageAllowlist?:
                ImageAllowlist;
            /**
             * Optional. The scope to which this CheckSet applies. If unset or an empty string (the default), applies to all namespaces and service accounts. See the Scope message documentation
        ...
        interface GkePolicy {
            /**
             * Optional. The CheckSets to apply, scoped by namespace or namespace and service account. Exactly one CheckSet will be evaluated for a given Pod (unless the list is empty, in which
             * case the behavior is "always allow"). If multiple CheckSets have scopes that match the namespace and service account of the Pod being evaluated, only the CheckSet with the MOST
             * SPECIFIC scope will match. CheckSets must be listed in order of decreasing specificity, i.e. if a scope matches a given service account (which must include the namespace), it must
             * come before a CheckSet with a scope matching just that namespace. This property is enforced by server-side validation. The purpose of this restriction is to ensure that if more than
             * one CheckSet matches a given Pod, the CheckSet that will be evaluated will always be the first in the list to match (because if any other matches, it must be less specific). If
             * `check_sets` is empty, the default behavior is to allow all images. If `check_sets` is non-empty, the last `check_sets` entry must always be a CheckSet with no scope set, i.e. a
             * catchall to handle any situation not caught by the preceding CheckSets.
             */
            checkSets?:
                CheckSet[];
        ...
        interface PlatformPolicy {
            ...
            gkePolicy?:
                GkePolicy;
            /** Output only. The relative resource name of the BinAuthz platform policy, in the form of `projects/*‍/platforms/*‍/policies/*`. */
            name?:
                string;
        ...
        interface TrustedDirectoryCheck {
             ...
             * us.pkg.dev, or gcr.io. Additionally, * can be used in three ways as wildcards: 1. leading * to match varying prefixes in registry subdomain (useful for location prefixes); 2.
             * trailing * after registry/ to match varying endings; 3. trailing ** after registry/ to match "/" as well. For example: -- gcr.io/my-project/my-repo is valid to match a single
             * directory -- *-docker.pkg.dev/my-project/my-repo or *.gcr.io/my-project are valid to match varying prefixes -- gcr.io/my-project/* will match all direct directories in my-project --
             * gcr.io/my-project/** would match all directories in my-project -- gcr.i* is not allowed since the registry is not completely specified -- sub*domain.gcr.io/nginx is not valid
             * because only leading * or trailing * are allowed. -- *pkg.dev/my-project/my-repo is not valid because leading * can only match subdomain -- **-docker.pkg.dev is not valid because
        ...
        interface VerificationRule {
             ...
             * can be used in only two ways: 1. trailing * after hosturi/ to match varying endings; 2. trailing ** after hosturi/ to match "/" as well. * and ** can only be used as wildcards and
             * can only occur at the end of the pattern after a /. (So it's not possible to match a URL that contains literal *.) For example: - "github.com/my-project/my-repo" is valid to match a
             * single repo - "github.com/my-project/*" will match all direct repos in my-project - "github.com/**" matches all repos in GitHub
             */
        ...
        interface VulnerabilityCheck {
            /**
             * Optional. A list of specific CVEs to ignore even if the vulnerability level violates maximumUnfixableSeverity or maximumFixableSeverity. CVEs are listed in the format of Container
             * Analysis note id. For example: - CVE-2021-20305 - CVE-2020-10543 The CVEs are applicable regardless of note provider project, e.g., an entry of `CVE-2021-20305` will allow
             ...
             * projects/my-gcp-project. An attempt will be made for each project to fetch vulnerabilities, and all valid vulnerabilities will be used to check against the vulnerability policy. If
             * no valid scan is found in all projects configured here, an error will be returned for the check.
             */
