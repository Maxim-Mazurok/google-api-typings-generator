...
declare namespace gapi.client {
    ...
    function load(urlOrObject: "https://safebrowsing.googleapis.com/$discovery/rest?version=v5"): Promise<void>;
    ...
    namespace safebrowsing {
        ...
        interface GoogleSecuritySafebrowsingV5SearchHashesResponse {
            /**
             * The client-side cache duration. The client shall add this duration to the current time to determine the expiration time. The expiration time then applies to every hash prefix
             * queried by the client in the request, regardless of how many full hashes are returned in the response. Even if the server returns no full hashes for a particular hash prefix, this
             * fact should also be cached by the client. Important: the client must not assume that the server will return the same cache duration for all responses. The server may choose
