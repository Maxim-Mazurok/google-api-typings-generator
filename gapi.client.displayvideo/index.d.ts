...
declare namespace gapi.client {
    ...
    namespace displayvideo {
        ...
        interface Advertiser {
            ...
            partnerId?: string;
            /**
             * Whether integration with Mediaocean (Prisma) is enabled. By enabling this: On behalf of my company, I authorize Mediaocean (Prisma) to send budget segment plans to Google, and I
             * authorize Google to send corresponding reporting and invoices from DV360 to Mediaocean for the purposes of budget planning, billing, and reconciliation for this advertiser.
             */
        ...
        interface CampaignBudget {
            ...
            displayName?: string;
            /**
             * Immutable. Must be unique under the campaign. If set, all impressions served against this budget will include this ID on the invoice if the customer has opted into
        ...
        interface Invoice {
            ...
            /**
             * Output only. The originally issued invoice that is being adjusted by this invoice, if applicable. If there is a corrected invoice, the replaced_invoice_ids field will be empty. May
            ...
            name?: string;
            /**
             * Output only. The total amount of costs or adjustments not tied to a particular budget, in micros of the invoice's currency. For example if currency_code is `USD`, then 1000000
             * micros is one US dollar.
             */
            ...
            /**
             * Output only. The URL to download a PDF copy of the invoice. Note that this URL is user specific and requires a valid OAuth 2.0 access token to access. The access token must be
             * provided in an `Authorization: Bearer` HTTP header and be authorized for one of the following scopes: * `https://www.googleapis.com/auth/display-video-mediaplanning` *
             * `https://www.googleapis.com/auth/display-video` The URL will only be usable for 7 days from when the api is called.
             */
            pdfUrl?: string;
            /** Output only. Purchase order number associated with the invoice. */
            purchaseOrderNumber?: string;
            /**
             * Output only. The originally issued invoice(s) that is being cancelled by this invoice, if applicable. If there are any replaced invoices, the corrected_invoice_id field will be
             * empty. May appear on invoice PDF as `Replaced invoice numbers`. Note: There may be multiple replaced invoices due to consolidation of multiple invoices into a single invoice.
             */
        ...
        interface ListInvoicesResponse {
            ...
            invoices?: Invoice[];
            /**
             * A token to retrieve the next page of results. Pass this value in the [ListInvoicesRequest.page_token] field in the subsequent call to `ListInvoices` method to retrieve the next page
             * of results.
             */
            nextPageToken?: string;
        }
        ...
        interface InvoicesResource {
            ...
            list(request?: {
                ...
                pageSize?: number;
                /**
                 * A token identifying a page of results the server should return. Typically, this is the value of [ListInvoicesResponse.next_page_token] returned from the previous call to
