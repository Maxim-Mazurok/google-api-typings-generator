...
declare namespace gapi.client {
    ...
    namespace bigquery {
        ...
        interface QueryRequest {
             ...
             * are nullipotent by definition. 3. When a duplicate mutating query request is detected (i.e. having the same request_id as an earlier query), it
             * returns: a. the results of the mutation if it completes successfully within the timeout. b. the running operation if it is still in progress at the end
             * of the timeout. 4. Its lifetime is limited to 15 minutes. In other words, if two requests are sent with the same request_id, but more than 15 minutes
        ...
        interface Table {
             ...
             * query. [TrustedTester] MATERIALIZED_VIEW: SQL query whose result is persisted. EXTERNAL: A table that references data stored in an external storage
