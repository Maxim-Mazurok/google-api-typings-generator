...
declare namespace gapi.client {
    ...
    namespace dataproc {
        ...
        interface Cluster {
            ...
            /**
             * Optional. The virtual cluster config, used when creating a Dataproc cluster that does not directly control the underlying compute resources, for example, when creating a
             * Dataproc-on-GKE cluster (https://cloud.google.com/dataproc/docs/concepts/jobs/dataproc-gke#create-a-dataproc-on-gke-cluster). Note that Dataproc may set default values, and values
             * may change when clusters are updated. Exactly one of config or virtualClusterConfig must be specified.
             */
        ...
        interface ClusterConfig {
            ...
            /**
             * Optional. Deprecated. Use VirtualClusterConfig based clusters instead. BETA. The Kubernetes Engine config for Dataproc clusters deployed to Kubernetes. Setting this is considered
        ...
        interface GkeClusterConfig {
            ...
            /**
             * Optional. GKE NodePools where workloads will be scheduled. At least one node pool must be assigned the 'default' role. Each role can be given to only a single NodePoolTarget. All
             * NodePools must have the same location settings. If a nodePoolTarget is not specified, Dataproc constructs a default nodePoolTarget.
             */
        ...
        interface GkeNodePoolConfig {
            ...
            /**
             * Optional. The list of Compute Engine zones (https://cloud.google.com/compute/docs/zones#available) where NodePool's nodes will be located.Note: Currently, only one zone may be
             * specified.If a location is not specified during NodePool creation, Dataproc will choose a location.
             */
            locations?: string[];
        }
        interface GkeNodePoolTarget {
            ...
            nodePool?: string;
            /**
             * Input only. The configuration for the GKE NodePool.If specified, Dataproc attempts to create a NodePool with the specified shape. If one with the same name already exists, it is
             * verified against all specified fields. If a field differs, the virtual cluster creation will fail.If omitted, any NodePool with the specified name is used. If a NodePool with the
             * specified name does not exist, Dataproc create a NodePool with default values.This is an input only field. It will not be returned by the API.
             */
        ...
        interface VirtualClusterConfig {
            ...
            /**
             * Optional. A Storage bucket used to stage job dependencies, config files, and job driver console output. If you do not specify a staging bucket, Cloud Dataproc will determine a Cloud
             * Storage location (US, ASIA, or EU) for your cluster's staging bucket according to the Compute Engine zone where your cluster is deployed, and then create and manage this
             * project-level, per-location bucket (see Dataproc staging and temp buckets (https://cloud.google.com/dataproc/docs/concepts/configuring-clusters/staging-bucket)). This field requires
             * a Cloud Storage bucket name, not a gs://... URI to a Cloud Storage bucket.
             */
