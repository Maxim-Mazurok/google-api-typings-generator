    ...
    namespace cloudbuild {
        ...
        interface BitbucketServerConfig {
            ...
            peeredNetwork?:
                string;
            /**
             * Immutable. IP range within the peered network. This is specified in CIDR notation with a slash and the subnet prefix size. You can optionally specify an IP address before the subnet
             * prefix value. e.g. `192.168.0.0/29` would specify an IP range starting at 192.168.0.0 with a 29 bit prefix size. `/16` would specify a prefix size of 16 bits, with an automatically
             * determined IP within the peered VPC. If unspecified, a value of `/24` will be used. The field only has an effect if peered_network is set.
             */
        ...
        interface GitFileSource {
            ...
            path?:
                string;
            /**
             * The fully qualified resource name of the Repos API repository. Either URI or repository can be specified. If unspecified, the repo from which the trigger invocation originated is
        ...
        interface GitHubEventsConfig {
            /**
             * Optional. The resource name of the github enterprise config that should be applied to this installation. For example:
        ...
        interface NetworkConfig {
            ...
            peeredNetwork?:
                string;
            /**
             * Immutable. Subnet IP range within the peered network. This is specified in CIDR notation with a slash and the subnet prefix size. You can optionally specify an IP address before the
             * subnet prefix value. e.g. `192.168.0.0/29` would specify an IP range starting at 192.168.0.0 with a prefix size of 29 bits. `/16` would specify a prefix size of 16 bits, with an
             * automatically determined IP within the peered VPC. If unspecified, a value of `/24` will be used.
             */
            peeredNetworkIpRange?:
                string;
        }
        ...
        interface RepositoryEventConfig {
            /** Filter to match changes in pull requests. */
            pullRequest?:
                PullRequestFilter;
            /** Filter to match changes in refs like branches, tags. */
        ...
        interface Results {
             ...
             * can produce this output by writing to `$BUILDER_OUTPUT/output`. Only the first 50KB of data is stored.
             */
            buildStepOutputs?:
                string[];
        ...
        interface BuildsResource {
             ...
             * original build pulled source from Cloud Storage without specifying the generation of the object, the new build will use the current object, which may be different from the original
             * build source. * If the original build pulled source from Cloud Storage and specified the generation of the object, the new build will attempt to use the same object, which may or
             * may not be available depending on the bucket's lifecycle management settings.
             */
            retry(request: {
                /** V1 error format. */
                "$.xgafv"?:
        ...
        interface TriggersResource {
             ...
             * v1/projects/{projectId}/locations/{region}/triggers/{triggerId}:run). The POST request that does not include the location endpoint in the path can only be used when running global
             * triggers.
             */
            run(request: {
                /** V1 error format. */
        ...
        interface LocationsResource {
            bitbucketServerConfigs:
                BitbucketServerConfigsResource;
            builds:
                BuildsResource;
            githubEnterpriseConfigs:
                GithubEnterpriseConfigsResource;
            gitLabConfigs:
                GitLabConfigsResource;
            operations:
                OperationsResource;
            triggers:
                TriggersResource;
