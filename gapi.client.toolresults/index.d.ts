...
declare namespace gapi.client {
    ...
    namespace toolresults {
        ...
        interface MergedResult {
             ...
             *
             * Combining: When the test cases from a suite are run in different steps (sharding), the results are added back together in one overview. (e.g., if
             * shard1 has 2 failures and shard2 has 1 failure than the overview failure_count = 3).
             *
             * Rollup: When test cases from the same suite are run multiple times (flaky), the results are combined (e.g., if testcase1.run1 fails, testcase1.run2
             * passes, and both testcase2.run1 and testcase2.run2 fail then the overview flaky_count = 1 and failure_count = 1).
             */
        ...
        interface Step {
             ...
             * also identifies the groups 'primary step' which indexes all the group members.
             *
        ...
        interface EnvironmentsResource {
             ...
             *
             * - PERMISSION_DENIED - if the user is not authorized to read project - INVALID_ARGUMENT - if the request is malformed - NOT_FOUND - if the Environment
             * does not exist
             */
            get(request?: {
                /** Data format for the response. */
        ...
        interface StepsResource {
            ...
            accessibilityClusters(request?: {
                 ...
                 * UPPERCASE. See http://www.unicode.org/reports/tr35/#Unicode_locale_identifier for details.
                 *
                 * Required.
                 */
                locale?: string;
