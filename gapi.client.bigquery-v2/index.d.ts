...
declare namespace gapi.client {
    ...
    function load(urlOrObject: "https://bigquery.googleapis.com/$discovery/rest?version=v2"): Promise<void>;
    ...
    namespace bigquery {
            ...
            access?:
                Array<{
                    ...
                    role?:
                        string;
                    /**
                     * [Pick one] A routine from a different dataset to grant access to. Queries executed against that routine will have read access to views/tables/routines in this dataset. Only
                     * UDF is supported for now. The role field is not required when this field is set. If that routine is updated by any user, access to the routine needs to be granted again via
                     * an update operation.
                     */
                    routine?:
                        RoutineReference;
                    ...
                    userByEmail?:
                        string;
                    /**
                     * [Pick one] A view from a different dataset to grant access to. Queries executed against that view will have read access to tables in this dataset. The role field is not
                     * required when this field is set. If that view is updated by any user, access to the view needs to be granted again via an update operation.
                     */
                    view?:
                        TableReference;
                }>;
        ...
        interface HivePartitioningOptions {
            /**
             * [Output-only] For permanent external tables, this field is populated with the hive partition keys in the order they were inferred. The types of the partition keys can be deduced by
             * checking the table schema (which will include the partition keys). Not every API will populate this field in the output. For example, Tables.Get will populate it, but Tables.List
        ...
        interface JobConfigurationQuery {
             ...
             */
            tableDefinitions?:
                { [P in string]: ExternalDataConfiguration };
            /** Time-based partitioning specification for the destination table. Only one of timePartitioning and rangePartitioning should be specified. */
            timePartitioning?:
                TimePartitioning;
            /**
        ...
        interface QueryParameterValue {
            ...
            structValues?:
                { [P in string]: QueryParameterValue };
        ...
        interface JobsResource {
            ...
            delete(request?: {
                ...
                fields?:
                    string;
                /**
                 * Required. Job ID of the job for which metadata is to be deleted. If this is a parent job which has child jobs, the metadata from all child jobs will be deleted as well. Direct
