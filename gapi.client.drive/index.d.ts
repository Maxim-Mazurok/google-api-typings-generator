    ...
    namespace drive {
        ...
        interface ChangesResource {
            ...
            list(request?: {
                ...
                alt?: string;
                /**
                 * The shared drive from which changes will be returned. If specified the change IDs will be reflective of the shared drive; use the combined drive ID and
        ...
        interface FilesResource {
            ...
            copy(request: {
                ...
                alt?: string;
                /**
                 * Set to true to opt in to API behavior that aims for all items to have exactly one parent. This parameter will only take effect if the item is not in a
            ...
            update(request: {
                ...
                /**
                 * Set to true to opt in to API behavior that aims for all items to have exactly one parent. This parameter will only take effect if the item is not in a
                 * shared drive. If the item's owner makes a request to add a single parent, the item will be removed from all current folders and placed in the requested
                 * folder. Other requests that increase the number of parents will fail, except when the canAddMyDriveParent file capability is true and a single parent
                 * is being added.
                 */
                enforceSingleParent?: boolean;
                /** Selector specifying which fields to include in a partial response. */
        ...
        interface PermissionsResource {
            ...
            create(request: {
                ...
                key?: string;
                /**
                 * This parameter will only take effect if the item is not in a shared drive and the request is attempting to transfer the ownership of the item. When set
                 * to true, the item will be moved to the new owner's My Drive root folder and all prior parents removed. If set to false, when enforceSingleParent=true,
                 * parents are not changed. If set to false, when enforceSingleParent=false, existing parents are not changed; however, the file will be added to the new
                 * owner's My Drive root folder, unless it is already in the new owner's My Drive.
                 */
