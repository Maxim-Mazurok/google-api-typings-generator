...
// Project: https://developers.google.com/apis-explorer/#search/dataflow
...
declare namespace gapi.client {
    ...
    namespace datapipelines {
        ...
        interface GoogleCloudDatapipelinesV1Pipeline {
             ...
             * `LOCATION_ID` is the canonical ID for the pipeline's location. The list of available locations can be obtained by calling ListLocations. Note that the Data Pipelines service is not
             * available in all regions. It depends on Cloud Scheduler, an App Engine application, so it's only available in [App Engine regions](https://cloud.google.com/about/locations#region).
        ...
        interface PipelinesResource {
            ...
            /**
             * Updates a pipeline. If successful, the updated [Pipeline] is returned. Returns `NOT_FOUND` if the pipeline doesn't exist. If UpdatePipeline does not return successfully, you can
             * retry the UpdatePipeline request until you receive a successful response.
             */
            patch(request: {
                /** V1 error format. */
                "$.xgafv"?: string;
                 ...
                 * The pipeline name. For example: `projects/PROJECT_ID/locations/LOCATION_ID/pipelines/PIPELINE_ID`. * `PROJECT_ID` can contain letters ([A-Za-z]), numbers ([0-9]), hyphens (-),
                 * colons (:), and periods (.). For more information, see [Identifying projects](https://cloud.google.com/resource-manager/docs/creating-managing-projects#identifying_projects) *
                 * `LOCATION_ID` is the canonical ID for the pipeline's location. The list of available locations can be obtained by calling ListLocations. Note that the Data Pipelines service is
                 * not available in all regions. It depends on Cloud Scheduler, an App Engine application, so it's only available in [App Engine
             ...
             * external system. Returns a "NOT_FOUND" error if the pipeline doesn't exist. Returns a "FOBIDDEN" error if the user doesn't have permission to access the pipeline or run jobs for the
             * pipeline.
             */
            run(request: {
                /** V1 error format. */
        ...
        interface LocationsResource {
            ...
            listPipelines(request?: {
                 ...
                 * eligible for filtering are: + `type`: The type of the pipeline (streaming or batch). Allowed values are `ALL`, `BATCH`, and `STREAMING`. + `executor_type`: The type of pipeline
                 * execution layer. This is always Dataflow for now, but more executors may be added later. Allowed values are `ALL` and `DATAFLOW`. + `status`: The activity status of the
