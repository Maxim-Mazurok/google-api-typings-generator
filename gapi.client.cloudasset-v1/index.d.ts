    ...
    namespace cloudasset {
        ...
        interface V1Resource {
            ...
            searchAllResources(request?: {
                 ...
                 * displayName * description * location * createTime * updateTime * state * parentFullResourceName * parentAssetType All the other fields such as repeated fields (e.g.,
                 * `networkTags`, `kmsKeys`), map fields (e.g., `labels`) and struct fields (e.g., `additionalAttributes`) are not supported.
                 */
                orderBy?:
                    string;
                /**
                 ...
                 * `project` * `folders` * `organization` * `displayName` * `description` * `location` * `labels` * `networkTags` * `kmsKeys` * `createTime` * `updateTime` * `state` *
                 * `additionalAttributes` * `parentFullResourceName` * `parentAssetType` Some fields of large size, such as `versionedResources` and `attachedResources`, are not returned by
                 * default, but you can specify them in the `read_mask` parameter if you want to include them. If `"*"` is specified, all [available
