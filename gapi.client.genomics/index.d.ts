...
// Generated from: https://genomics.googleapis.com/$discovery/rest?version=v2alpha1
// Revision: 20211130
...
declare namespace gapi.client {
    ...
    function load(name: "genomics", version: "v2alpha1"): PromiseLike<void>;
    function load(name: "genomics", version: "v2alpha1", callback: () => any): void;
    ...
    namespace genomics {
        ...
        interface Action {
            ...
            credentials?: Secret;
            /**
             * The encrypted environment to pass into the container. This environment is merged with values specified in the google.genomics.v2alpha1.Pipeline message, overwriting any duplicate
             * values. The secret must decrypt to a JSON-encoded dictionary where key-value pairs serve as environment variable names and their values. The decoded environment variables can
            ...
            entrypoint?: string;
            /**
             * The environment to pass into the container. This environment is merged with values specified in the google.genomics.v2alpha1.Pipeline message, overwriting any duplicate values. In
             * addition to the values passed here, a few other values are automatically injected into the environment. These cannot be hidden or overwritten. `GOOGLE_PIPELINE_FAILED` will be set
             * to "1" if the pipeline failed because an action has exited with a non-zero status (and did not have the `IGNORE_EXIT_STATUS` flag set). This can be used to determine if additional
             * debug or logging actions should execute. `GOOGLE_LAST_EXIT_STATUS` will be set to the exit status of the last non-background action that executed. This can be used by workflow
        ...
        interface WorkerStatus {
            ...
            attachedDisks?: { [P in string]: DiskStatus };
        ...
        interface OperationsResource {
            ...
            list(request?: {
                 ...
                 * this value is false. Once the pipeline finishes, the value is true. Examples: * `projectId = my-project AND createTime >= 1432140000` * `projectId = my-project AND createTime >=
                 * 1432140000 AND createTime <= 1432150000 AND status = RUNNING` * `projectId = my-project AND labels.color = *` * `projectId = my-project AND labels.color = red`
                 */
                filter?: string;
                /** API key. Your API key identifies your project and provides you with API access, quota, and reports. Required unless you provide an OAuth 2.0 token. */
                key?: string;
        ...
        interface WorkersResource {
            ...
            body: CheckInRequest): Request<CheckInResponse>;
        }
        ...
        const pipelines: PipelinesResource;

        const projects: ProjectsResource;

        const workers: WorkersResource;
    }
}
