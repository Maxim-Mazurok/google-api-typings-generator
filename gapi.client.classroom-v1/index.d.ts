    ...
    namespace classroom {
        ...
        interface Course {
            ...
            alternateLink?:
                string;
            /**
             * The Calendar ID for a calendar that all course members can see, to which Classroom adds events for course work and announcements in the course. The Calendar for a course is created
             * asynchronously when the `course is set to the `CourseState.ACTIVE` for the first time (i.e. at creation time or when it is updated to `ACTIVE` through the UI or the API). The
             * Calendar ID will not be populated until this processing is completed. Read-only.
             */
            calendarId?:
                string;
        ...
        interface UserProfile {
            ...
            photoUrl?:
                string;
            /**
             * Represents whether a Google Workspace for Education user's domain administrator has explicitly verified them as being a teacher. This field is always false if the user is not a
             * member of a Google Workspace for Education domain. Read-only
             */
            verifiedTeacher?:
                boolean;
        }
        ...
        interface CoursesResource {
             ...
             * the following request errors: * UserCannotOwnCourse * UserGroupsMembershipLimitReached * `ALREADY_EXISTS` if an alias was specified in the `id` and already exists.
             */
            create(request: {
                /** V1 error format. */
                "$.xgafv"?:
             ...
             * supplied. * `FAILED_PRECONDITION` for the following request errors: * CourseNotModifiable * InactiveCourseOwner * IneligibleOwner
             */
            patch(request: {
                /** V1 error format. */
                "$.xgafv"?:
        ...
        interface InvitationsResource {
             ...
             * exist. * `FAILED_PRECONDITION`: * if the requested user's account is disabled. * if the user already has this role or a role with greater permissions. * for the following request
             * errors: * IneligibleOwner * `ALREADY_EXISTS` if an invitation for the specified user and course already exists.
             */
            create(request: {
                /** V1 error format. */
                "$.xgafv"?:
