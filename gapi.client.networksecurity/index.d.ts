/* Type definitions for non-npm package Network Security API v1beta1 1.0 */
...
declare namespace gapi.client {
    ...
    function load(name: "networksecurity", version: "v1beta1"): PromiseLike<void>;
    ...
    namespace networksecurity {
        ...
        interface Destination {
            /**
             * Required. List of host names to match. Matched against HOST header in http requests. At least one host should match. Each host can be an exact match, or a prefix match (example
        ...
        interface ServerTlsPolicy {
            /**
             * Optional. Determines if server allows plaintext connections. If set to true, server allows plain text connections. By default, it is set to false. This setting is not exclusive of
             * other encryption modes. For example, if allow_open and mtls_policy are set, server allows both plain text and mTLS connections. See documentation of other encryption modes to
            ...
            labels?: { [P in string]: string };
            /**
             * Optional. Defines a mechanism to provision peer validation certificates for peer to peer authentication (Mutual TLS - mTLS). If not specified, client certificate will not be
             * requested. The connection is treated as TLS and not mTLS. If allow_open and mtls_policy are set, server allows both plain text and mTLS connections.
             */
            ...
            name?: string;
            /**
             * Optional. Defines a mechanism to provision server identity (public and private keys). Cannot be combined with allow_open as a permissive mode that allows both plain text and TLS is
             * not supported.
             */
