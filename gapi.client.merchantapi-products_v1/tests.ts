/* This is stub file for gapi.client.merchantapi-products_v1 definition tests */
// IMPORTANT
// This file was generated by https://github.com/Maxim-Mazurok/google-api-typings-generator. Please do not edit it manually.
// In case of any problems please post issue to https://github.com/Maxim-Mazurok/google-api-typings-generator

// Revision: 20260903

gapi.load('client', async () => {
  /** now we can use gapi.client */

  await gapi.client.load(
    'https://merchantapi.googleapis.com/$discovery/rest?version=products_v1',
  );
  /** now we can use gapi.client.merchantapi */

  /** don't forget to authenticate your client before sending any request to resources: */
  /** declare client_id registered in Google Developers Console */
  const client_id = '<<PUT YOUR CLIENT ID HERE>>';
  const scope = [
    /** Manage your product listings and accounts for Google Shopping */
    'https://www.googleapis.com/auth/content',
  ];
  const immediate = false;
  gapi.auth.authorize({client_id, scope, immediate}, authResult => {
    if (authResult && !authResult.error) {
      /** handle successful authorization */
      void run();
    } else {
      /** handle authorization error */
    }
  });

  async function run() {
    /** Deletes a product input from your Merchant Center account. After inserting, updating, or deleting a product input, it may take several minutes before the processed product can be retrieved. */
    await gapi.client.merchantapi.accounts.productInputs.delete({
      dataSource: 'Test string',
      name: 'Test string',
    });
    /** [Uploads a product input to your Merchant Center account](/merchant/api/guides/products/add-manage#add_a_product). You must have a products [data source](/merchant/api/guides/data-sources/api-sources#create-primary-data-source) to be able to insert a product. The unique identifier of the data source is passed as a query parameter in the request URL. If a product input with the same contentLanguage, offerId, and dataSource already exists, then the product input inserted by this method replaces that entry. After inserting, updating, or deleting a product input, it may take several minutes before the processed product can be retrieved. */
    await gapi.client.merchantapi.accounts.productInputs.insert(
      {
        dataSource: 'Test string',
        parent: 'Test string',
      },
      {
        base64EncodedName: 'Test string',
        base64EncodedProduct: 'Test string',
        contentLanguage: 'Test string',
        customAttributes: [
          {
            groupValues: undefined,
            name: 'Test string',
            value: 'Test string',
          },
        ],
        feedLabel: 'Test string',
        legacyLocal: true,
        name: 'Test string',
        offerId: 'Test string',
        product: 'Test string',
        productAttributes: {
          additionalImageLinks: ['Test string'],
          adsGrouping: 'Test string',
          adsLabels: ['Test string'],
          adsRedirect: 'Test string',
          adult: true,
          ageGroup: 'Test string',
          amenityFeature: ['Test string'],
          autoPricingMinPrice: {
            amountMicros: 'Test string',
            currencyCode: 'Test string',
          },
          availability: 'Test string',
          availabilityDate: 'Test string',
          bodyStyle: 'Test string',
          brand: 'Test string',
          canonicalLink: 'Test string',
          carrierShipping: [
            {
              carrierPrice: 'Test string',
              carrierPriceFlatAdjustment: {
                amountMicros: 'Test string',
                currencyCode: 'Test string',
              },
              carrierPricePercentageAdjustment: 42,
              carrierTransitTime: 'Test string',
              country: 'Test string',
              fixedMaxTransitTime: 'Test string',
              fixedMinTransitTime: 'Test string',
              flatPrice: {
                amountMicros: 'Test string',
                currencyCode: 'Test string',
              },
              maxHandlingTime: 'Test string',
              minHandlingTime: 'Test string',
              originPostalCode: 'Test string',
              postalCode: 'Test string',
              region: 'Test string',
            },
          ],
          certifications: [
            {
              certificationAuthority: 'Test string',
              certificationCode: 'Test string',
              certificationDocumentLink: 'Test string',
              certificationLabelLink: 'Test string',
              certificationName: 'Test string',
              certificationValue: 'Test string',
            },
          ],
          certifiedPreOwned: true,
          cloudExportAdditionalProperties: [
            {
              boolValue: true,
              floatValue: [42],
              intValue: ['Test string'],
              maxValue: 42,
              minValue: 42,
              propertyName: 'Test string',
              textValue: ['Test string'],
              unitCode: 'Test string',
            },
          ],
          co2Emissions: {
            unit: 'Test string',
            value: 'Test string',
          },
          color: 'Test string',
          condition: 'Test string',
          costOfGoodsSold: {
            amountMicros: 'Test string',
            currencyCode: 'Test string',
          },
          customLabel0: 'Test string',
          customLabel1: 'Test string',
          customLabel2: 'Test string',
          customLabel3: 'Test string',
          customLabel4: 'Test string',
          dateFirstRegistered: 'Test string',
          description: 'Test string',
          disclosureDate: 'Test string',
          displayAddress: {
            city: 'Test string',
            postalCode: 'Test string',
            region: 'Test string',
            streetName: 'Test string',
            streetNumber: 'Test string',
          },
          displayAdsId: 'Test string',
          displayAdsLink: 'Test string',
          displayAdsSimilarIds: ['Test string'],
          displayAdsTitle: 'Test string',
          displayAdsValue: 42,
          documentLinks: ['Test string'],
          electricRange: {
            unit: 'Test string',
            value: 'Test string',
          },
          emissionsStandard: 'Test string',
          energyConsumption: {
            unit: 'Test string',
            value: 42,
          },
          energyEfficiencyClass: 'Test string',
          engine: 'Test string',
          excludedDestinations: ['Test string'],
          expirationDate: 'Test string',
          externalSellerId: 'Test string',
          freeShippingThreshold: [
            {
              country: 'Test string',
              priceThreshold: {
                amountMicros: 'Test string',
                currencyCode: 'Test string',
              },
            },
          ],
          fuelConsumption: {
            unit: 'Test string',
            value: 42,
          },
          fuelConsumptionDischargedBattery: {
            unit: 'Test string',
            value: 42,
          },
          gender: 'Test string',
          googleProductCategory: 'Test string',
          gtins: ['Test string'],
          handlingCutoffTimes: [
            {
              country: 'Test string',
              cutoffTime: 'Test string',
              cutoffTimezone: 'Test string',
              disableDeliveryAfterCutoff: true,
            },
          ],
          identifierExists: true,
          imageLink: 'Test string',
          includedDestinations: ['Test string'],
          installment: {
            amount: {
              amountMicros: 'Test string',
              currencyCode: 'Test string',
            },
            annualPercentageRate: 42,
            creditType: 'Test string',
            downpayment: {
              amountMicros: 'Test string',
              currencyCode: 'Test string',
            },
            months: 'Test string',
            totalAmount: {
              amountMicros: 'Test string',
              currencyCode: 'Test string',
            },
          },
          isBundle: true,
          itemGroupId: 'Test string',
          itemGroupTitle: 'Test string',
          latitude: 42,
          leaseTerm: {
            durationUnit: 'Test string',
            durationValue: 'Test string',
            type: 'Test string',
          },
          lifestyleImageLinks: ['Test string'],
          link: 'Test string',
          linkTemplate: 'Test string',
          longitude: 42,
          loyaltyPoints: {
            name: 'Test string',
            pointsValue: 'Test string',
            ratio: 42,
          },
          loyaltyPrograms: [
            {
              cashbackForFutureUse: {
                amountMicros: 'Test string',
                currencyCode: 'Test string',
              },
              loyaltyPoints: 'Test string',
              memberPriceEffectiveDate: {
                endTime: 'Test string',
                startTime: 'Test string',
              },
              price: {
                amountMicros: 'Test string',
                currencyCode: 'Test string',
              },
              programLabel: 'Test string',
              shippingLabel: 'Test string',
              tierLabel: 'Test string',
            },
          ],
          material: 'Test string',
          maxEnergyEfficiencyClass: 'Test string',
          maxHandlingTime: 'Test string',
          maximumRetailPrice: {
            amountMicros: 'Test string',
            currencyCode: 'Test string',
          },
          mileage: {
            unit: 'Test string',
            value: 'Test string',
          },
          minEnergyEfficiencyClass: 'Test string',
          minHandlingTime: 'Test string',
          minimumOrderValues: [
            {
              country: 'Test string',
              price: {
                amountMicros: 'Test string',
                currencyCode: 'Test string',
              },
              service: 'Test string',
              surface: 'Test string',
            },
          ],
          mobileLink: 'Test string',
          mobileLinkTemplate: 'Test string',
          model: 'Test string',
          mpn: 'Test string',
          multipack: 'Test string',
          neighborhood: 'Test string',
          numberOfBathrooms: 42,
          numberOfBedrooms: 42,
          numberOfUnits: 'Test string',
          pattern: 'Test string',
          pause: 'Test string',
          petPolicy: {
            petsAllowed: true,
            petTypes: ['Test string'],
          },
          pickupCost: {
            flatRate: {
              amountMicros: 'Test string',
              currencyCode: 'Test string',
            },
            freeThreshold: {
              amountMicros: 'Test string',
              currencyCode: 'Test string',
            },
          },
          pickupMethod: 'Test string',
          pickupSla: 'Test string',
          popularityRank: 42,
          price: {
            amountMicros: 'Test string',
            currencyCode: 'Test string',
          },
          productDetails: [
            {
              attributeName: 'Test string',
              attributeValue: 'Test string',
              sectionName: 'Test string',
            },
          ],
          productFee: [
            {
              amount: {
                amountMicros: 'Test string',
                currencyCode: 'Test string',
              },
              type: 'Test string',
            },
          ],
          productHeight: {
            unit: 'Test string',
            value: 42,
          },
          productHighlights: ['Test string'],
          productLength: {
            unit: 'Test string',
            value: 42,
          },
          productTypes: ['Test string'],
          productWeight: {
            unit: 'Test string',
            value: 42,
          },
          productWidth: {
            unit: 'Test string',
            value: 42,
          },
          promotionIds: ['Test string'],
          propertyName: 'Test string',
          propertyType: 'Test string',
          questionsAndAnswers: [
            {
              answer: 'Test string',
              question: 'Test string',
            },
          ],
          relatedProducts: [
            {
              id: 'Test string',
              idType: 'Test string',
              relationshipType: 'Test string',
            },
          ],
          returnPolicyLabel: 'Test string',
          returns: [
            {
              countries: ['Test string'],
              itemConditions: ['Test string'],
              methods: ['Test string'],
              outcomes: ['Test string'],
              policyUrl: 'Test string',
              restockingFee: {
                amountMicros: 'Test string',
                currencyCode: 'Test string',
              },
              restockingPercentageFee: 42,
              shippingFee: {
                amountMicros: 'Test string',
                currencyCode: 'Test string',
              },
              shippingFeeType: 'Test string',
              windowDays: 'Test string',
              windowType: 'Test string',
            },
          ],
          salePrice: {
            amountMicros: 'Test string',
            currencyCode: 'Test string',
          },
          salePriceEffectiveDate: {
            endTime: 'Test string',
            startTime: 'Test string',
          },
          sellOnGoogleQuantity: 'Test string',
          shipping: [
            {
              country: 'Test string',
              handlingCutoffTime: 'Test string',
              handlingCutoffTimezone: 'Test string',
              locationGroupName: 'Test string',
              locationId: 'Test string',
              loyaltyProgramLabel: 'Test string',
              loyaltyTierLabel: 'Test string',
              maxHandlingTime: 'Test string',
              maxTransitTime: 'Test string',
              minHandlingTime: 'Test string',
              minTransitTime: 'Test string',
              postalCode: 'Test string',
              price: {
                amountMicros: 'Test string',
                currencyCode: 'Test string',
              },
              region: 'Test string',
              service: 'Test string',
            },
          ],
          shippingHandlingBusinessDays: [
            {
              businessDays: 'Test string',
              country: 'Test string',
            },
          ],
          shippingHeight: {
            unit: 'Test string',
            value: 42,
          },
          shippingLabel: 'Test string',
          shippingLength: {
            unit: 'Test string',
            value: 42,
          },
          shippingTransitBusinessDays: [
            {
              businessDays: 'Test string',
              country: 'Test string',
            },
          ],
          shippingWeight: {
            unit: 'Test string',
            value: 42,
          },
          shippingWidth: {
            unit: 'Test string',
            value: 42,
          },
          shoppingAdsExcludedCountries: ['Test string'],
          shortTitle: 'Test string',
          size: 'Test string',
          sizeSystem: 'Test string',
          sizeTypes: ['Test string'],
          specialtyHousingType: 'Test string',
          structuredDescription: {
            content: 'Test string',
            digitalSourceType: 'Test string',
          },
          structuredTitle: {
            content: 'Test string',
            digitalSourceType: 'Test string',
          },
          subscriptionCost: {
            amount: {
              amountMicros: 'Test string',
              currencyCode: 'Test string',
            },
            period: 'Test string',
            periodLength: 'Test string',
          },
          sustainabilityIncentives: [
            {
              amount: {
                amountMicros: 'Test string',
                currencyCode: 'Test string',
              },
              percentage: 42,
              type: 'Test string',
            },
          ],
          title: 'Test string',
          transitTimeLabel: 'Test string',
          trim: 'Test string',
          unitArea: {
            unit: 'Test string',
            value: 42,
          },
          unitPricingBaseMeasure: {
            unit: 'Test string',
            value: 'Test string',
          },
          unitPricingMeasure: {
            unit: 'Test string',
            value: 42,
          },
          utilitiesIncluded: ['Test string'],
          variantOptions: [
            {
              name: 'Test string',
              value: 'Test string',
            },
          ],
          vehicleAllInPrice: {
            amountMicros: 'Test string',
            currencyCode: 'Test string',
          },
          vehicleExpenses: {
            amountMicros: 'Test string',
            currencyCode: 'Test string',
          },
          vehicleMandatoryInspectionIncluded: true,
          vehicleMsrp: {
            amountMicros: 'Test string',
            currencyCode: 'Test string',
          },
          vehiclePriceType: 'Test string',
          videoLinks: ['Test string'],
          vin: 'Test string',
          virtualModelLink: 'Test string',
          warranty: {
            duration: 'Test string',
            mileage: {
              unit: 'Test string',
              value: 'Test string',
            },
          },
          year: 'Test string',
        },
        versionNumber: 'Test string',
      },
    );
    /** Updates the existing product input in your Merchant Center account. The name of the product input to update is taken from the `name` field within the `ProductInput` resource. After inserting, updating, or deleting a product input, it may take several minutes before the processed product can be retrieved. */
    await gapi.client.merchantapi.accounts.productInputs.patch(
      {
        dataSource: 'Test string',
        name: 'Test string',
        updateMask: 'Test string',
      },
      {
        base64EncodedName: 'Test string',
        base64EncodedProduct: 'Test string',
        contentLanguage: 'Test string',
        customAttributes: [
          {
            groupValues: undefined,
            name: 'Test string',
            value: 'Test string',
          },
        ],
        feedLabel: 'Test string',
        legacyLocal: true,
        name: 'Test string',
        offerId: 'Test string',
        product: 'Test string',
        productAttributes: {
          additionalImageLinks: ['Test string'],
          adsGrouping: 'Test string',
          adsLabels: ['Test string'],
          adsRedirect: 'Test string',
          adult: true,
          ageGroup: 'Test string',
          amenityFeature: ['Test string'],
          autoPricingMinPrice: {
            amountMicros: 'Test string',
            currencyCode: 'Test string',
          },
          availability: 'Test string',
          availabilityDate: 'Test string',
          bodyStyle: 'Test string',
          brand: 'Test string',
          canonicalLink: 'Test string',
          carrierShipping: [
            {
              carrierPrice: 'Test string',
              carrierPriceFlatAdjustment: {
                amountMicros: 'Test string',
                currencyCode: 'Test string',
              },
              carrierPricePercentageAdjustment: 42,
              carrierTransitTime: 'Test string',
              country: 'Test string',
              fixedMaxTransitTime: 'Test string',
              fixedMinTransitTime: 'Test string',
              flatPrice: {
                amountMicros: 'Test string',
                currencyCode: 'Test string',
              },
              maxHandlingTime: 'Test string',
              minHandlingTime: 'Test string',
              originPostalCode: 'Test string',
              postalCode: 'Test string',
              region: 'Test string',
            },
          ],
          certifications: [
            {
              certificationAuthority: 'Test string',
              certificationCode: 'Test string',
              certificationDocumentLink: 'Test string',
              certificationLabelLink: 'Test string',
              certificationName: 'Test string',
              certificationValue: 'Test string',
            },
          ],
          certifiedPreOwned: true,
          cloudExportAdditionalProperties: [
            {
              boolValue: true,
              floatValue: [42],
              intValue: ['Test string'],
              maxValue: 42,
              minValue: 42,
              propertyName: 'Test string',
              textValue: ['Test string'],
              unitCode: 'Test string',
            },
          ],
          co2Emissions: {
            unit: 'Test string',
            value: 'Test string',
          },
          color: 'Test string',
          condition: 'Test string',
          costOfGoodsSold: {
            amountMicros: 'Test string',
            currencyCode: 'Test string',
          },
          customLabel0: 'Test string',
          customLabel1: 'Test string',
          customLabel2: 'Test string',
          customLabel3: 'Test string',
          customLabel4: 'Test string',
          dateFirstRegistered: 'Test string',
          description: 'Test string',
          disclosureDate: 'Test string',
          displayAddress: {
            city: 'Test string',
            postalCode: 'Test string',
            region: 'Test string',
            streetName: 'Test string',
            streetNumber: 'Test string',
          },
          displayAdsId: 'Test string',
          displayAdsLink: 'Test string',
          displayAdsSimilarIds: ['Test string'],
          displayAdsTitle: 'Test string',
          displayAdsValue: 42,
          documentLinks: ['Test string'],
          electricRange: {
            unit: 'Test string',
            value: 'Test string',
          },
          emissionsStandard: 'Test string',
          energyConsumption: {
            unit: 'Test string',
            value: 42,
          },
          energyEfficiencyClass: 'Test string',
          engine: 'Test string',
          excludedDestinations: ['Test string'],
          expirationDate: 'Test string',
          externalSellerId: 'Test string',
          freeShippingThreshold: [
            {
              country: 'Test string',
              priceThreshold: {
                amountMicros: 'Test string',
                currencyCode: 'Test string',
              },
            },
          ],
          fuelConsumption: {
            unit: 'Test string',
            value: 42,
          },
          fuelConsumptionDischargedBattery: {
            unit: 'Test string',
            value: 42,
          },
          gender: 'Test string',
          googleProductCategory: 'Test string',
          gtins: ['Test string'],
          handlingCutoffTimes: [
            {
              country: 'Test string',
              cutoffTime: 'Test string',
              cutoffTimezone: 'Test string',
              disableDeliveryAfterCutoff: true,
            },
          ],
          identifierExists: true,
          imageLink: 'Test string',
          includedDestinations: ['Test string'],
          installment: {
            amount: {
              amountMicros: 'Test string',
              currencyCode: 'Test string',
            },
            annualPercentageRate: 42,
            creditType: 'Test string',
            downpayment: {
              amountMicros: 'Test string',
              currencyCode: 'Test string',
            },
            months: 'Test string',
            totalAmount: {
              amountMicros: 'Test string',
              currencyCode: 'Test string',
            },
          },
          isBundle: true,
          itemGroupId: 'Test string',
          itemGroupTitle: 'Test string',
          latitude: 42,
          leaseTerm: {
            durationUnit: 'Test string',
            durationValue: 'Test string',
            type: 'Test string',
          },
          lifestyleImageLinks: ['Test string'],
          link: 'Test string',
          linkTemplate: 'Test string',
          longitude: 42,
          loyaltyPoints: {
            name: 'Test string',
            pointsValue: 'Test string',
            ratio: 42,
          },
          loyaltyPrograms: [
            {
              cashbackForFutureUse: {
                amountMicros: 'Test string',
                currencyCode: 'Test string',
              },
              loyaltyPoints: 'Test string',
              memberPriceEffectiveDate: {
                endTime: 'Test string',
                startTime: 'Test string',
              },
              price: {
                amountMicros: 'Test string',
                currencyCode: 'Test string',
              },
              programLabel: 'Test string',
              shippingLabel: 'Test string',
              tierLabel: 'Test string',
            },
          ],
          material: 'Test string',
          maxEnergyEfficiencyClass: 'Test string',
          maxHandlingTime: 'Test string',
          maximumRetailPrice: {
            amountMicros: 'Test string',
            currencyCode: 'Test string',
          },
          mileage: {
            unit: 'Test string',
            value: 'Test string',
          },
          minEnergyEfficiencyClass: 'Test string',
          minHandlingTime: 'Test string',
          minimumOrderValues: [
            {
              country: 'Test string',
              price: {
                amountMicros: 'Test string',
                currencyCode: 'Test string',
              },
              service: 'Test string',
              surface: 'Test string',
            },
          ],
          mobileLink: 'Test string',
          mobileLinkTemplate: 'Test string',
          model: 'Test string',
          mpn: 'Test string',
          multipack: 'Test string',
          neighborhood: 'Test string',
          numberOfBathrooms: 42,
          numberOfBedrooms: 42,
          numberOfUnits: 'Test string',
          pattern: 'Test string',
          pause: 'Test string',
          petPolicy: {
            petsAllowed: true,
            petTypes: ['Test string'],
          },
          pickupCost: {
            flatRate: {
              amountMicros: 'Test string',
              currencyCode: 'Test string',
            },
            freeThreshold: {
              amountMicros: 'Test string',
              currencyCode: 'Test string',
            },
          },
          pickupMethod: 'Test string',
          pickupSla: 'Test string',
          popularityRank: 42,
          price: {
            amountMicros: 'Test string',
            currencyCode: 'Test string',
          },
          productDetails: [
            {
              attributeName: 'Test string',
              attributeValue: 'Test string',
              sectionName: 'Test string',
            },
          ],
          productFee: [
            {
              amount: {
                amountMicros: 'Test string',
                currencyCode: 'Test string',
              },
              type: 'Test string',
            },
          ],
          productHeight: {
            unit: 'Test string',
            value: 42,
          },
          productHighlights: ['Test string'],
          productLength: {
            unit: 'Test string',
            value: 42,
          },
          productTypes: ['Test string'],
          productWeight: {
            unit: 'Test string',
            value: 42,
          },
          productWidth: {
            unit: 'Test string',
            value: 42,
          },
          promotionIds: ['Test string'],
          propertyName: 'Test string',
          propertyType: 'Test string',
          questionsAndAnswers: [
            {
              answer: 'Test string',
              question: 'Test string',
            },
          ],
          relatedProducts: [
            {
              id: 'Test string',
              idType: 'Test string',
              relationshipType: 'Test string',
            },
          ],
          returnPolicyLabel: 'Test string',
          returns: [
            {
              countries: ['Test string'],
              itemConditions: ['Test string'],
              methods: ['Test string'],
              outcomes: ['Test string'],
              policyUrl: 'Test string',
              restockingFee: {
                amountMicros: 'Test string',
                currencyCode: 'Test string',
              },
              restockingPercentageFee: 42,
              shippingFee: {
                amountMicros: 'Test string',
                currencyCode: 'Test string',
              },
              shippingFeeType: 'Test string',
              windowDays: 'Test string',
              windowType: 'Test string',
            },
          ],
          salePrice: {
            amountMicros: 'Test string',
            currencyCode: 'Test string',
          },
          salePriceEffectiveDate: {
            endTime: 'Test string',
            startTime: 'Test string',
          },
          sellOnGoogleQuantity: 'Test string',
          shipping: [
            {
              country: 'Test string',
              handlingCutoffTime: 'Test string',
              handlingCutoffTimezone: 'Test string',
              locationGroupName: 'Test string',
              locationId: 'Test string',
              loyaltyProgramLabel: 'Test string',
              loyaltyTierLabel: 'Test string',
              maxHandlingTime: 'Test string',
              maxTransitTime: 'Test string',
              minHandlingTime: 'Test string',
              minTransitTime: 'Test string',
              postalCode: 'Test string',
              price: {
                amountMicros: 'Test string',
                currencyCode: 'Test string',
              },
              region: 'Test string',
              service: 'Test string',
            },
          ],
          shippingHandlingBusinessDays: [
            {
              businessDays: 'Test string',
              country: 'Test string',
            },
          ],
          shippingHeight: {
            unit: 'Test string',
            value: 42,
          },
          shippingLabel: 'Test string',
          shippingLength: {
            unit: 'Test string',
            value: 42,
          },
          shippingTransitBusinessDays: [
            {
              businessDays: 'Test string',
              country: 'Test string',
            },
          ],
          shippingWeight: {
            unit: 'Test string',
            value: 42,
          },
          shippingWidth: {
            unit: 'Test string',
            value: 42,
          },
          shoppingAdsExcludedCountries: ['Test string'],
          shortTitle: 'Test string',
          size: 'Test string',
          sizeSystem: 'Test string',
          sizeTypes: ['Test string'],
          specialtyHousingType: 'Test string',
          structuredDescription: {
            content: 'Test string',
            digitalSourceType: 'Test string',
          },
          structuredTitle: {
            content: 'Test string',
            digitalSourceType: 'Test string',
          },
          subscriptionCost: {
            amount: {
              amountMicros: 'Test string',
              currencyCode: 'Test string',
            },
            period: 'Test string',
            periodLength: 'Test string',
          },
          sustainabilityIncentives: [
            {
              amount: {
                amountMicros: 'Test string',
                currencyCode: 'Test string',
              },
              percentage: 42,
              type: 'Test string',
            },
          ],
          title: 'Test string',
          transitTimeLabel: 'Test string',
          trim: 'Test string',
          unitArea: {
            unit: 'Test string',
            value: 42,
          },
          unitPricingBaseMeasure: {
            unit: 'Test string',
            value: 'Test string',
          },
          unitPricingMeasure: {
            unit: 'Test string',
            value: 42,
          },
          utilitiesIncluded: ['Test string'],
          variantOptions: [
            {
              name: 'Test string',
              value: 'Test string',
            },
          ],
          vehicleAllInPrice: {
            amountMicros: 'Test string',
            currencyCode: 'Test string',
          },
          vehicleExpenses: {
            amountMicros: 'Test string',
            currencyCode: 'Test string',
          },
          vehicleMandatoryInspectionIncluded: true,
          vehicleMsrp: {
            amountMicros: 'Test string',
            currencyCode: 'Test string',
          },
          vehiclePriceType: 'Test string',
          videoLinks: ['Test string'],
          vin: 'Test string',
          virtualModelLink: 'Test string',
          warranty: {
            duration: 'Test string',
            mileage: {
              unit: 'Test string',
              value: 'Test string',
            },
          },
          year: 'Test string',
        },
        versionNumber: 'Test string',
      },
    );
    /** Retrieves the processed product from your Merchant Center account. After inserting, updating, or deleting a product input, it may take several minutes before the updated final product can be retrieved. */
    await gapi.client.merchantapi.accounts.products.get({
      name: 'Test string',
    });
    /** Lists the processed products in your Merchant Center account. The response might contain fewer items than specified by `pageSize`. Rely on `pageToken` to determine if there are more items to be requested. After inserting, updating, or deleting a product input, it may take several minutes before the updated processed product can be retrieved. */
    await gapi.client.merchantapi.accounts.products.list({
      pageSize: 42,
      pageToken: 'Test string',
      parent: 'Test string',
    });
  }
});
