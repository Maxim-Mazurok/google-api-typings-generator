...
declare namespace gapi.client {
    ...
    namespace domains {
        ...
        interface RegistrationsResource {
            ...
            /**
             * Deletes a `Registration` resource. For `Registration` resources , this method works if: * `state` is `EXPORTED` with `expire_time` in the past * `state` is `REGISTRATION_FAILED`
             * When an active domain is successfully deleted, you can continue to use the domain in [Google Domains](https://domains.google/) until it expires. The calling user becomes the
             * domain's sole owner in Google Domains, and permissions for the domain are subsequently managed there. The domain will not renew automatically unless the new owner sets up billing in
             * Google Domains.
             */
            delete(request?: {
                /** V1 error format. */
