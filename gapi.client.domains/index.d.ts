...
declare namespace gapi.client {
    ...
    namespace domains {
        ...
        interface TransferDomainRequest {
             ...
             * registrar. Use this option only if you are sure that the domain's current DNS service will not cease upon transfer, as is often the case for DNS services provided for free by the
             * registrar.
             */
            registration?: Registration;
        ...
        interface RegistrationsResource {
            ...
            /**
             * Deletes a `Registration` resource. For `Registration` resources using usage billing, this method works if: * `state` is `EXPORTED` with `expire_time` in the past * `state` is
             * `REGISTRATION_FAILED` * `state` is `TRANSFER_FAILED` This method works on any `Registration` resource using subscription billing, provided that the resource was created at least 1
             * day in the past. When an active domain is successfully deleted, you can continue to use the domain in [Google Domains](https://domains.google/) until it expires. The calling user
             * becomes the domain's sole owner in Google Domains, and permissions for the domain are subsequently managed there. The domain will not renew automatically unless the new owner sets
             * up billing in Google Domains.
             */
            delete(request?: {
                /** V1 error format. */
                "$.xgafv"?: string;
             ...
             * there. The domain will not renew automatically unless the new owner sets up billing in Google Domains.
             */
            export(request: {
                /** V1 error format. */
                "$.xgafv"?: string;
            ...
            /**
             * Gets parameters needed to transfer a domain name from another registrar to Cloud Domains. For domains managed by Google Domains, transferring to Cloud Domains is not yet supported.
            ...
            /**
             * Transfers a domain name from another registrar to Cloud Domains. For domains managed by Google Domains, transferring to Cloud Domains is not yet supported. Before calling this
             * method, go to the domain's current registrar to unlock the domain for transfer and retrieve the domain's transfer authorization code. Then call `RetrieveTransferParameters` to
