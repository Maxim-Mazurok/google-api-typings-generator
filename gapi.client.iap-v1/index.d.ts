...
declare namespace gapi.client {
    ...
    function load(urlOrObject: "https://iap.googleapis.com/$discovery/rest?version=v1"): Promise<void>;
    ...
    namespace iap {
        ...
        interface AttributePropagationSettings {
            /**
             * Whether the provided attribute propagation settings should be evaluated on user requests. If set to true, attributes returned from the expression will be propagated in the set
             * output credentials.
             */
            enable?:
                boolean;
             ...
             * `attributes.saml_attributes`, `attributes.iap_attributes`. The following functions are supported: - filter `.filter(, )`: Returns a subset of `` where `` is true for every item. -
             * in ` in `: Returns true if `` contains ``. - selectByName `.selectByName()`: Returns the attribute in `` with the given `` name, otherwise returns empty. - emitAs `.emitAs()`: Sets
             * the `` name field to the given `` for propagation in selected output credentials. - strict `.strict()`: Ignores the `x-goog-iap-attr-` prefix for the provided `` when propagating
             * with the `HEADER` output credential, such as request headers. - append `.append()` OR `.append()`: Appends the provided `` or `` to the end of ``. Example expression:
        ...
        interface Resource {
            /**
             * The proto or JSON formatted expected next state of the resource, wrapped in a google.protobuf.Any proto, against which the policy rules are evaluated. Services not integrated with
             * custom org policy can omit this field. Services integrated with custom org policy must populate this field for all requests where the API call changes the state of the resource.
             * Custom org policy backend uses these attributes to enforce custom org policies. When a proto is wrapped, it is generally the One Platform API proto. When a JSON string is wrapped,
             * use `google.protobuf.StringValue` for the inner value. It is sufficient to pass just the max set of attributes that are allowed for use in custom constraints; other attributes can
             * be omitted. See go/custom-constraints-org-policy-integration-guide for additional details.
             */
            expectedNextState?:
                { [P in string]: any };
            /**
        ...
        interface V1Resource {
            ...
            updateIapSettings(request: {
                ...
                quotaUser?:
                    string;
                /**
                 * The field mask specifying which IAP settings should be updated. If omitted, then all of the settings are updated. See
                 * https://developers.google.com/protocol-buffers/docs/reference/google.protobuf#fieldmask. Note: All IAP reauth settings must always be set together, using the field mask:
