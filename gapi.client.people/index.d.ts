...
declare namespace gapi.client {
    ...
    namespace people {
        ...
        interface Person {
            ...
            /**
             * The person's email addresses. For [`connections.list`](/people/api/rest/v1/people.connections/list), [`otherContacts.list`](/people/api/rest/v1/otherContacts/list), and
             * [`people.listDirectoryPeople`](/people/api/rest/v1/people/listDirectoryPeople) the number of email addresses is limited to 100. If a Person has more email addresses the entire set
             * can be obtained by calling ['people.get'](/people/api/rest/v1/people/get).
             */
            ...
            /**
             * The person's phone numbers. For [`connections.list`](/people/api/rest/v1/people.connections/list), [`otherContacts.list`](/people/api/rest/v1/otherContacts/list), and
             * [`people.listDirectoryPeople`](/people/api/rest/v1/people/listDirectoryPeople) the number of phone numbers is limited to 100. If a Person has more phone numbers the entire set can
             * be obtained by calling ['people.get'](/people/api/rest/v1/people/get).
             */
        ...
        interface PersonMetadata {
            /**
             * Output only. True if the person resource has been deleted. Populated only for [`connections.list`](/people/api/rest/v1/people.connections/list) and
        ...
        interface OtherContactsResource {
            ...
            list(request?: {
                 ...
                 * `sync_token` have an additional rate limit. When the `syncToken` is specified, resources deleted since the last sync will be returned as a person with
                 * [`PersonMetadata.deleted`](/people/api/rest/v1/people#Person.PersonMetadata.FIELDS.deleted) set to true. When the `syncToken` is specified, all other parameters provided to
        ...
        interface ConnectionsResource {
             ...
             * specified and is expired. Sync tokens expire after 7 days to prevent data drift between clients and the server. To handle a sync token expired error, a request should be sent
            ...
            list(request?: {
                 ...
                 * the `sync_token` param instead. Initial full sync requests that specify `request_sync_token` and do not specify `sync_token` have an additional rate limit per user. Each client
                 * should generally only be doing a full sync once every few days per user and so should not hit this limit.
                 */
                requestSyncToken?: boolean;
                ...
                /**
                 * Optional. A sync token, received from a previous `ListConnections` call. Provide this to retrieve only the resources changed since the last request. When the `syncToken` is
                 * specified, resources deleted since the last sync will be returned as a person with [`PersonMetadata.deleted`](/people/api/rest/v1/people#Person.PersonMetadata.FIELDS.deleted)
                 * set to true. When the `syncToken` is specified, all other parameters provided to `ListConnections` except `page_size` and `page_token` must match the initial call that provided
                 * the sync token. Sync tokens expire after seven days, after which a full sync request without a `sync_token` should be made.
                 */
                syncToken?: string;
                /** Upload protocol for media (e.g. "raw", "multipart"). */
                upload_protocol?: string;
        ...
        interface PeopleResource {
            ...
            getBatchGet(request?: {
                 ...
                 * contact, specify the resource name that identifies the contact as returned by [`people.connections.list`](/people/api/rest/v1/people.connections/list). There is a maximum of 200
                 * resource names.
                 */
                resourceNames?: string | string[];
                /** Optional. A mask of what source types to return. Defaults to READ_SOURCE_TYPE_CONTACT and READ_SOURCE_TYPE_PROFILE if not set. */
