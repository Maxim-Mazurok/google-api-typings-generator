/* This is stub file for gapi.client.networksecurity-v1beta1 definition tests */
// IMPORTANT
// This file was generated by https://github.com/Maxim-Mazurok/google-api-typings-generator. Please do not edit it manually.
// In case of any problems please post issue to https://github.com/Maxim-Mazurok/google-api-typings-generator

// Revision: 20260824

gapi.load('client', async () => {
  /** now we can use gapi.client */

  await gapi.client.load(
    'https://networksecurity.googleapis.com/$discovery/rest?version=v1beta1',
  );
  /** now we can use gapi.client.networksecurity */

  /** don't forget to authenticate your client before sending any request to resources: */
  /** declare client_id registered in Google Developers Console */
  const client_id = '<<PUT YOUR CLIENT ID HERE>>';
  const scope = [
    /** See, edit, configure, and delete your Google Cloud data and see the email address for your Google Account. */
    'https://www.googleapis.com/auth/cloud-platform',
  ];
  const immediate = false;
  gapi.auth.authorize({client_id, scope, immediate}, authResult => {
    if (authResult && !authResult.error) {
      /** handle successful authorization */
      void run();
    } else {
      /** handle authorization error */
    }
  });

  async function run() {
    /** Gets information about a location. */
    await gapi.client.networksecurity.organizations.locations.get({
      name: 'Test string',
    });
    /** Lists information about the supported locations for this service. This method lists locations based on the resource scope provided in the ListLocationsRequest.name field: * **Global locations**: If `name` is empty, the method lists the public locations available to all projects. * **Project-specific locations**: If `name` follows the format `projects/{project}`, the method lists locations visible to that specific project. This includes public, private, or other project-specific locations enabled for the project. For gRPC and client library implementations, the resource name is passed as the `name` field. For direct service calls, the resource name is incorporated into the request path based on the specific service implementation and version. */
    await gapi.client.networksecurity.organizations.locations.list({
      extraLocationTypes: 'Test string',
      filter: 'Test string',
      name: 'Test string',
      pageSize: 42,
      pageToken: 'Test string',
    });
    /** Adds items to an address group. */
    await gapi.client.networksecurity.organizations.locations.addressGroups.addItems(
      {
        addressGroup: 'Test string',
      },
      {
        items: ['Test string'],
        requestId: 'Test string',
      },
    );
    /** Clones items from one address group to another. */
    await gapi.client.networksecurity.organizations.locations.addressGroups.cloneItems(
      {
        addressGroup: 'Test string',
      },
      {
        requestId: 'Test string',
        sourceAddressGroup: 'Test string',
      },
    );
    /** Creates a new address group in a given project and location. */
    await gapi.client.networksecurity.organizations.locations.addressGroups.create(
      {
        addressGroupId: 'Test string',
        parent: 'Test string',
        requestId: 'Test string',
      },
      {
        capacity: 42,
        createTime: 'Test string',
        description: 'Test string',
        items: ['Test string'],
        labels: {
          A: 'Test string',
        },
        name: 'Test string',
        purpose: ['Test string'],
        selfLink: 'Test string',
        type: 'Test string',
        updateTime: 'Test string',
      },
    );
    /** Deletes an address group. */
    await gapi.client.networksecurity.organizations.locations.addressGroups.delete(
      {
        name: 'Test string',
        requestId: 'Test string',
      },
    );
    /** Gets details of a single address group. */
    await gapi.client.networksecurity.organizations.locations.addressGroups.get(
      {
        name: 'Test string',
      },
    );
    /** Lists address groups in a given project and location. */
    await gapi.client.networksecurity.organizations.locations.addressGroups.list(
      {
        pageSize: 42,
        pageToken: 'Test string',
        parent: 'Test string',
        returnPartialSuccess: true,
      },
    );
    /** Lists references of an address group. */
    await gapi.client.networksecurity.organizations.locations.addressGroups.listReferences(
      {
        addressGroup: 'Test string',
        pageSize: 42,
        pageToken: 'Test string',
      },
    );
    /** Updates parameters of an address group. */
    await gapi.client.networksecurity.organizations.locations.addressGroups.patch(
      {
        name: 'Test string',
        requestId: 'Test string',
        updateMask: 'Test string',
      },
      {
        capacity: 42,
        createTime: 'Test string',
        description: 'Test string',
        items: ['Test string'],
        labels: {
          A: 'Test string',
        },
        name: 'Test string',
        purpose: ['Test string'],
        selfLink: 'Test string',
        type: 'Test string',
        updateTime: 'Test string',
      },
    );
    /** Removes items from an address group. */
    await gapi.client.networksecurity.organizations.locations.addressGroups.removeItems(
      {
        addressGroup: 'Test string',
      },
      {
        items: ['Test string'],
        requestId: 'Test string',
      },
    );
    /** Returns permissions that a caller has on the specified resource. If the resource does not exist, this will return an empty set of permissions, not a `NOT_FOUND` error. Note: This operation is designed to be used for building permission-aware UIs and command-line tools, not for authorization checking. This operation may "fail open" without warning. */
    await gapi.client.networksecurity.organizations.locations.addressGroups.testIamPermissions(
      {
        resource: 'Test string',
      },
      {
        permissions: ['Test string'],
      },
    );
    /** Creates a new FirewallEndpoint in a given organization and location. */
    await gapi.client.networksecurity.organizations.locations.firewallEndpoints.create(
      {
        firewallEndpointId: 'Test string',
        parent: 'Test string',
        requestId: 'Test string',
        validateOnly: true,
      },
      {
        associatedNetworks: ['Test string'],
        associations: [
          {
            name: 'Test string',
            network: 'Test string',
          },
        ],
        billingProjectId: 'Test string',
        createTime: 'Test string',
        description: 'Test string',
        endpointSettings: {
          contentCloudRegion: 'Test string',
          httpPartialResponseBlocked: true,
          jumboFramesEnabled: true,
        },
        explicitPrivateServiceConnectAttachment: 'Test string',
        labels: {
          A: 'Test string',
        },
        name: 'Test string',
        reconciling: true,
        satisfiesPzi: true,
        satisfiesPzs: true,
        state: 'Test string',
        updateTime: 'Test string',
        wildfireSettings: {
          enabled: true,
          wildfireInlineCloudAnalysisSettings: {
            maxAnalysisDuration: 'Test string',
            submissionTimeoutLoggingDisabled: true,
            timeoutAction: 'Test string',
          },
          wildfireRealtimeLookupDuration: 'Test string',
          wildfireRealtimeLookupTimeoutAction: 'Test string',
          wildfireRegion: 'Test string',
        },
      },
    );
    /** Deletes a single org Endpoint. */
    await gapi.client.networksecurity.organizations.locations.firewallEndpoints.delete(
      {
        name: 'Test string',
        requestId: 'Test string',
      },
    );
    /** Gets details of a single org Endpoint. */
    await gapi.client.networksecurity.organizations.locations.firewallEndpoints.get(
      {
        name: 'Test string',
      },
    );
    /** Lists FirewallEndpoints in a given organization and location. */
    await gapi.client.networksecurity.organizations.locations.firewallEndpoints.list(
      {
        filter: 'Test string',
        orderBy: 'Test string',
        pageSize: 42,
        pageToken: 'Test string',
        parent: 'Test string',
      },
    );
    /** Update a single org Endpoint. */
    await gapi.client.networksecurity.organizations.locations.firewallEndpoints.patch(
      {
        name: 'Test string',
        requestId: 'Test string',
        updateMask: 'Test string',
      },
      {
        associatedNetworks: ['Test string'],
        associations: [
          {
            name: 'Test string',
            network: 'Test string',
          },
        ],
        billingProjectId: 'Test string',
        createTime: 'Test string',
        description: 'Test string',
        endpointSettings: {
          contentCloudRegion: 'Test string',
          httpPartialResponseBlocked: true,
          jumboFramesEnabled: true,
        },
        explicitPrivateServiceConnectAttachment: 'Test string',
        labels: {
          A: 'Test string',
        },
        name: 'Test string',
        reconciling: true,
        satisfiesPzi: true,
        satisfiesPzs: true,
        state: 'Test string',
        updateTime: 'Test string',
        wildfireSettings: {
          enabled: true,
          wildfireInlineCloudAnalysisSettings: {
            maxAnalysisDuration: 'Test string',
            submissionTimeoutLoggingDisabled: true,
            timeoutAction: 'Test string',
          },
          wildfireRealtimeLookupDuration: 'Test string',
          wildfireRealtimeLookupTimeoutAction: 'Test string',
          wildfireRegion: 'Test string',
        },
      },
    );
    /** Create WildfireVerdictChangeRequest in a given Firewall Endpoint in an organization and location. */
    await gapi.client.networksecurity.organizations.locations.firewallEndpoints.wildfireVerdictChangeRequests.create(
      {
        parent: 'Test string',
      },
      {
        comment: 'Test string',
        createTime: 'Test string',
        fileName: 'Test string',
        fileType: 'Test string',
        finalVerdict: 'Test string',
        name: 'Test string',
        newVerdict: 'Test string',
        oldVerdict: 'Test string',
        resolutionTime: 'Test string',
        sha256: 'Test string',
        sourceRegion: 'Test string',
        state: 'Test string',
        updateTime: 'Test string',
        wildfireVerdictChangeRequestId: 'Test string',
      },
    );
    /** Get WildfireVerdictChangeRequest in a given Firewall Endpoint in an organization and location. */
    await gapi.client.networksecurity.organizations.locations.firewallEndpoints.wildfireVerdictChangeRequests.get(
      {
        name: 'Test string',
      },
    );
    /** Lists WildfireVerdictChangeRequests in a given Firewall Endpoint in an organization and location. */
    await gapi.client.networksecurity.organizations.locations.firewallEndpoints.wildfireVerdictChangeRequests.list(
      {
        filter: 'Test string',
        pageSize: 42,
        pageToken: 'Test string',
        parent: 'Test string',
      },
    );
    /** Starts asynchronous cancellation on a long-running operation. The server makes a best effort to cancel the operation, but success is not guaranteed. If the server doesn't support this method, it returns `google.rpc.Code.UNIMPLEMENTED`. Clients can use Operations.GetOperation or other methods to check whether the cancellation succeeded or whether the operation completed despite cancellation. On successful cancellation, the operation is not deleted; instead, it becomes an operation with an Operation.error value with a google.rpc.Status.code of `1`, corresponding to `Code.CANCELLED`. */
    await gapi.client.networksecurity.organizations.locations.operations.cancel(
      {
        name: 'Test string',
      },
      {},
    );
    /** Deletes a long-running operation. This method indicates that the client is no longer interested in the operation result. It does not cancel the operation. If the server doesn't support this method, it returns `google.rpc.Code.UNIMPLEMENTED`. */
    await gapi.client.networksecurity.organizations.locations.operations.delete(
      {
        name: 'Test string',
      },
    );
    /** Gets the latest state of a long-running operation. Clients can use this method to poll the operation result at intervals as recommended by the API service. */
    await gapi.client.networksecurity.organizations.locations.operations.get({
      name: 'Test string',
    });
    /** Lists operations that match the specified filter in the request. If the server doesn't support this method, it returns `UNIMPLEMENTED`. */
    await gapi.client.networksecurity.organizations.locations.operations.list({
      filter: 'Test string',
      name: 'Test string',
      pageSize: 42,
      pageToken: 'Test string',
      returnPartialSuccess: true,
    });
    /** Creates a new SecurityProfileGroup in a given organization and location. */
    await gapi.client.networksecurity.organizations.locations.securityProfileGroups.create(
      {
        parent: 'Test string',
        securityProfileGroupId: 'Test string',
      },
      {
        createTime: 'Test string',
        customInterceptProfile: 'Test string',
        customMirroringProfile: 'Test string',
        dataPathId: 'Test string',
        description: 'Test string',
        etag: 'Test string',
        labels: {
          A: 'Test string',
        },
        name: 'Test string',
        threatPreventionProfile: 'Test string',
        updateTime: 'Test string',
        urlFilteringProfile: 'Test string',
        wildfireAnalysisProfile: 'Test string',
      },
    );
    /** Deletes a single SecurityProfileGroup. */
    await gapi.client.networksecurity.organizations.locations.securityProfileGroups.delete(
      {
        etag: 'Test string',
        name: 'Test string',
      },
    );
    /** Gets details of a single SecurityProfileGroup. */
    await gapi.client.networksecurity.organizations.locations.securityProfileGroups.get(
      {
        name: 'Test string',
      },
    );
    /** Lists SecurityProfileGroups in a given organization and location. */
    await gapi.client.networksecurity.organizations.locations.securityProfileGroups.list(
      {
        pageSize: 42,
        pageToken: 'Test string',
        parent: 'Test string',
      },
    );
    /** Updates the parameters of a single SecurityProfileGroup. */
    await gapi.client.networksecurity.organizations.locations.securityProfileGroups.patch(
      {
        name: 'Test string',
        updateMask: 'Test string',
      },
      {
        createTime: 'Test string',
        customInterceptProfile: 'Test string',
        customMirroringProfile: 'Test string',
        dataPathId: 'Test string',
        description: 'Test string',
        etag: 'Test string',
        labels: {
          A: 'Test string',
        },
        name: 'Test string',
        threatPreventionProfile: 'Test string',
        updateTime: 'Test string',
        urlFilteringProfile: 'Test string',
        wildfireAnalysisProfile: 'Test string',
      },
    );
    /** Creates a new SecurityProfile in a given organization and location. */
    await gapi.client.networksecurity.organizations.locations.securityProfiles.create(
      {
        parent: 'Test string',
        securityProfileId: 'Test string',
      },
      {
        createTime: 'Test string',
        customInterceptProfile: {
          interceptEndpointGroup: 'Test string',
        },
        customMirroringProfile: {
          mirroringEndpointGroup: 'Test string',
        },
        description: 'Test string',
        etag: 'Test string',
        labels: {
          A: 'Test string',
        },
        name: 'Test string',
        threatPreventionProfile: {
          antivirusOverrides: [
            {
              action: 'Test string',
              protocol: 'Test string',
            },
          ],
          severityOverrides: [
            {
              action: 'Test string',
              severity: 'Test string',
            },
          ],
          threatOverrides: [
            {
              action: 'Test string',
              threatId: 'Test string',
              type: 'Test string',
            },
          ],
        },
        type: 'Test string',
        updateTime: 'Test string',
        urlFilteringProfile: {
          urlFilters: [
            {
              filteringAction: 'Test string',
              priority: 42,
              urls: ['Test string'],
            },
          ],
        },
        wildfireAnalysisProfile: {
          wildfireInlineCloudAnalysisRules: [
            {
              action: 'Test string',
              customFileTypes: {
                fileTypes: ['Test string'],
              },
              direction: 'Test string',
              fileSelectionMode: 'Test string',
            },
          ],
          wildfireInlineMlOverrides: [
            {
              action: 'Test string',
              protocol: 'Test string',
            },
          ],
          wildfireInlineMlSetting: {
            fileExceptions: [
              {
                filename: 'Test string',
                partialHash: 'Test string',
              },
            ],
            inlineMlConfigs: [
              {
                action: 'Test string',
                fileType: 'Test string',
              },
            ],
          },
          wildfireInlineMlSettings: [
            {
              fileExceptions: [
                {
                  filename: 'Test string',
                  partialHash: 'Test string',
                },
              ],
              inlineMlConfigs: [
                {
                  action: 'Test string',
                  fileType: 'Test string',
                },
              ],
            },
          ],
          wildfireOverrides: [
            {
              action: 'Test string',
              protocol: 'Test string',
            },
          ],
          wildfireRealtimeLookup: true,
          wildfireSubmissionRules: [
            {
              customFileTypes: {
                fileTypes: ['Test string'],
              },
              direction: 'Test string',
              fileSelectionMode: 'Test string',
            },
          ],
          wildfireThreatOverrides: [
            {
              action: 'Test string',
              threatId: 'Test string',
            },
          ],
        },
      },
    );
    /** Deletes a single SecurityProfile. */
    await gapi.client.networksecurity.organizations.locations.securityProfiles.delete(
      {
        etag: 'Test string',
        name: 'Test string',
      },
    );
    /** Gets details of a single SecurityProfile. */
    await gapi.client.networksecurity.organizations.locations.securityProfiles.get(
      {
        name: 'Test string',
      },
    );
    /** Lists SecurityProfiles in a given organization and location. */
    await gapi.client.networksecurity.organizations.locations.securityProfiles.list(
      {
        pageSize: 42,
        pageToken: 'Test string',
        parent: 'Test string',
      },
    );
    /** Updates the parameters of a single SecurityProfile. */
    await gapi.client.networksecurity.organizations.locations.securityProfiles.patch(
      {
        name: 'Test string',
        updateMask: 'Test string',
      },
      {
        createTime: 'Test string',
        customInterceptProfile: {
          interceptEndpointGroup: 'Test string',
        },
        customMirroringProfile: {
          mirroringEndpointGroup: 'Test string',
        },
        description: 'Test string',
        etag: 'Test string',
        labels: {
          A: 'Test string',
        },
        name: 'Test string',
        threatPreventionProfile: {
          antivirusOverrides: [
            {
              action: 'Test string',
              protocol: 'Test string',
            },
          ],
          severityOverrides: [
            {
              action: 'Test string',
              severity: 'Test string',
            },
          ],
          threatOverrides: [
            {
              action: 'Test string',
              threatId: 'Test string',
              type: 'Test string',
            },
          ],
        },
        type: 'Test string',
        updateTime: 'Test string',
        urlFilteringProfile: {
          urlFilters: [
            {
              filteringAction: 'Test string',
              priority: 42,
              urls: ['Test string'],
            },
          ],
        },
        wildfireAnalysisProfile: {
          wildfireInlineCloudAnalysisRules: [
            {
              action: 'Test string',
              customFileTypes: {
                fileTypes: ['Test string'],
              },
              direction: 'Test string',
              fileSelectionMode: 'Test string',
            },
          ],
          wildfireInlineMlOverrides: [
            {
              action: 'Test string',
              protocol: 'Test string',
            },
          ],
          wildfireInlineMlSetting: {
            fileExceptions: [
              {
                filename: 'Test string',
                partialHash: 'Test string',
              },
            ],
            inlineMlConfigs: [
              {
                action: 'Test string',
                fileType: 'Test string',
              },
            ],
          },
          wildfireInlineMlSettings: [
            {
              fileExceptions: [
                {
                  filename: 'Test string',
                  partialHash: 'Test string',
                },
              ],
              inlineMlConfigs: [
                {
                  action: 'Test string',
                  fileType: 'Test string',
                },
              ],
            },
          ],
          wildfireOverrides: [
            {
              action: 'Test string',
              protocol: 'Test string',
            },
          ],
          wildfireRealtimeLookup: true,
          wildfireSubmissionRules: [
            {
              customFileTypes: {
                fileTypes: ['Test string'],
              },
              direction: 'Test string',
              fileSelectionMode: 'Test string',
            },
          ],
          wildfireThreatOverrides: [
            {
              action: 'Test string',
              threatId: 'Test string',
            },
          ],
        },
      },
    );
    /** Gets information about a location. */
    await gapi.client.networksecurity.projects.locations.get({
      name: 'Test string',
    });
    /** Lists information about the supported locations for this service. This method lists locations based on the resource scope provided in the ListLocationsRequest.name field: * **Global locations**: If `name` is empty, the method lists the public locations available to all projects. * **Project-specific locations**: If `name` follows the format `projects/{project}`, the method lists locations visible to that specific project. This includes public, private, or other project-specific locations enabled for the project. For gRPC and client library implementations, the resource name is passed as the `name` field. For direct service calls, the resource name is incorporated into the request path based on the specific service implementation and version. */
    await gapi.client.networksecurity.projects.locations.list({
      extraLocationTypes: 'Test string',
      filter: 'Test string',
      name: 'Test string',
      pageSize: 42,
      pageToken: 'Test string',
    });
    /** Adds items to an address group. */
    await gapi.client.networksecurity.projects.locations.addressGroups.addItems(
      {
        addressGroup: 'Test string',
      },
      {
        items: ['Test string'],
        requestId: 'Test string',
      },
    );
    /** Clones items from one address group to another. */
    await gapi.client.networksecurity.projects.locations.addressGroups.cloneItems(
      {
        addressGroup: 'Test string',
      },
      {
        requestId: 'Test string',
        sourceAddressGroup: 'Test string',
      },
    );
    /** Creates a new address group in a given project and location. */
    await gapi.client.networksecurity.projects.locations.addressGroups.create(
      {
        addressGroupId: 'Test string',
        parent: 'Test string',
        requestId: 'Test string',
      },
      {
        capacity: 42,
        createTime: 'Test string',
        description: 'Test string',
        items: ['Test string'],
        labels: {
          A: 'Test string',
        },
        name: 'Test string',
        purpose: ['Test string'],
        selfLink: 'Test string',
        type: 'Test string',
        updateTime: 'Test string',
      },
    );
    /** Deletes a single address group. */
    await gapi.client.networksecurity.projects.locations.addressGroups.delete({
      name: 'Test string',
      requestId: 'Test string',
    });
    /** Gets details of a single address group. */
    await gapi.client.networksecurity.projects.locations.addressGroups.get({
      name: 'Test string',
    });
    /** Gets the access control policy for a resource. Returns an empty policy if the resource exists and does not have a policy set. */
    await gapi.client.networksecurity.projects.locations.addressGroups.getIamPolicy(
      {
        'options.requestedPolicyVersion': 42,
        resource: 'Test string',
      },
    );
    /** Lists address groups in a given project and location. */
    await gapi.client.networksecurity.projects.locations.addressGroups.list({
      pageSize: 42,
      pageToken: 'Test string',
      parent: 'Test string',
      returnPartialSuccess: true,
    });
    /** Lists references of an address group. */
    await gapi.client.networksecurity.projects.locations.addressGroups.listReferences(
      {
        addressGroup: 'Test string',
        pageSize: 42,
        pageToken: 'Test string',
      },
    );
    /** Updates the parameters of a single address group. */
    await gapi.client.networksecurity.projects.locations.addressGroups.patch(
      {
        name: 'Test string',
        requestId: 'Test string',
        updateMask: 'Test string',
      },
      {
        capacity: 42,
        createTime: 'Test string',
        description: 'Test string',
        items: ['Test string'],
        labels: {
          A: 'Test string',
        },
        name: 'Test string',
        purpose: ['Test string'],
        selfLink: 'Test string',
        type: 'Test string',
        updateTime: 'Test string',
      },
    );
    /** Removes items from an address group. */
    await gapi.client.networksecurity.projects.locations.addressGroups.removeItems(
      {
        addressGroup: 'Test string',
      },
      {
        items: ['Test string'],
        requestId: 'Test string',
      },
    );
    /** Sets the access control policy on the specified resource. Replaces any existing policy. Can return `NOT_FOUND`, `INVALID_ARGUMENT`, and `PERMISSION_DENIED` errors. */
    await gapi.client.networksecurity.projects.locations.addressGroups.setIamPolicy(
      {
        resource: 'Test string',
      },
      {
        policy: {
          auditConfigs: [
            {
              auditLogConfigs: [
                {
                  exemptedMembers: ['Test string'],
                  logType: 'Test string',
                },
              ],
              service: 'Test string',
            },
          ],
          bindings: [
            {
              condition: {
                description: 'Test string',
                expression: 'Test string',
                location: 'Test string',
                title: 'Test string',
              },
              members: ['Test string'],
              role: 'Test string',
            },
          ],
          etag: 'Test string',
          version: 42,
        },
        updateMask: 'Test string',
      },
    );
    /** Returns permissions that a caller has on the specified resource. If the resource does not exist, this will return an empty set of permissions, not a `NOT_FOUND` error. Note: This operation is designed to be used for building permission-aware UIs and command-line tools, not for authorization checking. This operation may "fail open" without warning. */
    await gapi.client.networksecurity.projects.locations.addressGroups.testIamPermissions(
      {
        resource: 'Test string',
      },
      {
        permissions: ['Test string'],
      },
    );
    /** Creates a new AuthorizationPolicy in a given project and location. */
    await gapi.client.networksecurity.projects.locations.authorizationPolicies.create(
      {
        authorizationPolicyId: 'Test string',
        parent: 'Test string',
      },
      {
        action: 'Test string',
        createTime: 'Test string',
        description: 'Test string',
        labels: {
          A: 'Test string',
        },
        name: 'Test string',
        rules: [
          {
            destinations: [
              {
                hosts: ['Test string'],
                httpHeaderMatch: {
                  headerName: 'Test string',
                  regexMatch: 'Test string',
                },
                methods: ['Test string'],
                ports: [42],
              },
            ],
            sources: [
              {
                ipBlocks: ['Test string'],
                principals: ['Test string'],
              },
            ],
          },
        ],
        updateTime: 'Test string',
      },
    );
    /** Deletes a single AuthorizationPolicy. */
    await gapi.client.networksecurity.projects.locations.authorizationPolicies.delete(
      {
        name: 'Test string',
      },
    );
    /** Gets details of a single AuthorizationPolicy. */
    await gapi.client.networksecurity.projects.locations.authorizationPolicies.get(
      {
        name: 'Test string',
      },
    );
    /** Gets the access control policy for a resource. Returns an empty policy if the resource exists and does not have a policy set. */
    await gapi.client.networksecurity.projects.locations.authorizationPolicies.getIamPolicy(
      {
        'options.requestedPolicyVersion': 42,
        resource: 'Test string',
      },
    );
    /** Lists AuthorizationPolicies in a given project and location. */
    await gapi.client.networksecurity.projects.locations.authorizationPolicies.list(
      {
        pageSize: 42,
        pageToken: 'Test string',
        parent: 'Test string',
      },
    );
    /** Updates the parameters of a single AuthorizationPolicy. */
    await gapi.client.networksecurity.projects.locations.authorizationPolicies.patch(
      {
        name: 'Test string',
        updateMask: 'Test string',
      },
      {
        action: 'Test string',
        createTime: 'Test string',
        description: 'Test string',
        labels: {
          A: 'Test string',
        },
        name: 'Test string',
        rules: [
          {
            destinations: [
              {
                hosts: ['Test string'],
                httpHeaderMatch: {
                  headerName: 'Test string',
                  regexMatch: 'Test string',
                },
                methods: ['Test string'],
                ports: [42],
              },
            ],
            sources: [
              {
                ipBlocks: ['Test string'],
                principals: ['Test string'],
              },
            ],
          },
        ],
        updateTime: 'Test string',
      },
    );
    /** Sets the access control policy on the specified resource. Replaces any existing policy. Can return `NOT_FOUND`, `INVALID_ARGUMENT`, and `PERMISSION_DENIED` errors. */
    await gapi.client.networksecurity.projects.locations.authorizationPolicies.setIamPolicy(
      {
        resource: 'Test string',
      },
      {
        policy: {
          auditConfigs: [
            {
              auditLogConfigs: [
                {
                  exemptedMembers: ['Test string'],
                  logType: 'Test string',
                },
              ],
              service: 'Test string',
            },
          ],
          bindings: [
            {
              condition: {
                description: 'Test string',
                expression: 'Test string',
                location: 'Test string',
                title: 'Test string',
              },
              members: ['Test string'],
              role: 'Test string',
            },
          ],
          etag: 'Test string',
          version: 42,
        },
        updateMask: 'Test string',
      },
    );
    /** Returns permissions that a caller has on the specified resource. If the resource does not exist, this will return an empty set of permissions, not a `NOT_FOUND` error. Note: This operation is designed to be used for building permission-aware UIs and command-line tools, not for authorization checking. This operation may "fail open" without warning. */
    await gapi.client.networksecurity.projects.locations.authorizationPolicies.testIamPermissions(
      {
        resource: 'Test string',
      },
      {
        permissions: ['Test string'],
      },
    );
    /** Creates a new AuthzPolicy in a given project and location. */
    await gapi.client.networksecurity.projects.locations.authzPolicies.create(
      {
        authzPolicyId: 'Test string',
        parent: 'Test string',
        requestId: 'Test string',
      },
      {
        action: 'Test string',
        createTime: 'Test string',
        customProvider: {
          authzExtension: {
            resources: ['Test string'],
          },
          cloudIap: {},
        },
        description: 'Test string',
        httpRules: [
          {
            from: {
              notSources: [
                {
                  ipBlocks: [
                    {
                      length: 42,
                      prefix: 'Test string',
                    },
                  ],
                  principals: [
                    {
                      principal: {
                        contains: 'Test string',
                        exact: 'Test string',
                        ignoreCase: true,
                        prefix: 'Test string',
                        suffix: 'Test string',
                      },
                      principalSelector: 'Test string',
                    },
                  ],
                  resources: [
                    {
                      iamServiceAccount: {
                        contains: 'Test string',
                        exact: 'Test string',
                        ignoreCase: true,
                        prefix: 'Test string',
                        suffix: 'Test string',
                      },
                      tagValueIdSet: {
                        ids: ['Test string'],
                      },
                    },
                  ],
                },
              ],
              sources: [
                {
                  ipBlocks: [
                    {
                      length: 42,
                      prefix: 'Test string',
                    },
                  ],
                  principals: [
                    {
                      principal: {
                        contains: 'Test string',
                        exact: 'Test string',
                        ignoreCase: true,
                        prefix: 'Test string',
                        suffix: 'Test string',
                      },
                      principalSelector: 'Test string',
                    },
                  ],
                  resources: [
                    {
                      iamServiceAccount: {
                        contains: 'Test string',
                        exact: 'Test string',
                        ignoreCase: true,
                        prefix: 'Test string',
                        suffix: 'Test string',
                      },
                      tagValueIdSet: {
                        ids: ['Test string'],
                      },
                    },
                  ],
                },
              ],
            },
            to: {
              notOperations: [
                {
                  headerSet: {
                    headers: [
                      {
                        name: 'Test string',
                        value: {
                          contains: 'Test string',
                          exact: 'Test string',
                          ignoreCase: true,
                          prefix: 'Test string',
                          suffix: 'Test string',
                        },
                      },
                    ],
                  },
                  hosts: [
                    {
                      contains: 'Test string',
                      exact: 'Test string',
                      ignoreCase: true,
                      prefix: 'Test string',
                      suffix: 'Test string',
                    },
                  ],
                  mcp: {
                    baseProtocolMethodsOption: 'Test string',
                    methods: [
                      {
                        name: 'Test string',
                        params: [
                          {
                            contains: 'Test string',
                            exact: 'Test string',
                            ignoreCase: true,
                            prefix: 'Test string',
                            suffix: 'Test string',
                          },
                        ],
                      },
                    ],
                  },
                  methods: ['Test string'],
                  paths: [
                    {
                      contains: 'Test string',
                      exact: 'Test string',
                      ignoreCase: true,
                      prefix: 'Test string',
                      suffix: 'Test string',
                    },
                  ],
                  snis: [
                    {
                      contains: 'Test string',
                      exact: 'Test string',
                      ignoreCase: true,
                      prefix: 'Test string',
                      suffix: 'Test string',
                    },
                  ],
                },
              ],
              operations: [
                {
                  headerSet: {
                    headers: [
                      {
                        name: 'Test string',
                        value: {
                          contains: 'Test string',
                          exact: 'Test string',
                          ignoreCase: true,
                          prefix: 'Test string',
                          suffix: 'Test string',
                        },
                      },
                    ],
                  },
                  hosts: [
                    {
                      contains: 'Test string',
                      exact: 'Test string',
                      ignoreCase: true,
                      prefix: 'Test string',
                      suffix: 'Test string',
                    },
                  ],
                  mcp: {
                    baseProtocolMethodsOption: 'Test string',
                    methods: [
                      {
                        name: 'Test string',
                        params: [
                          {
                            contains: 'Test string',
                            exact: 'Test string',
                            ignoreCase: true,
                            prefix: 'Test string',
                            suffix: 'Test string',
                          },
                        ],
                      },
                    ],
                  },
                  methods: ['Test string'],
                  paths: [
                    {
                      contains: 'Test string',
                      exact: 'Test string',
                      ignoreCase: true,
                      prefix: 'Test string',
                      suffix: 'Test string',
                    },
                  ],
                  snis: [
                    {
                      contains: 'Test string',
                      exact: 'Test string',
                      ignoreCase: true,
                      prefix: 'Test string',
                      suffix: 'Test string',
                    },
                  ],
                },
              ],
            },
            when: 'Test string',
          },
        ],
        labels: {
          A: 'Test string',
        },
        name: 'Test string',
        networkRules: [
          {
            from: {
              notSources: [
                {
                  ipBlocks: [
                    {
                      length: 42,
                      prefix: 'Test string',
                    },
                  ],
                  principals: [
                    {
                      principal: {
                        contains: 'Test string',
                        exact: 'Test string',
                        ignoreCase: true,
                        prefix: 'Test string',
                        suffix: 'Test string',
                      },
                      principalSelector: 'Test string',
                    },
                  ],
                  resources: [
                    {
                      iamServiceAccount: {
                        contains: 'Test string',
                        exact: 'Test string',
                        ignoreCase: true,
                        prefix: 'Test string',
                        suffix: 'Test string',
                      },
                      tagValueIdSet: {
                        ids: ['Test string'],
                      },
                    },
                  ],
                },
              ],
              sources: [
                {
                  ipBlocks: [
                    {
                      length: 42,
                      prefix: 'Test string',
                    },
                  ],
                  principals: [
                    {
                      principal: {
                        contains: 'Test string',
                        exact: 'Test string',
                        ignoreCase: true,
                        prefix: 'Test string',
                        suffix: 'Test string',
                      },
                      principalSelector: 'Test string',
                    },
                  ],
                  resources: [
                    {
                      iamServiceAccount: {
                        contains: 'Test string',
                        exact: 'Test string',
                        ignoreCase: true,
                        prefix: 'Test string',
                        suffix: 'Test string',
                      },
                      tagValueIdSet: {
                        ids: ['Test string'],
                      },
                    },
                  ],
                },
              ],
            },
            to: {
              notOperations: [
                {
                  headerSet: {
                    headers: [
                      {
                        name: 'Test string',
                        value: {
                          contains: 'Test string',
                          exact: 'Test string',
                          ignoreCase: true,
                          prefix: 'Test string',
                          suffix: 'Test string',
                        },
                      },
                    ],
                  },
                  hosts: [
                    {
                      contains: 'Test string',
                      exact: 'Test string',
                      ignoreCase: true,
                      prefix: 'Test string',
                      suffix: 'Test string',
                    },
                  ],
                  mcp: {
                    baseProtocolMethodsOption: 'Test string',
                    methods: [
                      {
                        name: 'Test string',
                        params: [
                          {
                            contains: 'Test string',
                            exact: 'Test string',
                            ignoreCase: true,
                            prefix: 'Test string',
                            suffix: 'Test string',
                          },
                        ],
                      },
                    ],
                  },
                  methods: ['Test string'],
                  paths: [
                    {
                      contains: 'Test string',
                      exact: 'Test string',
                      ignoreCase: true,
                      prefix: 'Test string',
                      suffix: 'Test string',
                    },
                  ],
                  snis: [
                    {
                      contains: 'Test string',
                      exact: 'Test string',
                      ignoreCase: true,
                      prefix: 'Test string',
                      suffix: 'Test string',
                    },
                  ],
                },
              ],
              operations: [
                {
                  headerSet: {
                    headers: [
                      {
                        name: 'Test string',
                        value: {
                          contains: 'Test string',
                          exact: 'Test string',
                          ignoreCase: true,
                          prefix: 'Test string',
                          suffix: 'Test string',
                        },
                      },
                    ],
                  },
                  hosts: [
                    {
                      contains: 'Test string',
                      exact: 'Test string',
                      ignoreCase: true,
                      prefix: 'Test string',
                      suffix: 'Test string',
                    },
                  ],
                  mcp: {
                    baseProtocolMethodsOption: 'Test string',
                    methods: [
                      {
                        name: 'Test string',
                        params: [
                          {
                            contains: 'Test string',
                            exact: 'Test string',
                            ignoreCase: true,
                            prefix: 'Test string',
                            suffix: 'Test string',
                          },
                        ],
                      },
                    ],
                  },
                  methods: ['Test string'],
                  paths: [
                    {
                      contains: 'Test string',
                      exact: 'Test string',
                      ignoreCase: true,
                      prefix: 'Test string',
                      suffix: 'Test string',
                    },
                  ],
                  snis: [
                    {
                      contains: 'Test string',
                      exact: 'Test string',
                      ignoreCase: true,
                      prefix: 'Test string',
                      suffix: 'Test string',
                    },
                  ],
                },
              ],
            },
            when: 'Test string',
          },
        ],
        policyProfile: 'Test string',
        target: {
          loadBalancingScheme: 'Test string',
          resources: ['Test string'],
        },
        updateTime: 'Test string',
      },
    );
    /** Deletes a single AuthzPolicy. */
    await gapi.client.networksecurity.projects.locations.authzPolicies.delete({
      name: 'Test string',
      requestId: 'Test string',
    });
    /** Gets details of a single AuthzPolicy. */
    await gapi.client.networksecurity.projects.locations.authzPolicies.get({
      name: 'Test string',
    });
    /** Gets the access control policy for a resource. Returns an empty policy if the resource exists and does not have a policy set. */
    await gapi.client.networksecurity.projects.locations.authzPolicies.getIamPolicy(
      {
        'options.requestedPolicyVersion': 42,
        resource: 'Test string',
      },
    );
    /** Lists AuthzPolicies in a given project and location. */
    await gapi.client.networksecurity.projects.locations.authzPolicies.list({
      filter: 'Test string',
      orderBy: 'Test string',
      pageSize: 42,
      pageToken: 'Test string',
      parent: 'Test string',
    });
    /** Updates the parameters of a single AuthzPolicy. */
    await gapi.client.networksecurity.projects.locations.authzPolicies.patch(
      {
        name: 'Test string',
        requestId: 'Test string',
        updateMask: 'Test string',
      },
      {
        action: 'Test string',
        createTime: 'Test string',
        customProvider: {
          authzExtension: {
            resources: ['Test string'],
          },
          cloudIap: {},
        },
        description: 'Test string',
        httpRules: [
          {
            from: {
              notSources: [
                {
                  ipBlocks: [
                    {
                      length: 42,
                      prefix: 'Test string',
                    },
                  ],
                  principals: [
                    {
                      principal: {
                        contains: 'Test string',
                        exact: 'Test string',
                        ignoreCase: true,
                        prefix: 'Test string',
                        suffix: 'Test string',
                      },
                      principalSelector: 'Test string',
                    },
                  ],
                  resources: [
                    {
                      iamServiceAccount: {
                        contains: 'Test string',
                        exact: 'Test string',
                        ignoreCase: true,
                        prefix: 'Test string',
                        suffix: 'Test string',
                      },
                      tagValueIdSet: {
                        ids: ['Test string'],
                      },
                    },
                  ],
                },
              ],
              sources: [
                {
                  ipBlocks: [
                    {
                      length: 42,
                      prefix: 'Test string',
                    },
                  ],
                  principals: [
                    {
                      principal: {
                        contains: 'Test string',
                        exact: 'Test string',
                        ignoreCase: true,
                        prefix: 'Test string',
                        suffix: 'Test string',
                      },
                      principalSelector: 'Test string',
                    },
                  ],
                  resources: [
                    {
                      iamServiceAccount: {
                        contains: 'Test string',
                        exact: 'Test string',
                        ignoreCase: true,
                        prefix: 'Test string',
                        suffix: 'Test string',
                      },
                      tagValueIdSet: {
                        ids: ['Test string'],
                      },
                    },
                  ],
                },
              ],
            },
            to: {
              notOperations: [
                {
                  headerSet: {
                    headers: [
                      {
                        name: 'Test string',
                        value: {
                          contains: 'Test string',
                          exact: 'Test string',
                          ignoreCase: true,
                          prefix: 'Test string',
                          suffix: 'Test string',
                        },
                      },
                    ],
                  },
                  hosts: [
                    {
                      contains: 'Test string',
                      exact: 'Test string',
                      ignoreCase: true,
                      prefix: 'Test string',
                      suffix: 'Test string',
                    },
                  ],
                  mcp: {
                    baseProtocolMethodsOption: 'Test string',
                    methods: [
                      {
                        name: 'Test string',
                        params: [
                          {
                            contains: 'Test string',
                            exact: 'Test string',
                            ignoreCase: true,
                            prefix: 'Test string',
                            suffix: 'Test string',
                          },
                        ],
                      },
                    ],
                  },
                  methods: ['Test string'],
                  paths: [
                    {
                      contains: 'Test string',
                      exact: 'Test string',
                      ignoreCase: true,
                      prefix: 'Test string',
                      suffix: 'Test string',
                    },
                  ],
                  snis: [
                    {
                      contains: 'Test string',
                      exact: 'Test string',
                      ignoreCase: true,
                      prefix: 'Test string',
                      suffix: 'Test string',
                    },
                  ],
                },
              ],
              operations: [
                {
                  headerSet: {
                    headers: [
                      {
                        name: 'Test string',
                        value: {
                          contains: 'Test string',
                          exact: 'Test string',
                          ignoreCase: true,
                          prefix: 'Test string',
                          suffix: 'Test string',
                        },
                      },
                    ],
                  },
                  hosts: [
                    {
                      contains: 'Test string',
                      exact: 'Test string',
                      ignoreCase: true,
                      prefix: 'Test string',
                      suffix: 'Test string',
                    },
                  ],
                  mcp: {
                    baseProtocolMethodsOption: 'Test string',
                    methods: [
                      {
                        name: 'Test string',
                        params: [
                          {
                            contains: 'Test string',
                            exact: 'Test string',
                            ignoreCase: true,
                            prefix: 'Test string',
                            suffix: 'Test string',
                          },
                        ],
                      },
                    ],
                  },
                  methods: ['Test string'],
                  paths: [
                    {
                      contains: 'Test string',
                      exact: 'Test string',
                      ignoreCase: true,
                      prefix: 'Test string',
                      suffix: 'Test string',
                    },
                  ],
                  snis: [
                    {
                      contains: 'Test string',
                      exact: 'Test string',
                      ignoreCase: true,
                      prefix: 'Test string',
                      suffix: 'Test string',
                    },
                  ],
                },
              ],
            },
            when: 'Test string',
          },
        ],
        labels: {
          A: 'Test string',
        },
        name: 'Test string',
        networkRules: [
          {
            from: {
              notSources: [
                {
                  ipBlocks: [
                    {
                      length: 42,
                      prefix: 'Test string',
                    },
                  ],
                  principals: [
                    {
                      principal: {
                        contains: 'Test string',
                        exact: 'Test string',
                        ignoreCase: true,
                        prefix: 'Test string',
                        suffix: 'Test string',
                      },
                      principalSelector: 'Test string',
                    },
                  ],
                  resources: [
                    {
                      iamServiceAccount: {
                        contains: 'Test string',
                        exact: 'Test string',
                        ignoreCase: true,
                        prefix: 'Test string',
                        suffix: 'Test string',
                      },
                      tagValueIdSet: {
                        ids: ['Test string'],
                      },
                    },
                  ],
                },
              ],
              sources: [
                {
                  ipBlocks: [
                    {
                      length: 42,
                      prefix: 'Test string',
                    },
                  ],
                  principals: [
                    {
                      principal: {
                        contains: 'Test string',
                        exact: 'Test string',
                        ignoreCase: true,
                        prefix: 'Test string',
                        suffix: 'Test string',
                      },
                      principalSelector: 'Test string',
                    },
                  ],
                  resources: [
                    {
                      iamServiceAccount: {
                        contains: 'Test string',
                        exact: 'Test string',
                        ignoreCase: true,
                        prefix: 'Test string',
                        suffix: 'Test string',
                      },
                      tagValueIdSet: {
                        ids: ['Test string'],
                      },
                    },
                  ],
                },
              ],
            },
            to: {
              notOperations: [
                {
                  headerSet: {
                    headers: [
                      {
                        name: 'Test string',
                        value: {
                          contains: 'Test string',
                          exact: 'Test string',
                          ignoreCase: true,
                          prefix: 'Test string',
                          suffix: 'Test string',
                        },
                      },
                    ],
                  },
                  hosts: [
                    {
                      contains: 'Test string',
                      exact: 'Test string',
                      ignoreCase: true,
                      prefix: 'Test string',
                      suffix: 'Test string',
                    },
                  ],
                  mcp: {
                    baseProtocolMethodsOption: 'Test string',
                    methods: [
                      {
                        name: 'Test string',
                        params: [
                          {
                            contains: 'Test string',
                            exact: 'Test string',
                            ignoreCase: true,
                            prefix: 'Test string',
                            suffix: 'Test string',
                          },
                        ],
                      },
                    ],
                  },
                  methods: ['Test string'],
                  paths: [
                    {
                      contains: 'Test string',
                      exact: 'Test string',
                      ignoreCase: true,
                      prefix: 'Test string',
                      suffix: 'Test string',
                    },
                  ],
                  snis: [
                    {
                      contains: 'Test string',
                      exact: 'Test string',
                      ignoreCase: true,
                      prefix: 'Test string',
                      suffix: 'Test string',
                    },
                  ],
                },
              ],
              operations: [
                {
                  headerSet: {
                    headers: [
                      {
                        name: 'Test string',
                        value: {
                          contains: 'Test string',
                          exact: 'Test string',
                          ignoreCase: true,
                          prefix: 'Test string',
                          suffix: 'Test string',
                        },
                      },
                    ],
                  },
                  hosts: [
                    {
                      contains: 'Test string',
                      exact: 'Test string',
                      ignoreCase: true,
                      prefix: 'Test string',
                      suffix: 'Test string',
                    },
                  ],
                  mcp: {
                    baseProtocolMethodsOption: 'Test string',
                    methods: [
                      {
                        name: 'Test string',
                        params: [
                          {
                            contains: 'Test string',
                            exact: 'Test string',
                            ignoreCase: true,
                            prefix: 'Test string',
                            suffix: 'Test string',
                          },
                        ],
                      },
                    ],
                  },
                  methods: ['Test string'],
                  paths: [
                    {
                      contains: 'Test string',
                      exact: 'Test string',
                      ignoreCase: true,
                      prefix: 'Test string',
                      suffix: 'Test string',
                    },
                  ],
                  snis: [
                    {
                      contains: 'Test string',
                      exact: 'Test string',
                      ignoreCase: true,
                      prefix: 'Test string',
                      suffix: 'Test string',
                    },
                  ],
                },
              ],
            },
            when: 'Test string',
          },
        ],
        policyProfile: 'Test string',
        target: {
          loadBalancingScheme: 'Test string',
          resources: ['Test string'],
        },
        updateTime: 'Test string',
      },
    );
    /** Sets the access control policy on the specified resource. Replaces any existing policy. Can return `NOT_FOUND`, `INVALID_ARGUMENT`, and `PERMISSION_DENIED` errors. */
    await gapi.client.networksecurity.projects.locations.authzPolicies.setIamPolicy(
      {
        resource: 'Test string',
      },
      {
        policy: {
          auditConfigs: [
            {
              auditLogConfigs: [
                {
                  exemptedMembers: ['Test string'],
                  logType: 'Test string',
                },
              ],
              service: 'Test string',
            },
          ],
          bindings: [
            {
              condition: {
                description: 'Test string',
                expression: 'Test string',
                location: 'Test string',
                title: 'Test string',
              },
              members: ['Test string'],
              role: 'Test string',
            },
          ],
          etag: 'Test string',
          version: 42,
        },
        updateMask: 'Test string',
      },
    );
    /** Returns permissions that a caller has on the specified resource. If the resource does not exist, this will return an empty set of permissions, not a `NOT_FOUND` error. Note: This operation is designed to be used for building permission-aware UIs and command-line tools, not for authorization checking. This operation may "fail open" without warning. */
    await gapi.client.networksecurity.projects.locations.authzPolicies.testIamPermissions(
      {
        resource: 'Test string',
      },
      {
        permissions: ['Test string'],
      },
    );
    /** Creates a new BackendAuthenticationConfig in a given project and location. */
    await gapi.client.networksecurity.projects.locations.backendAuthenticationConfigs.create(
      {
        backendAuthenticationConfigId: 'Test string',
        parent: 'Test string',
      },
      {
        clientCertificate: 'Test string',
        createTime: 'Test string',
        description: 'Test string',
        etag: 'Test string',
        labels: {
          A: 'Test string',
        },
        name: 'Test string',
        trustConfig: 'Test string',
        updateTime: 'Test string',
        wellKnownRoots: 'Test string',
      },
    );
    /** Deletes a single BackendAuthenticationConfig to BackendAuthenticationConfig. */
    await gapi.client.networksecurity.projects.locations.backendAuthenticationConfigs.delete(
      {
        etag: 'Test string',
        name: 'Test string',
      },
    );
    /** Gets details of a single BackendAuthenticationConfig to BackendAuthenticationConfig. */
    await gapi.client.networksecurity.projects.locations.backendAuthenticationConfigs.get(
      {
        name: 'Test string',
      },
    );
    /** Lists BackendAuthenticationConfigs in a given project and location. */
    await gapi.client.networksecurity.projects.locations.backendAuthenticationConfigs.list(
      {
        pageSize: 42,
        pageToken: 'Test string',
        parent: 'Test string',
      },
    );
    /** Updates the parameters of a single BackendAuthenticationConfig to BackendAuthenticationConfig. */
    await gapi.client.networksecurity.projects.locations.backendAuthenticationConfigs.patch(
      {
        name: 'Test string',
        updateMask: 'Test string',
      },
      {
        clientCertificate: 'Test string',
        createTime: 'Test string',
        description: 'Test string',
        etag: 'Test string',
        labels: {
          A: 'Test string',
        },
        name: 'Test string',
        trustConfig: 'Test string',
        updateTime: 'Test string',
        wellKnownRoots: 'Test string',
      },
    );
    /** Creates a new ClientTlsPolicy in a given project and location. */
    await gapi.client.networksecurity.projects.locations.clientTlsPolicies.create(
      {
        clientTlsPolicyId: 'Test string',
        parent: 'Test string',
      },
      {
        clientCertificate: {
          certificateProviderInstance: {
            pluginInstance: 'Test string',
          },
          grpcEndpoint: {
            targetUri: 'Test string',
          },
        },
        createTime: 'Test string',
        description: 'Test string',
        labels: {
          A: 'Test string',
        },
        name: 'Test string',
        serverValidationCa: [
          {
            certificateProviderInstance: {
              pluginInstance: 'Test string',
            },
            grpcEndpoint: {
              targetUri: 'Test string',
            },
          },
        ],
        sni: 'Test string',
        updateTime: 'Test string',
      },
    );
    /** Deletes a single ClientTlsPolicy. */
    await gapi.client.networksecurity.projects.locations.clientTlsPolicies.delete(
      {
        name: 'Test string',
      },
    );
    /** Gets details of a single ClientTlsPolicy. */
    await gapi.client.networksecurity.projects.locations.clientTlsPolicies.get({
      name: 'Test string',
    });
    /** Gets the access control policy for a resource. Returns an empty policy if the resource exists and does not have a policy set. */
    await gapi.client.networksecurity.projects.locations.clientTlsPolicies.getIamPolicy(
      {
        'options.requestedPolicyVersion': 42,
        resource: 'Test string',
      },
    );
    /** Lists ClientTlsPolicies in a given project and location. */
    await gapi.client.networksecurity.projects.locations.clientTlsPolicies.list(
      {
        pageSize: 42,
        pageToken: 'Test string',
        parent: 'Test string',
      },
    );
    /** Updates the parameters of a single ClientTlsPolicy. */
    await gapi.client.networksecurity.projects.locations.clientTlsPolicies.patch(
      {
        name: 'Test string',
        updateMask: 'Test string',
      },
      {
        clientCertificate: {
          certificateProviderInstance: {
            pluginInstance: 'Test string',
          },
          grpcEndpoint: {
            targetUri: 'Test string',
          },
        },
        createTime: 'Test string',
        description: 'Test string',
        labels: {
          A: 'Test string',
        },
        name: 'Test string',
        serverValidationCa: [
          {
            certificateProviderInstance: {
              pluginInstance: 'Test string',
            },
            grpcEndpoint: {
              targetUri: 'Test string',
            },
          },
        ],
        sni: 'Test string',
        updateTime: 'Test string',
      },
    );
    /** Sets the access control policy on the specified resource. Replaces any existing policy. Can return `NOT_FOUND`, `INVALID_ARGUMENT`, and `PERMISSION_DENIED` errors. */
    await gapi.client.networksecurity.projects.locations.clientTlsPolicies.setIamPolicy(
      {
        resource: 'Test string',
      },
      {
        policy: {
          auditConfigs: [
            {
              auditLogConfigs: [
                {
                  exemptedMembers: ['Test string'],
                  logType: 'Test string',
                },
              ],
              service: 'Test string',
            },
          ],
          bindings: [
            {
              condition: {
                description: 'Test string',
                expression: 'Test string',
                location: 'Test string',
                title: 'Test string',
              },
              members: ['Test string'],
              role: 'Test string',
            },
          ],
          etag: 'Test string',
          version: 42,
        },
        updateMask: 'Test string',
      },
    );
    /** Returns permissions that a caller has on the specified resource. If the resource does not exist, this will return an empty set of permissions, not a `NOT_FOUND` error. Note: This operation is designed to be used for building permission-aware UIs and command-line tools, not for authorization checking. This operation may "fail open" without warning. */
    await gapi.client.networksecurity.projects.locations.clientTlsPolicies.testIamPermissions(
      {
        resource: 'Test string',
      },
      {
        permissions: ['Test string'],
      },
    );
    /** Creates a new DnsThreatDetector in a given project and location. */
    await gapi.client.networksecurity.projects.locations.dnsThreatDetectors.create(
      {
        dnsThreatDetectorId: 'Test string',
        parent: 'Test string',
      },
      {
        createTime: 'Test string',
        excludedNetworks: ['Test string'],
        labels: {
          A: 'Test string',
        },
        name: 'Test string',
        provider: 'Test string',
        updateTime: 'Test string',
      },
    );
    /** Deletes a single DnsThreatDetector. */
    await gapi.client.networksecurity.projects.locations.dnsThreatDetectors.delete(
      {
        name: 'Test string',
      },
    );
    /** Gets the details of a single DnsThreatDetector. */
    await gapi.client.networksecurity.projects.locations.dnsThreatDetectors.get(
      {
        name: 'Test string',
      },
    );
    /** Lists DnsThreatDetectors in a given project and location. */
    await gapi.client.networksecurity.projects.locations.dnsThreatDetectors.list(
      {
        pageSize: 42,
        pageToken: 'Test string',
        parent: 'Test string',
      },
    );
    /** Updates a single DnsThreatDetector. */
    await gapi.client.networksecurity.projects.locations.dnsThreatDetectors.patch(
      {
        name: 'Test string',
        updateMask: 'Test string',
      },
      {
        createTime: 'Test string',
        excludedNetworks: ['Test string'],
        labels: {
          A: 'Test string',
        },
        name: 'Test string',
        provider: 'Test string',
        updateTime: 'Test string',
      },
    );
    /** Creates a new FirewallEndpointAssociation in a given project and location. */
    await gapi.client.networksecurity.projects.locations.firewallEndpointAssociations.create(
      {
        firewallEndpointAssociationId: 'Test string',
        parent: 'Test string',
        requestId: 'Test string',
      },
      {
        createTime: 'Test string',
        disabled: true,
        firewallEndpoint: 'Test string',
        labels: {
          A: 'Test string',
        },
        name: 'Test string',
        network: 'Test string',
        reconciling: true,
        state: 'Test string',
        tlsInspectionPolicy: 'Test string',
        updateTime: 'Test string',
      },
    );
    /** Deletes a single FirewallEndpointAssociation. */
    await gapi.client.networksecurity.projects.locations.firewallEndpointAssociations.delete(
      {
        name: 'Test string',
        requestId: 'Test string',
      },
    );
    /** Gets details of a single FirewallEndpointAssociation. */
    await gapi.client.networksecurity.projects.locations.firewallEndpointAssociations.get(
      {
        name: 'Test string',
      },
    );
    /** Lists Associations in a given project and location. */
    await gapi.client.networksecurity.projects.locations.firewallEndpointAssociations.list(
      {
        filter: 'Test string',
        orderBy: 'Test string',
        pageSize: 42,
        pageToken: 'Test string',
        parent: 'Test string',
      },
    );
    /** Update a single FirewallEndpointAssociation. */
    await gapi.client.networksecurity.projects.locations.firewallEndpointAssociations.patch(
      {
        name: 'Test string',
        requestId: 'Test string',
        updateMask: 'Test string',
      },
      {
        createTime: 'Test string',
        disabled: true,
        firewallEndpoint: 'Test string',
        labels: {
          A: 'Test string',
        },
        name: 'Test string',
        network: 'Test string',
        reconciling: true,
        state: 'Test string',
        tlsInspectionPolicy: 'Test string',
        updateTime: 'Test string',
      },
    );
    /** Creates a new FirewallEndpoint in a given project and location. */
    await gapi.client.networksecurity.projects.locations.firewallEndpoints.create(
      {
        firewallEndpointId: 'Test string',
        parent: 'Test string',
        requestId: 'Test string',
        validateOnly: true,
      },
      {
        associatedNetworks: ['Test string'],
        associations: [
          {
            name: 'Test string',
            network: 'Test string',
          },
        ],
        billingProjectId: 'Test string',
        createTime: 'Test string',
        description: 'Test string',
        endpointSettings: {
          contentCloudRegion: 'Test string',
          httpPartialResponseBlocked: true,
          jumboFramesEnabled: true,
        },
        explicitPrivateServiceConnectAttachment: 'Test string',
        labels: {
          A: 'Test string',
        },
        name: 'Test string',
        reconciling: true,
        satisfiesPzi: true,
        satisfiesPzs: true,
        state: 'Test string',
        updateTime: 'Test string',
        wildfireSettings: {
          enabled: true,
          wildfireInlineCloudAnalysisSettings: {
            maxAnalysisDuration: 'Test string',
            submissionTimeoutLoggingDisabled: true,
            timeoutAction: 'Test string',
          },
          wildfireRealtimeLookupDuration: 'Test string',
          wildfireRealtimeLookupTimeoutAction: 'Test string',
          wildfireRegion: 'Test string',
        },
      },
    );
    /** Deletes a single project Endpoint. */
    await gapi.client.networksecurity.projects.locations.firewallEndpoints.delete(
      {
        name: 'Test string',
        requestId: 'Test string',
      },
    );
    /** Gets details of a single project Endpoint. */
    await gapi.client.networksecurity.projects.locations.firewallEndpoints.get({
      name: 'Test string',
    });
    /** Lists FirewallEndpoints in a given project and location. */
    await gapi.client.networksecurity.projects.locations.firewallEndpoints.list(
      {
        filter: 'Test string',
        orderBy: 'Test string',
        pageSize: 42,
        pageToken: 'Test string',
        parent: 'Test string',
      },
    );
    /** Update a single project Endpoint. */
    await gapi.client.networksecurity.projects.locations.firewallEndpoints.patch(
      {
        name: 'Test string',
        requestId: 'Test string',
        updateMask: 'Test string',
      },
      {
        associatedNetworks: ['Test string'],
        associations: [
          {
            name: 'Test string',
            network: 'Test string',
          },
        ],
        billingProjectId: 'Test string',
        createTime: 'Test string',
        description: 'Test string',
        endpointSettings: {
          contentCloudRegion: 'Test string',
          httpPartialResponseBlocked: true,
          jumboFramesEnabled: true,
        },
        explicitPrivateServiceConnectAttachment: 'Test string',
        labels: {
          A: 'Test string',
        },
        name: 'Test string',
        reconciling: true,
        satisfiesPzi: true,
        satisfiesPzs: true,
        state: 'Test string',
        updateTime: 'Test string',
        wildfireSettings: {
          enabled: true,
          wildfireInlineCloudAnalysisSettings: {
            maxAnalysisDuration: 'Test string',
            submissionTimeoutLoggingDisabled: true,
            timeoutAction: 'Test string',
          },
          wildfireRealtimeLookupDuration: 'Test string',
          wildfireRealtimeLookupTimeoutAction: 'Test string',
          wildfireRegion: 'Test string',
        },
      },
    );
    /** Create WildfireVerdictChangeRequest in a given Firewall Endpoint in a project and location. */
    await gapi.client.networksecurity.projects.locations.firewallEndpoints.wildfireVerdictChangeRequests.create(
      {
        parent: 'Test string',
      },
      {
        comment: 'Test string',
        createTime: 'Test string',
        fileName: 'Test string',
        fileType: 'Test string',
        finalVerdict: 'Test string',
        name: 'Test string',
        newVerdict: 'Test string',
        oldVerdict: 'Test string',
        resolutionTime: 'Test string',
        sha256: 'Test string',
        sourceRegion: 'Test string',
        state: 'Test string',
        updateTime: 'Test string',
        wildfireVerdictChangeRequestId: 'Test string',
      },
    );
    /** Get WildfireVerdictChangeRequest in a given Firewall Endpoint in a project and location. */
    await gapi.client.networksecurity.projects.locations.firewallEndpoints.wildfireVerdictChangeRequests.get(
      {
        name: 'Test string',
      },
    );
    /** Lists WildfireVerdictChangeRequests in a given Firewall Endpoint in a project and location. */
    await gapi.client.networksecurity.projects.locations.firewallEndpoints.wildfireVerdictChangeRequests.list(
      {
        filter: 'Test string',
        pageSize: 42,
        pageToken: 'Test string',
        parent: 'Test string',
      },
    );
    /** Creates a new GatewaySecurityPolicy in a given project and location. */
    await gapi.client.networksecurity.projects.locations.gatewaySecurityPolicies.create(
      {
        gatewaySecurityPolicyId: 'Test string',
        parent: 'Test string',
      },
      {
        createTime: 'Test string',
        description: 'Test string',
        name: 'Test string',
        tlsInspectionPolicy: 'Test string',
        updateTime: 'Test string',
      },
    );
    /** Deletes a single GatewaySecurityPolicy. */
    await gapi.client.networksecurity.projects.locations.gatewaySecurityPolicies.delete(
      {
        name: 'Test string',
      },
    );
    /** Gets details of a single GatewaySecurityPolicy. */
    await gapi.client.networksecurity.projects.locations.gatewaySecurityPolicies.get(
      {
        name: 'Test string',
      },
    );
    /** Lists GatewaySecurityPolicies in a given project and location. */
    await gapi.client.networksecurity.projects.locations.gatewaySecurityPolicies.list(
      {
        pageSize: 42,
        pageToken: 'Test string',
        parent: 'Test string',
      },
    );
    /** Updates the parameters of a single GatewaySecurityPolicy. */
    await gapi.client.networksecurity.projects.locations.gatewaySecurityPolicies.patch(
      {
        name: 'Test string',
        updateMask: 'Test string',
      },
      {
        createTime: 'Test string',
        description: 'Test string',
        name: 'Test string',
        tlsInspectionPolicy: 'Test string',
        updateTime: 'Test string',
      },
    );
    /** Creates a new GatewaySecurityPolicy in a given project and location. */
    await gapi.client.networksecurity.projects.locations.gatewaySecurityPolicies.rules.create(
      {
        gatewaySecurityPolicyRuleId: 'Test string',
        parent: 'Test string',
      },
      {
        applicationMatcher: 'Test string',
        basicProfile: 'Test string',
        createTime: 'Test string',
        description: 'Test string',
        enabled: true,
        name: 'Test string',
        priority: 42,
        sessionMatcher: 'Test string',
        tlsInspectionEnabled: true,
        updateTime: 'Test string',
      },
    );
    /** Deletes a single GatewaySecurityPolicyRule. */
    await gapi.client.networksecurity.projects.locations.gatewaySecurityPolicies.rules.delete(
      {
        name: 'Test string',
      },
    );
    /** Gets details of a single GatewaySecurityPolicyRule. */
    await gapi.client.networksecurity.projects.locations.gatewaySecurityPolicies.rules.get(
      {
        name: 'Test string',
      },
    );
    /** Lists GatewaySecurityPolicyRules in a given project and location. */
    await gapi.client.networksecurity.projects.locations.gatewaySecurityPolicies.rules.list(
      {
        pageSize: 42,
        pageToken: 'Test string',
        parent: 'Test string',
      },
    );
    /** Updates the parameters of a single GatewaySecurityPolicyRule. */
    await gapi.client.networksecurity.projects.locations.gatewaySecurityPolicies.rules.patch(
      {
        name: 'Test string',
        updateMask: 'Test string',
      },
      {
        applicationMatcher: 'Test string',
        basicProfile: 'Test string',
        createTime: 'Test string',
        description: 'Test string',
        enabled: true,
        name: 'Test string',
        priority: 42,
        sessionMatcher: 'Test string',
        tlsInspectionEnabled: true,
        updateTime: 'Test string',
      },
    );
    /** Creates a deployment group in a given project and location. See https://google.aip.dev/133. */
    await gapi.client.networksecurity.projects.locations.interceptDeploymentGroups.create(
      {
        interceptDeploymentGroupId: 'Test string',
        parent: 'Test string',
        requestId: 'Test string',
      },
      {
        connectedEndpointGroups: [
          {
            name: 'Test string',
          },
        ],
        createTime: 'Test string',
        description: 'Test string',
        labels: {
          A: 'Test string',
        },
        locations: [
          {
            location: 'Test string',
            state: 'Test string',
          },
        ],
        name: 'Test string',
        nestedDeployments: [
          {
            name: 'Test string',
            state: 'Test string',
          },
        ],
        network: 'Test string',
        reconciling: true,
        state: 'Test string',
        updateTime: 'Test string',
      },
    );
    /** Deletes a deployment group. See https://google.aip.dev/135. */
    await gapi.client.networksecurity.projects.locations.interceptDeploymentGroups.delete(
      {
        name: 'Test string',
        requestId: 'Test string',
      },
    );
    /** Gets a specific deployment group. See https://google.aip.dev/131. */
    await gapi.client.networksecurity.projects.locations.interceptDeploymentGroups.get(
      {
        name: 'Test string',
      },
    );
    /** Lists deployment groups in a given project and location. See https://google.aip.dev/132. */
    await gapi.client.networksecurity.projects.locations.interceptDeploymentGroups.list(
      {
        filter: 'Test string',
        orderBy: 'Test string',
        pageSize: 42,
        pageToken: 'Test string',
        parent: 'Test string',
      },
    );
    /** Updates a deployment group. See https://google.aip.dev/134. */
    await gapi.client.networksecurity.projects.locations.interceptDeploymentGroups.patch(
      {
        name: 'Test string',
        requestId: 'Test string',
        updateMask: 'Test string',
      },
      {
        connectedEndpointGroups: [
          {
            name: 'Test string',
          },
        ],
        createTime: 'Test string',
        description: 'Test string',
        labels: {
          A: 'Test string',
        },
        locations: [
          {
            location: 'Test string',
            state: 'Test string',
          },
        ],
        name: 'Test string',
        nestedDeployments: [
          {
            name: 'Test string',
            state: 'Test string',
          },
        ],
        network: 'Test string',
        reconciling: true,
        state: 'Test string',
        updateTime: 'Test string',
      },
    );
    /** Creates a deployment in a given project and location. See https://google.aip.dev/133. */
    await gapi.client.networksecurity.projects.locations.interceptDeployments.create(
      {
        interceptDeploymentId: 'Test string',
        parent: 'Test string',
        requestId: 'Test string',
      },
      {
        createTime: 'Test string',
        description: 'Test string',
        forwardingRule: 'Test string',
        interceptDeploymentGroup: 'Test string',
        labels: {
          A: 'Test string',
        },
        name: 'Test string',
        reconciling: true,
        state: 'Test string',
        updateTime: 'Test string',
      },
    );
    /** Deletes a deployment. See https://google.aip.dev/135. */
    await gapi.client.networksecurity.projects.locations.interceptDeployments.delete(
      {
        name: 'Test string',
        requestId: 'Test string',
      },
    );
    /** Gets a specific deployment. See https://google.aip.dev/131. */
    await gapi.client.networksecurity.projects.locations.interceptDeployments.get(
      {
        name: 'Test string',
      },
    );
    /** Lists deployments in a given project and location. See https://google.aip.dev/132. */
    await gapi.client.networksecurity.projects.locations.interceptDeployments.list(
      {
        filter: 'Test string',
        orderBy: 'Test string',
        pageSize: 42,
        pageToken: 'Test string',
        parent: 'Test string',
      },
    );
    /** Updates a deployment. See https://google.aip.dev/134. */
    await gapi.client.networksecurity.projects.locations.interceptDeployments.patch(
      {
        name: 'Test string',
        requestId: 'Test string',
        updateMask: 'Test string',
      },
      {
        createTime: 'Test string',
        description: 'Test string',
        forwardingRule: 'Test string',
        interceptDeploymentGroup: 'Test string',
        labels: {
          A: 'Test string',
        },
        name: 'Test string',
        reconciling: true,
        state: 'Test string',
        updateTime: 'Test string',
      },
    );
    /** Creates an association in a given project and location. See https://google.aip.dev/133. */
    await gapi.client.networksecurity.projects.locations.interceptEndpointGroupAssociations.create(
      {
        interceptEndpointGroupAssociationId: 'Test string',
        parent: 'Test string',
        requestId: 'Test string',
      },
      {
        createTime: 'Test string',
        interceptEndpointGroup: 'Test string',
        labels: {
          A: 'Test string',
        },
        locations: [
          {
            location: 'Test string',
            state: 'Test string',
          },
        ],
        locationsDetails: [
          {
            location: 'Test string',
            state: 'Test string',
          },
        ],
        name: 'Test string',
        network: 'Test string',
        networkCookie: 42,
        reconciling: true,
        state: 'Test string',
        updateTime: 'Test string',
      },
    );
    /** Deletes an association. See https://google.aip.dev/135. */
    await gapi.client.networksecurity.projects.locations.interceptEndpointGroupAssociations.delete(
      {
        name: 'Test string',
        requestId: 'Test string',
      },
    );
    /** Gets a specific association. See https://google.aip.dev/131. */
    await gapi.client.networksecurity.projects.locations.interceptEndpointGroupAssociations.get(
      {
        name: 'Test string',
      },
    );
    /** Lists associations in a given project and location. See https://google.aip.dev/132. */
    await gapi.client.networksecurity.projects.locations.interceptEndpointGroupAssociations.list(
      {
        filter: 'Test string',
        orderBy: 'Test string',
        pageSize: 42,
        pageToken: 'Test string',
        parent: 'Test string',
      },
    );
    /** Updates an association. See https://google.aip.dev/134. */
    await gapi.client.networksecurity.projects.locations.interceptEndpointGroupAssociations.patch(
      {
        name: 'Test string',
        requestId: 'Test string',
        updateMask: 'Test string',
      },
      {
        createTime: 'Test string',
        interceptEndpointGroup: 'Test string',
        labels: {
          A: 'Test string',
        },
        locations: [
          {
            location: 'Test string',
            state: 'Test string',
          },
        ],
        locationsDetails: [
          {
            location: 'Test string',
            state: 'Test string',
          },
        ],
        name: 'Test string',
        network: 'Test string',
        networkCookie: 42,
        reconciling: true,
        state: 'Test string',
        updateTime: 'Test string',
      },
    );
    /** Creates an endpoint group in a given project and location. See https://google.aip.dev/133. */
    await gapi.client.networksecurity.projects.locations.interceptEndpointGroups.create(
      {
        interceptEndpointGroupId: 'Test string',
        parent: 'Test string',
        requestId: 'Test string',
      },
      {
        associations: [
          {
            name: 'Test string',
            network: 'Test string',
            state: 'Test string',
          },
        ],
        connectedDeploymentGroup: {
          locations: [
            {
              location: 'Test string',
              state: 'Test string',
            },
          ],
          name: 'Test string',
        },
        createTime: 'Test string',
        description: 'Test string',
        interceptDeploymentGroup: 'Test string',
        labels: {
          A: 'Test string',
        },
        name: 'Test string',
        reconciling: true,
        state: 'Test string',
        updateTime: 'Test string',
      },
    );
    /** Deletes an endpoint group. See https://google.aip.dev/135. */
    await gapi.client.networksecurity.projects.locations.interceptEndpointGroups.delete(
      {
        name: 'Test string',
        requestId: 'Test string',
      },
    );
    /** Gets a specific endpoint group. See https://google.aip.dev/131. */
    await gapi.client.networksecurity.projects.locations.interceptEndpointGroups.get(
      {
        name: 'Test string',
      },
    );
    /** Lists endpoint groups in a given project and location. See https://google.aip.dev/132. */
    await gapi.client.networksecurity.projects.locations.interceptEndpointGroups.list(
      {
        filter: 'Test string',
        orderBy: 'Test string',
        pageSize: 42,
        pageToken: 'Test string',
        parent: 'Test string',
      },
    );
    /** Updates an endpoint group. See https://google.aip.dev/134. */
    await gapi.client.networksecurity.projects.locations.interceptEndpointGroups.patch(
      {
        name: 'Test string',
        requestId: 'Test string',
        updateMask: 'Test string',
      },
      {
        associations: [
          {
            name: 'Test string',
            network: 'Test string',
            state: 'Test string',
          },
        ],
        connectedDeploymentGroup: {
          locations: [
            {
              location: 'Test string',
              state: 'Test string',
            },
          ],
          name: 'Test string',
        },
        createTime: 'Test string',
        description: 'Test string',
        interceptDeploymentGroup: 'Test string',
        labels: {
          A: 'Test string',
        },
        name: 'Test string',
        reconciling: true,
        state: 'Test string',
        updateTime: 'Test string',
      },
    );
    /** Creates a deployment group in a given project and location. See https://google.aip.dev/133. */
    await gapi.client.networksecurity.projects.locations.mirroringDeploymentGroups.create(
      {
        mirroringDeploymentGroupId: 'Test string',
        parent: 'Test string',
        requestId: 'Test string',
      },
      {
        connectedEndpointGroups: [
          {
            name: 'Test string',
          },
        ],
        createTime: 'Test string',
        description: 'Test string',
        labels: {
          A: 'Test string',
        },
        locations: [
          {
            location: 'Test string',
            state: 'Test string',
          },
        ],
        name: 'Test string',
        nestedDeployments: [
          {
            name: 'Test string',
            state: 'Test string',
          },
        ],
        network: 'Test string',
        reconciling: true,
        state: 'Test string',
        updateTime: 'Test string',
      },
    );
    /** Deletes a deployment group. See https://google.aip.dev/135. */
    await gapi.client.networksecurity.projects.locations.mirroringDeploymentGroups.delete(
      {
        name: 'Test string',
        requestId: 'Test string',
      },
    );
    /** Gets a specific deployment group. See https://google.aip.dev/131. */
    await gapi.client.networksecurity.projects.locations.mirroringDeploymentGroups.get(
      {
        name: 'Test string',
      },
    );
    /** Lists deployment groups in a given project and location. See https://google.aip.dev/132. */
    await gapi.client.networksecurity.projects.locations.mirroringDeploymentGroups.list(
      {
        filter: 'Test string',
        orderBy: 'Test string',
        pageSize: 42,
        pageToken: 'Test string',
        parent: 'Test string',
      },
    );
    /** Updates a deployment group. See https://google.aip.dev/134. */
    await gapi.client.networksecurity.projects.locations.mirroringDeploymentGroups.patch(
      {
        name: 'Test string',
        requestId: 'Test string',
        updateMask: 'Test string',
      },
      {
        connectedEndpointGroups: [
          {
            name: 'Test string',
          },
        ],
        createTime: 'Test string',
        description: 'Test string',
        labels: {
          A: 'Test string',
        },
        locations: [
          {
            location: 'Test string',
            state: 'Test string',
          },
        ],
        name: 'Test string',
        nestedDeployments: [
          {
            name: 'Test string',
            state: 'Test string',
          },
        ],
        network: 'Test string',
        reconciling: true,
        state: 'Test string',
        updateTime: 'Test string',
      },
    );
    /** Creates a deployment in a given project and location. See https://google.aip.dev/133. */
    await gapi.client.networksecurity.projects.locations.mirroringDeployments.create(
      {
        mirroringDeploymentId: 'Test string',
        parent: 'Test string',
        requestId: 'Test string',
      },
      {
        createTime: 'Test string',
        description: 'Test string',
        forwardingRule: 'Test string',
        labels: {
          A: 'Test string',
        },
        mirroringDeploymentGroup: 'Test string',
        name: 'Test string',
        reconciling: true,
        state: 'Test string',
        updateTime: 'Test string',
      },
    );
    /** Deletes a deployment. See https://google.aip.dev/135. */
    await gapi.client.networksecurity.projects.locations.mirroringDeployments.delete(
      {
        name: 'Test string',
        requestId: 'Test string',
      },
    );
    /** Gets a specific deployment. See https://google.aip.dev/131. */
    await gapi.client.networksecurity.projects.locations.mirroringDeployments.get(
      {
        name: 'Test string',
      },
    );
    /** Lists deployments in a given project and location. See https://google.aip.dev/132. */
    await gapi.client.networksecurity.projects.locations.mirroringDeployments.list(
      {
        filter: 'Test string',
        orderBy: 'Test string',
        pageSize: 42,
        pageToken: 'Test string',
        parent: 'Test string',
      },
    );
    /** Updates a deployment. See https://google.aip.dev/134. */
    await gapi.client.networksecurity.projects.locations.mirroringDeployments.patch(
      {
        name: 'Test string',
        requestId: 'Test string',
        updateMask: 'Test string',
      },
      {
        createTime: 'Test string',
        description: 'Test string',
        forwardingRule: 'Test string',
        labels: {
          A: 'Test string',
        },
        mirroringDeploymentGroup: 'Test string',
        name: 'Test string',
        reconciling: true,
        state: 'Test string',
        updateTime: 'Test string',
      },
    );
    /** Creates an association in a given project and location. See https://google.aip.dev/133. */
    await gapi.client.networksecurity.projects.locations.mirroringEndpointGroupAssociations.create(
      {
        mirroringEndpointGroupAssociationId: 'Test string',
        parent: 'Test string',
        requestId: 'Test string',
      },
      {
        createTime: 'Test string',
        labels: {
          A: 'Test string',
        },
        locations: [
          {
            location: 'Test string',
            state: 'Test string',
          },
        ],
        locationsDetails: [
          {
            location: 'Test string',
            state: 'Test string',
          },
        ],
        mirroringEndpointGroup: 'Test string',
        name: 'Test string',
        network: 'Test string',
        networkCookie: 42,
        reconciling: true,
        state: 'Test string',
        updateTime: 'Test string',
      },
    );
    /** Deletes an association. See https://google.aip.dev/135. */
    await gapi.client.networksecurity.projects.locations.mirroringEndpointGroupAssociations.delete(
      {
        name: 'Test string',
        requestId: 'Test string',
      },
    );
    /** Gets a specific association. See https://google.aip.dev/131. */
    await gapi.client.networksecurity.projects.locations.mirroringEndpointGroupAssociations.get(
      {
        name: 'Test string',
      },
    );
    /** Lists associations in a given project and location. See https://google.aip.dev/132. */
    await gapi.client.networksecurity.projects.locations.mirroringEndpointGroupAssociations.list(
      {
        filter: 'Test string',
        orderBy: 'Test string',
        pageSize: 42,
        pageToken: 'Test string',
        parent: 'Test string',
      },
    );
    /** Updates an association. See https://google.aip.dev/134. */
    await gapi.client.networksecurity.projects.locations.mirroringEndpointGroupAssociations.patch(
      {
        name: 'Test string',
        requestId: 'Test string',
        updateMask: 'Test string',
      },
      {
        createTime: 'Test string',
        labels: {
          A: 'Test string',
        },
        locations: [
          {
            location: 'Test string',
            state: 'Test string',
          },
        ],
        locationsDetails: [
          {
            location: 'Test string',
            state: 'Test string',
          },
        ],
        mirroringEndpointGroup: 'Test string',
        name: 'Test string',
        network: 'Test string',
        networkCookie: 42,
        reconciling: true,
        state: 'Test string',
        updateTime: 'Test string',
      },
    );
    /** Creates an endpoint group in a given project and location. See https://google.aip.dev/133. */
    await gapi.client.networksecurity.projects.locations.mirroringEndpointGroups.create(
      {
        mirroringEndpointGroupId: 'Test string',
        parent: 'Test string',
        requestId: 'Test string',
      },
      {
        associations: [
          {
            name: 'Test string',
            network: 'Test string',
            state: 'Test string',
          },
        ],
        connectedDeploymentGroups: [
          {
            locations: [
              {
                location: 'Test string',
                state: 'Test string',
              },
            ],
            name: 'Test string',
          },
        ],
        createTime: 'Test string',
        description: 'Test string',
        labels: {
          A: 'Test string',
        },
        mirroringDeploymentGroup: 'Test string',
        name: 'Test string',
        reconciling: true,
        state: 'Test string',
        type: 'Test string',
        updateTime: 'Test string',
      },
    );
    /** Deletes an endpoint group. See https://google.aip.dev/135. */
    await gapi.client.networksecurity.projects.locations.mirroringEndpointGroups.delete(
      {
        name: 'Test string',
        requestId: 'Test string',
      },
    );
    /** Gets a specific endpoint group. See https://google.aip.dev/131. */
    await gapi.client.networksecurity.projects.locations.mirroringEndpointGroups.get(
      {
        name: 'Test string',
      },
    );
    /** Lists endpoint groups in a given project and location. See https://google.aip.dev/132. */
    await gapi.client.networksecurity.projects.locations.mirroringEndpointGroups.list(
      {
        filter: 'Test string',
        orderBy: 'Test string',
        pageSize: 42,
        pageToken: 'Test string',
        parent: 'Test string',
      },
    );
    /** Updates an endpoint group. See https://google.aip.dev/134. */
    await gapi.client.networksecurity.projects.locations.mirroringEndpointGroups.patch(
      {
        name: 'Test string',
        requestId: 'Test string',
        updateMask: 'Test string',
      },
      {
        associations: [
          {
            name: 'Test string',
            network: 'Test string',
            state: 'Test string',
          },
        ],
        connectedDeploymentGroups: [
          {
            locations: [
              {
                location: 'Test string',
                state: 'Test string',
              },
            ],
            name: 'Test string',
          },
        ],
        createTime: 'Test string',
        description: 'Test string',
        labels: {
          A: 'Test string',
        },
        mirroringDeploymentGroup: 'Test string',
        name: 'Test string',
        reconciling: true,
        state: 'Test string',
        type: 'Test string',
        updateTime: 'Test string',
      },
    );
    /** Starts asynchronous cancellation on a long-running operation. The server makes a best effort to cancel the operation, but success is not guaranteed. If the server doesn't support this method, it returns `google.rpc.Code.UNIMPLEMENTED`. Clients can use Operations.GetOperation or other methods to check whether the cancellation succeeded or whether the operation completed despite cancellation. On successful cancellation, the operation is not deleted; instead, it becomes an operation with an Operation.error value with a google.rpc.Status.code of `1`, corresponding to `Code.CANCELLED`. */
    await gapi.client.networksecurity.projects.locations.operations.cancel(
      {
        name: 'Test string',
      },
      {},
    );
    /** Deletes a long-running operation. This method indicates that the client is no longer interested in the operation result. It does not cancel the operation. If the server doesn't support this method, it returns `google.rpc.Code.UNIMPLEMENTED`. */
    await gapi.client.networksecurity.projects.locations.operations.delete({
      name: 'Test string',
    });
    /** Gets the latest state of a long-running operation. Clients can use this method to poll the operation result at intervals as recommended by the API service. */
    await gapi.client.networksecurity.projects.locations.operations.get({
      name: 'Test string',
    });
    /** Lists operations that match the specified filter in the request. If the server doesn't support this method, it returns `UNIMPLEMENTED`. */
    await gapi.client.networksecurity.projects.locations.operations.list({
      filter: 'Test string',
      name: 'Test string',
      pageSize: 42,
      pageToken: 'Test string',
      returnPartialSuccess: true,
    });
    /** Creates a new SACAttachment in a given project and location. */
    await gapi.client.networksecurity.projects.locations.sacAttachments.create(
      {
        parent: 'Test string',
        requestId: 'Test string',
        sacAttachmentId: 'Test string',
      },
      {
        country: 'Test string',
        createTime: 'Test string',
        labels: {
          A: 'Test string',
        },
        name: 'Test string',
        nccGateway: 'Test string',
        sacRealm: 'Test string',
        state: 'Test string',
        symantecOptions: {
          symantecLocationName: 'Test string',
          symantecSite: 'Test string',
        },
        timeZone: 'Test string',
        updateTime: 'Test string',
      },
    );
    /** Deletes the specified attachment. */
    await gapi.client.networksecurity.projects.locations.sacAttachments.delete({
      ignorePartnerDeletionErrors: true,
      name: 'Test string',
      requestId: 'Test string',
    });
    /** Returns the specified attachment. */
    await gapi.client.networksecurity.projects.locations.sacAttachments.get({
      name: 'Test string',
    });
    /** Lists SACAttachments in a given project and location. */
    await gapi.client.networksecurity.projects.locations.sacAttachments.list({
      filter: 'Test string',
      orderBy: 'Test string',
      pageSize: 42,
      pageToken: 'Test string',
      parent: 'Test string',
    });
    /** Creates a new SACRealm in a given project. */
    await gapi.client.networksecurity.projects.locations.sacRealms.create(
      {
        parent: 'Test string',
        requestId: 'Test string',
        sacRealmId: 'Test string',
      },
      {
        createTime: 'Test string',
        labels: {
          A: 'Test string',
        },
        name: 'Test string',
        pairingKey: {
          expireTime: 'Test string',
          key: 'Test string',
        },
        securityService: 'Test string',
        state: 'Test string',
        symantecOptions: {
          availableSymantecSites: ['Test string'],
          secretPath: 'Test string',
          symantecConnectionState: 'Test string',
        },
        updateTime: 'Test string',
      },
    );
    /** Deletes the specified realm. */
    await gapi.client.networksecurity.projects.locations.sacRealms.delete({
      name: 'Test string',
      requestId: 'Test string',
    });
    /** Returns the specified realm. */
    await gapi.client.networksecurity.projects.locations.sacRealms.get({
      name: 'Test string',
    });
    /** Lists SACRealms in a given project. */
    await gapi.client.networksecurity.projects.locations.sacRealms.list({
      filter: 'Test string',
      orderBy: 'Test string',
      pageSize: 42,
      pageToken: 'Test string',
      parent: 'Test string',
    });
    /** Creates a new SecurityProfileGroup in a given project and location. */
    await gapi.client.networksecurity.projects.locations.securityProfileGroups.create(
      {
        parent: 'Test string',
        securityProfileGroupId: 'Test string',
      },
      {
        createTime: 'Test string',
        customInterceptProfile: 'Test string',
        customMirroringProfile: 'Test string',
        dataPathId: 'Test string',
        description: 'Test string',
        etag: 'Test string',
        labels: {
          A: 'Test string',
        },
        name: 'Test string',
        threatPreventionProfile: 'Test string',
        updateTime: 'Test string',
        urlFilteringProfile: 'Test string',
        wildfireAnalysisProfile: 'Test string',
      },
    );
    /** Deletes a single SecurityProfileGroup. */
    await gapi.client.networksecurity.projects.locations.securityProfileGroups.delete(
      {
        etag: 'Test string',
        name: 'Test string',
      },
    );
    /** Gets details of a single SecurityProfileGroup. */
    await gapi.client.networksecurity.projects.locations.securityProfileGroups.get(
      {
        name: 'Test string',
      },
    );
    /** Lists SecurityProfileGroups in a given project and location. */
    await gapi.client.networksecurity.projects.locations.securityProfileGroups.list(
      {
        pageSize: 42,
        pageToken: 'Test string',
        parent: 'Test string',
      },
    );
    /** Updates the parameters of a single SecurityProfileGroup. */
    await gapi.client.networksecurity.projects.locations.securityProfileGroups.patch(
      {
        name: 'Test string',
        updateMask: 'Test string',
      },
      {
        createTime: 'Test string',
        customInterceptProfile: 'Test string',
        customMirroringProfile: 'Test string',
        dataPathId: 'Test string',
        description: 'Test string',
        etag: 'Test string',
        labels: {
          A: 'Test string',
        },
        name: 'Test string',
        threatPreventionProfile: 'Test string',
        updateTime: 'Test string',
        urlFilteringProfile: 'Test string',
        wildfireAnalysisProfile: 'Test string',
      },
    );
    /** Creates a new SecurityProfile in a given project and location. */
    await gapi.client.networksecurity.projects.locations.securityProfiles.create(
      {
        parent: 'Test string',
        securityProfileId: 'Test string',
      },
      {
        createTime: 'Test string',
        customInterceptProfile: {
          interceptEndpointGroup: 'Test string',
        },
        customMirroringProfile: {
          mirroringEndpointGroup: 'Test string',
        },
        description: 'Test string',
        etag: 'Test string',
        labels: {
          A: 'Test string',
        },
        name: 'Test string',
        threatPreventionProfile: {
          antivirusOverrides: [
            {
              action: 'Test string',
              protocol: 'Test string',
            },
          ],
          severityOverrides: [
            {
              action: 'Test string',
              severity: 'Test string',
            },
          ],
          threatOverrides: [
            {
              action: 'Test string',
              threatId: 'Test string',
              type: 'Test string',
            },
          ],
        },
        type: 'Test string',
        updateTime: 'Test string',
        urlFilteringProfile: {
          urlFilters: [
            {
              filteringAction: 'Test string',
              priority: 42,
              urls: ['Test string'],
            },
          ],
        },
        wildfireAnalysisProfile: {
          wildfireInlineCloudAnalysisRules: [
            {
              action: 'Test string',
              customFileTypes: {
                fileTypes: ['Test string'],
              },
              direction: 'Test string',
              fileSelectionMode: 'Test string',
            },
          ],
          wildfireInlineMlOverrides: [
            {
              action: 'Test string',
              protocol: 'Test string',
            },
          ],
          wildfireInlineMlSetting: {
            fileExceptions: [
              {
                filename: 'Test string',
                partialHash: 'Test string',
              },
            ],
            inlineMlConfigs: [
              {
                action: 'Test string',
                fileType: 'Test string',
              },
            ],
          },
          wildfireInlineMlSettings: [
            {
              fileExceptions: [
                {
                  filename: 'Test string',
                  partialHash: 'Test string',
                },
              ],
              inlineMlConfigs: [
                {
                  action: 'Test string',
                  fileType: 'Test string',
                },
              ],
            },
          ],
          wildfireOverrides: [
            {
              action: 'Test string',
              protocol: 'Test string',
            },
          ],
          wildfireRealtimeLookup: true,
          wildfireSubmissionRules: [
            {
              customFileTypes: {
                fileTypes: ['Test string'],
              },
              direction: 'Test string',
              fileSelectionMode: 'Test string',
            },
          ],
          wildfireThreatOverrides: [
            {
              action: 'Test string',
              threatId: 'Test string',
            },
          ],
        },
      },
    );
    /** Deletes a single SecurityProfile. */
    await gapi.client.networksecurity.projects.locations.securityProfiles.delete(
      {
        etag: 'Test string',
        name: 'Test string',
      },
    );
    /** Gets details of a single SecurityProfile. */
    await gapi.client.networksecurity.projects.locations.securityProfiles.get({
      name: 'Test string',
    });
    /** Lists SecurityProfiles in a given project and location. */
    await gapi.client.networksecurity.projects.locations.securityProfiles.list({
      pageSize: 42,
      pageToken: 'Test string',
      parent: 'Test string',
    });
    /** Updates the parameters of a single SecurityProfile. */
    await gapi.client.networksecurity.projects.locations.securityProfiles.patch(
      {
        name: 'Test string',
        updateMask: 'Test string',
      },
      {
        createTime: 'Test string',
        customInterceptProfile: {
          interceptEndpointGroup: 'Test string',
        },
        customMirroringProfile: {
          mirroringEndpointGroup: 'Test string',
        },
        description: 'Test string',
        etag: 'Test string',
        labels: {
          A: 'Test string',
        },
        name: 'Test string',
        threatPreventionProfile: {
          antivirusOverrides: [
            {
              action: 'Test string',
              protocol: 'Test string',
            },
          ],
          severityOverrides: [
            {
              action: 'Test string',
              severity: 'Test string',
            },
          ],
          threatOverrides: [
            {
              action: 'Test string',
              threatId: 'Test string',
              type: 'Test string',
            },
          ],
        },
        type: 'Test string',
        updateTime: 'Test string',
        urlFilteringProfile: {
          urlFilters: [
            {
              filteringAction: 'Test string',
              priority: 42,
              urls: ['Test string'],
            },
          ],
        },
        wildfireAnalysisProfile: {
          wildfireInlineCloudAnalysisRules: [
            {
              action: 'Test string',
              customFileTypes: {
                fileTypes: ['Test string'],
              },
              direction: 'Test string',
              fileSelectionMode: 'Test string',
            },
          ],
          wildfireInlineMlOverrides: [
            {
              action: 'Test string',
              protocol: 'Test string',
            },
          ],
          wildfireInlineMlSetting: {
            fileExceptions: [
              {
                filename: 'Test string',
                partialHash: 'Test string',
              },
            ],
            inlineMlConfigs: [
              {
                action: 'Test string',
                fileType: 'Test string',
              },
            ],
          },
          wildfireInlineMlSettings: [
            {
              fileExceptions: [
                {
                  filename: 'Test string',
                  partialHash: 'Test string',
                },
              ],
              inlineMlConfigs: [
                {
                  action: 'Test string',
                  fileType: 'Test string',
                },
              ],
            },
          ],
          wildfireOverrides: [
            {
              action: 'Test string',
              protocol: 'Test string',
            },
          ],
          wildfireRealtimeLookup: true,
          wildfireSubmissionRules: [
            {
              customFileTypes: {
                fileTypes: ['Test string'],
              },
              direction: 'Test string',
              fileSelectionMode: 'Test string',
            },
          ],
          wildfireThreatOverrides: [
            {
              action: 'Test string',
              threatId: 'Test string',
            },
          ],
        },
      },
    );
    /** Creates a new ServerTlsPolicy in a given project and location. */
    await gapi.client.networksecurity.projects.locations.serverTlsPolicies.create(
      {
        parent: 'Test string',
        serverTlsPolicyId: 'Test string',
      },
      {
        allowOpen: true,
        createTime: 'Test string',
        description: 'Test string',
        labels: {
          A: 'Test string',
        },
        mtlsPolicy: {
          clientValidationCa: [
            {
              certificateProviderInstance: {
                pluginInstance: 'Test string',
              },
              grpcEndpoint: {
                targetUri: 'Test string',
              },
            },
          ],
          clientValidationMode: 'Test string',
          clientValidationTrustConfig: 'Test string',
        },
        name: 'Test string',
        serverCertificate: {
          certificateProviderInstance: {
            pluginInstance: 'Test string',
          },
          grpcEndpoint: {
            targetUri: 'Test string',
          },
        },
        updateTime: 'Test string',
      },
    );
    /** Deletes a single ServerTlsPolicy. */
    await gapi.client.networksecurity.projects.locations.serverTlsPolicies.delete(
      {
        name: 'Test string',
      },
    );
    /** Gets details of a single ServerTlsPolicy. */
    await gapi.client.networksecurity.projects.locations.serverTlsPolicies.get({
      name: 'Test string',
    });
    /** Gets the access control policy for a resource. Returns an empty policy if the resource exists and does not have a policy set. */
    await gapi.client.networksecurity.projects.locations.serverTlsPolicies.getIamPolicy(
      {
        'options.requestedPolicyVersion': 42,
        resource: 'Test string',
      },
    );
    /** Lists ServerTlsPolicies in a given project and location. */
    await gapi.client.networksecurity.projects.locations.serverTlsPolicies.list(
      {
        pageSize: 42,
        pageToken: 'Test string',
        parent: 'Test string',
        returnPartialSuccess: true,
      },
    );
    /** Updates the parameters of a single ServerTlsPolicy. */
    await gapi.client.networksecurity.projects.locations.serverTlsPolicies.patch(
      {
        name: 'Test string',
        updateMask: 'Test string',
      },
      {
        allowOpen: true,
        createTime: 'Test string',
        description: 'Test string',
        labels: {
          A: 'Test string',
        },
        mtlsPolicy: {
          clientValidationCa: [
            {
              certificateProviderInstance: {
                pluginInstance: 'Test string',
              },
              grpcEndpoint: {
                targetUri: 'Test string',
              },
            },
          ],
          clientValidationMode: 'Test string',
          clientValidationTrustConfig: 'Test string',
        },
        name: 'Test string',
        serverCertificate: {
          certificateProviderInstance: {
            pluginInstance: 'Test string',
          },
          grpcEndpoint: {
            targetUri: 'Test string',
          },
        },
        updateTime: 'Test string',
      },
    );
    /** Sets the access control policy on the specified resource. Replaces any existing policy. Can return `NOT_FOUND`, `INVALID_ARGUMENT`, and `PERMISSION_DENIED` errors. */
    await gapi.client.networksecurity.projects.locations.serverTlsPolicies.setIamPolicy(
      {
        resource: 'Test string',
      },
      {
        policy: {
          auditConfigs: [
            {
              auditLogConfigs: [
                {
                  exemptedMembers: ['Test string'],
                  logType: 'Test string',
                },
              ],
              service: 'Test string',
            },
          ],
          bindings: [
            {
              condition: {
                description: 'Test string',
                expression: 'Test string',
                location: 'Test string',
                title: 'Test string',
              },
              members: ['Test string'],
              role: 'Test string',
            },
          ],
          etag: 'Test string',
          version: 42,
        },
        updateMask: 'Test string',
      },
    );
    /** Returns permissions that a caller has on the specified resource. If the resource does not exist, this will return an empty set of permissions, not a `NOT_FOUND` error. Note: This operation is designed to be used for building permission-aware UIs and command-line tools, not for authorization checking. This operation may "fail open" without warning. */
    await gapi.client.networksecurity.projects.locations.serverTlsPolicies.testIamPermissions(
      {
        resource: 'Test string',
      },
      {
        permissions: ['Test string'],
      },
    );
    /** Creates a new TlsInspectionPolicy in a given project and location. */
    await gapi.client.networksecurity.projects.locations.tlsInspectionPolicies.create(
      {
        parent: 'Test string',
        tlsInspectionPolicyId: 'Test string',
      },
      {
        caPool: 'Test string',
        certificateIssuanceMode: 'Test string',
        createTime: 'Test string',
        customTlsFeatures: ['Test string'],
        description: 'Test string',
        excludePublicCaSet: true,
        minTlsVersion: 'Test string',
        name: 'Test string',
        tlsFeatureProfile: 'Test string',
        trustConfig: 'Test string',
        updateTime: 'Test string',
      },
    );
    /** Deletes a single TlsInspectionPolicy. */
    await gapi.client.networksecurity.projects.locations.tlsInspectionPolicies.delete(
      {
        force: true,
        name: 'Test string',
      },
    );
    /** Gets details of a single TlsInspectionPolicy. */
    await gapi.client.networksecurity.projects.locations.tlsInspectionPolicies.get(
      {
        name: 'Test string',
      },
    );
    /** Lists TlsInspectionPolicies in a given project and location. */
    await gapi.client.networksecurity.projects.locations.tlsInspectionPolicies.list(
      {
        pageSize: 42,
        pageToken: 'Test string',
        parent: 'Test string',
      },
    );
    /** Updates the parameters of a single TlsInspectionPolicy. */
    await gapi.client.networksecurity.projects.locations.tlsInspectionPolicies.patch(
      {
        name: 'Test string',
        updateMask: 'Test string',
      },
      {
        caPool: 'Test string',
        certificateIssuanceMode: 'Test string',
        createTime: 'Test string',
        customTlsFeatures: ['Test string'],
        description: 'Test string',
        excludePublicCaSet: true,
        minTlsVersion: 'Test string',
        name: 'Test string',
        tlsFeatureProfile: 'Test string',
        trustConfig: 'Test string',
        updateTime: 'Test string',
      },
    );
    /** Creates a new UrlList in a given project and location. */
    await gapi.client.networksecurity.projects.locations.urlLists.create(
      {
        parent: 'Test string',
        urlListId: 'Test string',
      },
      {
        createTime: 'Test string',
        description: 'Test string',
        name: 'Test string',
        updateTime: 'Test string',
        values: ['Test string'],
      },
    );
    /** Deletes a single UrlList. */
    await gapi.client.networksecurity.projects.locations.urlLists.delete({
      name: 'Test string',
    });
    /** Gets details of a single UrlList. */
    await gapi.client.networksecurity.projects.locations.urlLists.get({
      name: 'Test string',
    });
    /** Lists UrlLists in a given project and location. */
    await gapi.client.networksecurity.projects.locations.urlLists.list({
      pageSize: 42,
      pageToken: 'Test string',
      parent: 'Test string',
    });
    /** Updates the parameters of a single UrlList. */
    await gapi.client.networksecurity.projects.locations.urlLists.patch(
      {
        name: 'Test string',
        updateMask: 'Test string',
      },
      {
        createTime: 'Test string',
        description: 'Test string',
        name: 'Test string',
        updateTime: 'Test string',
        values: ['Test string'],
      },
    );
  }
});
