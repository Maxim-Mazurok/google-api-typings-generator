...
declare namespace gapi.client {
    ...
    namespace datacatalog {
        ...
        interface GoogleCloudDatacatalogV1SearchCatalogRequest {
             ...
             * (`desc`) as default If this parameter is omitted, it defaults to the descending `relevance`.
             */
            orderBy?: string;
            /**
            ...
            pageToken?: string;
            /**
             * Optional. The query string with a minimum of 3 characters and specific syntax. For more information, see [Data Catalog search syntax](/data-catalog/docs/how-to/search-reference). An
             * empty query string returns all data assets (in the specified scope) that you have access to. A query string can be a simple `xyz` or qualified by predicates: * `name:x` * `column:y`
             * * `description:z`
             */
            query?: string;
            /**
        ...
        interface GoogleCloudDatacatalogV1SearchCatalogRequestScope {
            ...
            /**
             * Optional. If `true`, include public tag templates in the search results. By default, they are included only if you have explicit permissions on them to view them. For example, if
             * you are the owner. Other scope fields, for example, ``include_org_ids``, still restrict the returned public tag templates and at least one of them is required.
             */
        ...
        interface GoogleCloudDatacatalogV1TagTemplate {
            ...
            /**
             * Indicates whether this is a public tag template. Every user has view access to a *public* tag template by default. This means that: * Every user can use this tag template to tag an
             * entry. * If an entry is tagged using the tag template, the tag is always shown in the response to ``ListTags`` called on the entry. * To get the template using the GetTagTemplate
             * method, you need view access either on the project or the organization the tag template resides in but no other permission is needed. * Operations on the tag template other than
             * viewing (for example, editing IAM policies) follow standard IAM structures. Tags created with a public tag template are referred to as public tags. You can search for a public tag
             * by value with a simple search query instead of using a ``tag:`` predicate. Public tag templates may not appear in search results depending on scope, see:
             * include_public_tag_templates Note: If an [IAM domain restriction](https://cloud.google.com/resource-manager/docs/organization-policy/restricting-domains) is configured in the tag
             * template's location, the public access will not be enabled but the simple search for tag values will still work.
             */
        ...
        interface TagTemplatesResource {
            ...
            patch(request: {
                 ...
                 * all modifiable fields are overwritten. If such fields are non-required and omitted in the request body, their values are emptied. Note: Updating the ``is_publicly_readable``
                 * field may require up to 12 hours to take effect in search results. Additionally, it also requires the ``tagTemplates.getIamPolicy`` and ``tagTemplates.setIamPolicy``
                 * permissions.
                 */
                updateMask?: string;
                /** Upload protocol for media (e.g. "raw", "multipart"). */
