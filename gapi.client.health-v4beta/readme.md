# TypeScript typings for Google Health API v4beta
...
```
npm install @types/gapi.client.health-v4beta --save-dev
```
...
{
  "compilerOptions": {
    "types": ["gapi", "gapi.auth2", "gapi.client", "gapi.client.health-v4beta"]
  }
}
```
...
```typescript
gapi.client.load(
  'https://health.googleapis.com/$discovery/rest?version=v4beta',
  () => {
    // now we can use:
    // gapi.client.health
  },
...
gapi.client.load('health', 'v4beta', () => {
  // now we can use:
  // gapi.client.health
});
```
...
For provenance information see [Provenance section on NPM](https://www.npmjs.com/package/@maxim_mazurok/gapi.client.health-v4beta#Provenance:~:text=none-,Provenance,-Built%20and%20signed)
