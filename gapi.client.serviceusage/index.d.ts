...
declare namespace gapi.client {
    ...
    function load(name: "serviceusage", version: "v1"): PromiseLike<void>;
    ...
    namespace serviceusage {
        interface AdminQuotaPolicy {
             ...
             * in that region. This map has the following restrictions: ∗ If "region" appears as a key, its value must be a valid Cloud region. ∗ If "zone" appears as
             * a key, its value must be a valid Cloud zone. ∗ Keys other than "region" or "zone" are not valid.
             */
        ...
        interface BatchEnableServicesRequest {
            /**
             * The identifiers of the services to enable on the project. A valid identifier would be: serviceusage.googleapis.com Enabling services requires that each
             * service is public or is shared with the user enabling the service. A single request can enable a maximum of 20 services at a time. If more than 20
        ...
        interface DisableServiceRequest {
            ...
            /**
             * Indicates if services that are enabled and which depend on this service should also be disabled. If not set, an error will be generated if any enabled
             * services depend on the service to be disabled. When set, the service, and any enabled services that depend on it, will be disabled together.
             */
        ...
        interface GetServiceIdentityResponse {
            /**
             * Service identity that service producer can use to access consumer resources. If exists is true, it contains email and unique_id. If exists is false, it
        ...
        interface GoogleApiServiceusageV1Service {
            /**
             * The service configuration of the available service. Some fields may be filtered out of the configuration in responses to the `ListServices` method.
        ...
        interface ServicesResource {
            ...
            batchEnable(request: {
                ...
                oauth_token?: string;
                /**
                 * Parent to enable services on. An example name would be: `projects/123` where `123` is the project number. The `BatchEnableServices` method currently
            ...
            batchGet(request?: {
                ...
                key?: string;
                /**
                 * Names of the services to retrieve. An example name would be: `projects/123/services/serviceusage.googleapis.com` where `123` is the project number. A
                ...
                oauth_token?: string;
                /**
                 * Parent to retrieve services from. If this is set, the parent of all of the services specified in `names` must match this field. An example name would
                 * be: `projects/123` where `123` is the project number. The `BatchGetServices` method currently only supports projects.
                 */
                parent: string;
                /** Returns response with indentations and line breaks. */
                prettyPrint?: boolean;
            ...
            enable(request: {
                ...
                key?: string;
                /**
                 * Name of the consumer and service to enable the service on. The `EnableService` and `DisableService` methods currently only support projects. Enabling a
                 * service requires that the service is public or is shared with the user enabling the service. An example name would be:
                 * `projects/123/services/serviceusage.googleapis.com` where `123` is the project number.
                 */
                name: string;
                /** OAuth 2.0 token for the current user. */
            ...
            get(request?: {
                ...
                key?: string;
                /**
                 * Name of the consumer and service to get the `ConsumerState` for. An example name would be: `projects/123/services/serviceusage.googleapis.com` where
