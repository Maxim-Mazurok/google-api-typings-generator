...
declare namespace gapi.client {
    ...
    namespace games {
        ...
        interface ResolveSnapshotHeadRequest {
            /**
        ...
        interface ResolveSnapshotHeadResponse {
        ...
        interface SnapshotCoverImageResource {
            /**
             * Output only. Hash-like weak identifier of the uploaded image bytes, consistent per player per application per hash version. Within the context of a single player/application, it's
             * guaranteed that two identical images coming from two different uploads will have the same content hash for the same hash algorithm version. It's extremely likely, though not
             * guaranteed, that if two content hashes are equal, the images are identical. More than one content hash can be returned if more than one hash versions are supported.
             */
        ...
        interface SnapshotDataResource {
            /**
             * Output only. Hash-like weak identifier of the uploaded blob, consistent per player per application per hash version. Within the context of a single player/application, it's
             * guaranteed that two identical blobs coming from two different uploads will have the same content hash for the same hash algorithm version. It's extremely likely, though not
             * guaranteed, that if two content hashes are equal, the blobs are identical. More than one content hash can be returned if more than one hash versions are supported.
             */
            contentHash?: ContentHash[];
        ...
        interface SnapshotExtended {
             ...
             */
            conflictingRevisions?: SnapshotRevision[];
        ...
        interface SnapshotsExtendedResource {
            ...
            resolveSnapshotHead(request: {
                ...
                uploadType?: string;
                /** Request body */
                resource: ResolveSnapshotHeadRequest;
            }): Request<ResolveSnapshotHeadResponse>;
            resolveSnapshotHead(request: {
                /** V1 error format. */
                "$.xgafv"?: string;
                /** OAuth access token. */
            ...
            body: ResolveSnapshotHeadRequest): Request<ResolveSnapshotHeadResponse>;
        }
        ...
        const snapshotsExtended: SnapshotsExtendedResource;
