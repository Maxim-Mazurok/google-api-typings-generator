...
declare namespace gapi.client {
    ...
    namespace chat {
        ...
        interface ActionStatus {
            ...
            statusCode?: string;
            /**
             * This message will be the corresponding string to the above status_code. If unset, an appropriate generic message based on the status_code will be shown to the user. If this field is
             * set then the message will be surfaced to the user for both successes and errors.
             */
        ...
        interface Dialog {
            /**
             * Body of the dialog, which will be rendered in a modal. NOTE: The following fields within the objects are not supported: google.apps.card.v1.Widget.date_time_picker
             * google.apps.card.v1.DecoratedText.SwitchControl.on_change_action google.apps.card.v1.TextInput.on_change_action google.apps.card.v1.SelectionInput.on_change_action
             * google.apps.card.v1.DateTimePicker.on_change_action Setting the fields above will have no effect on the dialog.
             */
        ...
        interface DynamiteIntegrationLogEntry {
        ...
        interface Membership {
            ...
            createTime?: string;
            /**
             * A user in Google Chat. Represents a person in the People API. Formatted as `users/person_id` where `person_id` is available from the [People
        ...
        interface ConversationsResource {
            ...
            messages(request: {
                ...
                quotaUser?: string;
                /**
                 * Optional. A unique request ID for this message. If a message has already been created in the space with this request ID, the subsequent request will return the existing message
                 * and no new message will be created.
                 */
                requestId?: string;
                /**
                 * Optional. Opaque thread identifier string that can be specified to group messages into a single thread. If this is the first message with a given thread identifier, a new thread
                 * is created. Subsequent messages with the same thread identifier will be posted into the same thread. This relieves bots and webhooks from having to store the Google Chat thread
        ...
        interface MessagesResource {
            ...
            update(request: {
                ...
                quotaUser?: string;
                /**
                 * Required. The field paths to be updated, comma separated if there are multiple. Currently supported field paths: * text * cards * gsuite_message_integration_render_data *
                 * attachment
                 */
                updateMask?: string;
                /** Upload protocol for media (e.g. "raw", "multipart"). */
